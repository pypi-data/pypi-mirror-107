...
% else:
    ...
    % for article in query.limit(10):
                            <a class="list-group-item" href="${request.route_path('articles.visual', article=article.id)}" title="${translate('View article "{0}"').format(article.title)}">${article.title}</a>
    % endfor
                        </div><!-- /.list-group -->
                    </div><!-- /.panel -->
                </aside>
