"""
 * $Id: 1.1+4 utils.py d9a751b54766 2021-05-28 od $
 *
...
class Client(object):
    ...
    RESP_TIMEOUT = 30 # for "resp_timeout"
    RETRY = 3  # for "retry"
    ...
    def __get0(url: str, conf: Dict):
        """HTTP GET to generate a Client"""
        conf = conf or {}
        timeout = (conf.get('conn_timeout', Client.CONN_TIMEOUT), conf.get('read_timeout', Client.READ_TIMEOUT))
    ...
    def __post0(url: str, files: Dict, headers: Dict, conf: Dict, sync: bool = True):
        ...
        conf = conf or {}
        if sync:
            timeout = (conf.get('conn_timeout', Client.CONN_TIMEOUT), conf.get('resp_timeout', Client.RESP_TIMEOUT))
        else:
            timeout = (conf.get('conn_timeout', Client.CONN_TIMEOUT), conf.get('read_timeout', Client.READ_TIMEOUT))
    ...
    def execute_async(self, url: str = None, files: List[str] = None,
        ...
        exp = int(time.time() * 1000.0) + int(conf.get('resp_timeout', Client.RESP_TIMEOUT) * 1000.0)
