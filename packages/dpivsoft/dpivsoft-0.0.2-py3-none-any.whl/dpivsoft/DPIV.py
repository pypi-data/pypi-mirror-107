...
def corrFFT1(Img1, Img2):
    ...
    if Parameters.weighting:
        [i_matrix, j_matrix] = np.meshgrid(np.arange(box_size_x), np.arange(box_size_y))
        Weighting_Function = weight_function(i_matrix, j_matrix, box_size_x, box_size_y)

    for i in range(0, no_boxes_1_x):
        for j in range(0, no_boxes_1_y):

            box_origin_x_1[j,i] = (1 + np.round((i)*(Width-box_size_x-2)/(no_boxes_1_x-1)))
            box_origin_y_1[j,i] = (1 + np.round((j)*(Height-box_size_y-2)/(no_boxes_1_y-1)))

            x_1[j,i]= (box_origin_x_1[j,i]-1+box_size_x/2)
            y_1[j,i]= (box_origin_y_1[j,i]-1+box_size_y/2)

            #Define sub images to work
            SubImg1 = (Img1[int(box_origin_y_1[j,i]):int(box_origin_y_1[j,i])+box_size_y,
                int(box_origin_x_1[j,i]):int(box_origin_x_1[j,i])+box_size_x])
            SubImg2 = (Img2[int(box_origin_y_1[j,i]):int(box_origin_y_1[j,i])+box_size_y,
                int(box_origin_x_1[j,i]):int(box_origin_x_1[j,i])+box_size_x])
            ...
            #Cross Correlation Function of the pair images scaled with sigma1 and sigma2
            correlation = np.fft.fftshift(np.real(np.fft.ifft2(np.multiply(np.conj(
                np.fft.fft2(SubImg1)), np.fft.fft2(SubImg2)))))/(Sigma1*Sigma2)

            #Find Peaks of the correlation Fucntion
            [epsilon_x,epsilon_y,col_idx,row_idx] = find_peaks(correlation, box_size_x,
                    box_size_y, window_x, window_y)
...
def corrFFT1bis(Img1, Img2, x_1, y_1, u_1, v_1):
    ...
    for calc in range(1,Parameters.no_iter_1):
        ...
        [du_dx,du_dy,dv_dx,dv_dy] = Jacobian_Matrix(u_1, v_1, x_1, y_1, no_boxes_1_x, no_boxes_1_y)

        for j in range(0,no_boxes_1_y):
            for i in range(0,no_boxes_1_y):

                #Obtain deformed image.
                [SubImg1, SubImg2, u_index, v_index] = deform_image(Img1, Img2, Width, Height,
                        box_origin_x, box_origin_y, i_matrix, j_matrix, box_size_x, box_size_y,
                        u_1, v_1, du_dx, du_dy, dv_dx, dv_dy, i, j)
                ...
                #Cross Correlation Function of the pair images scaled with sigma1 and sigma2
                correlation = np.fft.fftshift(np.abs(np.fft.ifft2(np.multiply(np.conj(
                    np.fft.fft2(SubImg1)), np.fft.fft2(SubImg2)))))/(Sigma1*Sigma2)
                ...
                u_1[j,i] = u_index[row_idx, col_idx]+epsilon_x+col_idx-box_size_x/2
                v_1[j,i] = v_index[row_idx, col_idx]+epsilon_y+row_idx-box_size_y/2
...
def corrFFT2(Img1, Img2, x_1, y_1, u_1, v_1):
    ...
    i_matrix, j_matrix = np.meshgrid(np.arange(0, box_size_2_x), np.arange(0, box_size_2_y))

    if Parameters.weighting:
        Weighting_Function = weight_function(i_matrix, j_matrix, box_size_2_x, box_size_2_y)
    ...
    x_2 = 2+np.round(np.arange(0,no_boxes_2_x)*(Width-x_margin-6)/(no_boxes_2_x-1)+x_margin/2)
    y_2 = 2+np.round(np.arange(0,no_boxes_2_y)*(Height-y_margin-6)/(no_boxes_2_y-1)+y_margin/2)
    ...
    du_dx, du_dy, dv_dx, dv_dy, u_2, v_2, x_2, y_2 = interpolations(du_dx, du_dy, dv_dx, dv_dy,
            u_1, v_1, x_1, y_1, x_2, y_2, no_boxes_1_x*no_boxes_1_y)
    ...
    for j in range(0,no_boxes_2_y):
        for i in range(0,no_boxes_2_x):
            ...
            while ((np.abs(epsilon_x>0.5) or np.abs(epsilon_y>0.5)) and k<Parameters.no_iter_2):
                k = k+1

                #Obtain deformed image.
                SubImg1, SubImg2, u_index, v_index = deform_image(Img1, Img2, Width, Height,
                        box_origin_x_2, box_origin_y_2, i_matrix, j_matrix, box_size_2_x, box_size_2_y,
                ...
                u_2[j,i] = u_index[row_idx,col_idx]+epsilon_x+col_idx-box_size_2_x/2
                v_2[j,i] = v_index[row_idx,col_idx]+epsilon_y+row_idx-box_size_2_y/2
...
def find_peaks(correlation, box_size_x, box_size_y, window_x, window_y):
    ...
    #Find first peak
    maxcor1 = (np.amax(
        correlation[int(np.round(box_size_y/2-window_y/2)):int(np.round(box_size_y/2+window_y/2+1)),
        int(np.round(box_size_x/2-window_x/2)):int(np.round(box_size_x/2+window_x/2+1))]))
    ...
    #Find second peak
    maxcor2 = (np.amax(
        correlation_z[int(np.round(box_size_y/2-window_y/2)):int(np.round(box_size_y/2+window_y/2+1)),
        int(np.round(box_size_x/2-window_x/2)):int(np.round(box_size_x/2+window_x/2+1))]))
    ...
    matmax2 = correlation[np.round(max_row2-1):np.round(max_row2+2),np.round(max_col2-1):np.round(max_col2+2)]
    ...
    fit_peak = (np.divide(np.divide(fit_peak,1-np.abs(max_row-box_size_y/2 +
        weight_j)/box_size_y),1-np.abs(max_col-box_size_x/2+weight_i)/box_size_x))
    ...
    if (fit_peak[1,1] == np.amin(fit_peak[1,:])):
        ...
        if (fit_peak[1,0]>0 and fit_peak[1,1]>0 and fit_peak[1,2]>0 and
                (np.log(fit_peak[1,0])+np.log(fit_peak[1,2])-2*np.log(fit_peak[1,1])) != 0):
            epsilon_x = (0.5*(np.log(fit_peak[1,0])-np.log(fit_peak[1,2]))
                    /(np.log(fit_peak[1,0])+np.log(fit_peak[1,2])-2*np.log(fit_peak[1,1])))
        else:
            epsilon_x = 0
    ...
    if (fit_peak[1,1] == np.amin(fit_peak[:,1])):
        ...
        if (fit_peak[0,1]>0 and fit_peak[1,1]>0 and fit_peak[2,1]>0 and
                (np.log(fit_peak[0,1])+np.log(fit_peak[2,1])-2*np.log(fit_peak[1,1])) != 0):
            epsilon_y = (0.5*(np.log(fit_peak[0,1])-np.log(fit_peak[2,1]))
                    /(np.log(fit_peak[0,1])+np.log(fit_peak[2,1])-2*np.log(fit_peak[1,1])))
        else:
            epsilon_y = 0
...
def median_filter(u,v,limit):
    ...
    for j in range(1,num_row-1):
        ...
        delta_u = np.sqrt(np.power(u[j,1:num_col-1]-u_median,2) + np.power(v[j,1:num_col-1]-v_median,2))

        #counter of vectors changed
        change_u = 1/2+np.sign(delta_u-median_magnitude*limit)/2

        #save final value
        uf[j,1:num_col-1] = u[j,1:num_col-1] + np.multiply(change_u,(u_median-u[j,1:num_col-1]))
        vf[j,1:num_col-1] = v[j,1:num_col-1] + np.multiply(change_u,(v_median-v[j,1:num_col-1]))
    ...
    #Change the outlier vectors on the upper row using with 6-neighbors medians
    j=num_row-1
    u_neigh[0,:] = u[j-1,0:num_col-2]
    u_neigh[1,:] = u[j-1,1:num_col-1]
    u_neigh[2,:] = u[j-1,2:num_col]
    ...
    median_magnitude = np.sqrt(np.power(u_median,2) + np.power(v_median,2))
    delta_u = np.sqrt(np.power(u[1:num_row-1,j]-u_median,2) + np.power(v[1:num_row-1,j]-v_median,2))

    #counter of vectors changed
    change_u = 1/2+np.sign(delta_u-median_magnitude*limit)/2

    #save final value
    uf[1:num_row-1,j] = u[1:num_row-1,j]+ np.multiply(change_u,(u_median-u[1:num_row-1,j]))
    vf[1:num_row-1,j] = v[1:num_row-1,j]+ np.multiply(change_u,(v_median-v[1:num_row-1,j]))
    err_vect = err_vect+np.sum(change_u)

    #Change the outlier vectors on the right colum row using with 6-neighbors medians
    j = num_col-1
    ...
    u_median = np.median([u[num_row-1,0], u[num_row-1,1], u[num_row-2,0], u[num_row-2,1]])
    v_median = np.median([v[num_row-1,0], v[num_row-1,1], v[num_row-2,0], v[num_row-2,1]])
    median_magnitude = np.sqrt(u_median*u_median + v_median*v_median)
    delta_u = np.sqrt((u[num_row-1,0]-u_median)**2 +(v[num_row-1,0]-v_median)**2)
    if delta_u>median_magnitude*limit:
    ...
    u_median = np.median([u[0,num_col-1], u[1,num_col-1], u[0,num_col-2], u[1,num_col-2]])
    v_median = np.median([v[0,num_col-1], v[1,num_col-1], v[0,num_col-2], v[1,num_col-2]])
    median_magnitude = np.sqrt(u_median*u_median + v_median*v_median)
    delta_u = np.sqrt((u[0,num_col-1]-u_median)**2 +(v[0,num_col-1]-v_median)**2)
    if delta_u > median_magnitude*limit:
    ...
    median_magnitude = np.sqrt(u_median*u_median + v_median*v_median)
    delta_u = np.sqrt((u[num_row-1,num_col-1]-u_median)**2+(v[num_row-1,num_col-1]-v_median)**2)
    if delta_u > median_magnitude*limit:
...
def translated_pixels(i_index, j_index, u_index, v_index, Width, Height, box_size_x, box_size_y):
    ...
    return i_frac_1, j_frac_1, i_frac_2, j_frac_2, j_index_1, i_index_1, j_index_2, i_index_2


def deform_image(Img1, Img2, Width, Height, box_origin_x, box_origin_y,
        i_matrix, j_matrix, box_size_x, box_size_y, u, v, du_dx, du_dy, dv_dx, dv_dy, i, j):
    ...
    u_index = (u[j,i]*np.ones([box_size_y,box_size_x])+du_dx[j,i]*i_matrix+du_dy[j,i]*j_matrix
        -(box_size_x)/2*(du_dx[j,i]+du_dy[j,i])*np.ones([box_size_y,box_size_x]))

    v_index = (v[j,i]*np.ones([box_size_y,box_size_x])+dv_dx[j,i]*i_matrix+dv_dy[j,i]*j_matrix
        - (box_size_y)/2*(dv_dx[j,i]+dv_dy[j,i])*np.ones([box_size_y,box_size_x]))


    #Position of translated pixels of images 1 and 2 according to velocity field
    [i_frac_1,j_frac_1,i_frac_2,j_frac_2,j_index_1,i_index_1,j_index_2,i_index_2] = translated_pixels(
            i_index, j_index, u_index, v_index, Width,Height, box_size_x, box_size_y)

    #Define sub_image1 deformed according to velocity field
    SubImg1 = (np.multiply(np.multiply(1-j_frac_1,1-i_frac_1),Img1[j_index_1.astype(int),i_index_1.astype(int)])
        +np.multiply(np.multiply(1-j_frac_1,i_frac_1),Img1[j_index_1.astype(int),i_index_1.astype(int)+1])
        +np.multiply(np.multiply(j_frac_1,1-i_frac_1),Img1[j_index_1.astype(int)+1,i_index_1.astype(int)])
        +np.multiply(np.multiply(j_frac_1,i_frac_1),Img1[j_index_1.astype(int)+1,i_index_1.astype(int)+1]))
    SubImg1 = SubImg1 - np.sum(np.sum(SubImg1))/(box_size_x*box_size_y)

    #Define sub_image2 deformed according to velocity field
    SubImg2 = (np.multiply(np.multiply(1-j_frac_2,1-i_frac_2),Img2[j_index_2.astype(int),i_index_2.astype(int)])
        +np.multiply(np.multiply(1-j_frac_2,i_frac_2),Img2[j_index_2.astype(int),i_index_2.astype(int)+1])
        +np.multiply(np.multiply(j_frac_2,1-i_frac_2),Img2[j_index_2.astype(int)+1,i_index_2.astype(int)])
        +np.multiply(np.multiply(j_frac_2,i_frac_2),Img2[j_index_2.astype(int)+1,i_index_2.astype(int)+1]))
...
def weight_function(i_matrix, j_matrix, box_size_x, box_size_y):
    ...
    Weighting_Func = 9*np.multiply(4*np.power(zeta,2)-4*np.abs(zeta)+1,4*np.power(zeta,2)-4*np.abs(eta)+1)
