...
class D80Image(DOSImage):
    ...
    def __init__(self, filename):
        ...
        self.bam_blocks = []
        for sector in self.BAM_SECTORS:
            self.bam_blocks.append(D80D82BAMBlock(self, self.BAM_TRACK, sector))
    ...
    def dos_version(self):
        return self.dir_block.get(0x1b)

    @property
    def name(self):
        name = self.dir_block.get(0x06, 0x16)
        return name.rstrip(b'\xa0')

    @property
    def id(self):
        id = self.dir_block.get(0x18, 0x1a)
        return id
    ...
    def dos_version(self, version):
        self.dir_block.set(0x1b, version)

    @name.setter
    def name(self, nam):
        self.dir_block.set(6, nam[:16].ljust(16, b'\xa0'))

    @id.setter
    def id(self, did):
        if len(did) != 2:
            raise ValueError("Invalid disk id, "+str(did))
        self.dir_block.set(0x18, did)

    @dos_type.setter
    def dos_type(self, dtype):
        self.dir_block.set(2, dtype)
        self.dir_block.set(0x1c, dtype)
