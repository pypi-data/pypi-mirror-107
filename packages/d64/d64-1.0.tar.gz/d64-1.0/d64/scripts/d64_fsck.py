...
def check_super_side_sector(path, file_blocks):
    ...
    try:
        ...
        if not IMAGE.bam.is_allocated(super_side_sector.track, super_side_sector.sector):
        ...
        remember_used(super_side_sector.track, super_side_sector.sector)
        sector_count += 1
        ...
        try:
            first_ss_link = super_side_sector.next_block()
            if first_ss_link != first_ss:
                msg = "File {!s}, invalid link to first side sector group {!s}".format(path.name, first_ss_link)
                errors += fix_error(msg, fix_link, block=super_side_sector, link_to=first_ss)
        except ValueError:
            ts = super_side_sector.get(0, 2)
            msg = "File {!s}, invalid link to first side sector group, {:d}:{:d}".format(path.name, *ts)
            errors += fix_error(msg, fix_link, block=super_side_sector, link_to=first_ss)
