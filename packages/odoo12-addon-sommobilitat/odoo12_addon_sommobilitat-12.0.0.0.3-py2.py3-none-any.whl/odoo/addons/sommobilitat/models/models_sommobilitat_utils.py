...
class sommobilitat_utils(object):
  ...
  def fields_map(self):
    return {
      ...
      'member_details_socialr': 'name',
      'member_details_dni': 'vat',
      'member_details_cif': 'vat',
      ...
      'member_details_datebirth': 'birthdate_date',
      'member_details_address': 'street',
      'member_details_zip': 'zip',
      'member_details_province': 'state_id',
      ...
      'member_details_cooperator_register_number': 'cooperator_register_number',
      ...
      for cf in wp_member.custom_fields:
        if cf['key'] in fmap:
          if cf['key'] == 'member_details_datebirth':
            bd = False
            try:
              bd = datetime.strptime(str(cf['value']), "%d/%m/%Y")
            except ValueError:
            ...
            if bool(cf['value']):
              ...
              related_company = parent.env['res.partner'].search([
                ('vat', '=', group_config)
              ], order="cooperator_register_number desc")
          ...
          elif cf['key'] == 'member_details_cooperator_register_number':
            if int(cf['value']) > 0:
              m_data[fmap[cf['key']]] = int(cf['value'])
            else:
              if str(cf['value']) == '-1':
                m_data['cs_user_type'] = 'promo'
              if str(cf['value']) == '-3':
                m_data['cs_user_type'] = 'organisation'
