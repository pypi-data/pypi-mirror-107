...
class Search(metaclass=abc.ABCMeta):
	...
	def __init__(
		...
		sleep_min=0.0,
		sleep_max=1.5,
		languages=['af', 'ach', 'ak', 'am', 'ar', 'az', 'be', 'bem', 'bg', 'bh',
		'bn', 'br', 'bs', 'ca', 'chr', 'ckb', 'co', 'crs', 'cs', 'cy', 'da', 'de',
		'ee', 'el', 'en', 'eo', 'es', 'es-419', 'et', 'eu', 'fa', 'fi', 'fo', 'fr',
		...
		'sd', 'sh', 'si', 'sk', 'sl', 'sn', 'so', 'sq', 'sr', 'sr-ME', 'st', 'su',
		'sv', 'sw', 'ta', 'te', 'tg', 'th', 'ti', 'tk', 'tl', 'tn', 'to', 'tr', 'tt',
		'tum', 'tw', 'ug', 'uk', 'ur', 'uz', 'vi', 'wo', 'xh', 'xx-bork', 'xx-elmer',
		'xx-hacker', 'xx-klingon', 'xx-pirate', 'yi', 'yo', 'zh-CN', 'zh-TW', 'zu'],
		domains=['com', 'al', 'dz', 'as', 'ad', 'am', 'ac', 'at', 'az', 'bs', 'by',
		'be', 'bj', 'ba', 'vg', 'bg', 'bf', 'cm', 'ca', 'cv', 'cat', 'cf', 'td', 'cl',
		'cn', 'cd', 'cg', 'ci', 'hr', 'cz', 'dk', 'dj', 'dm', 'tl', 'ec', 'ee', 'fm',
		'fi', 'fr', 'ga', 'gm', 'ps', 'ge', 'de', 'gr', 'gl', 'gp', 'gg', 'gy', 'ht',
		'hn', 'hu', 'is', 'iq', 'ie', 'im', 'it', 'jp', 'je', 'jo', 'kz', 'ki', 'kg',
		'la', 'lv', 'li', 'lt', 'lu', 'mk', 'mg', 'mw', 'mv', 'ml', 'mu', 'md', 'mn',
		'me', 'ms', 'nr', 'nl', 'ne', 'ng', 'nu', 'no', 'ps', 'pn', 'pl', 'pt', 'ro',
		'ru', 'rw', 'sh', 'ws', 'sm', 'st', 'sn', 'rs', 'sc', 'sk', 'si', 'so', 'es',
		'lk', 'sr', 'ch', 'tg', 'tk', 'to', 'tt', 'tn', 'tm', 'ae', 'vu'],
		pdf_report=False,
		pdf_timeout=30,
			...
			with ThreadPoolExecutor(max_workers=self.social_threads) as executor:
				for link in searched_item["links"]:
					...
					for __social, __social_obj in self.mapping_main.items():
						if (self.mapping_adr[__social] in __domain) and not(__social_obj is None):
...
class Google(Search):
	...
	def search(self, items):
		...
		for item in items:
			self.search_results.append({"item": item, "links": list(search_item(item=str(item)))})
	...
	def search(self, items):
		...
		for item in items:
			self.search_results.append({
				"item": item,
				"links": list(search_item(item=str(item)))
				})
			time.sleep(random.uniform(self.sleep_min, self.sleep_max))
			if self.cache:
				self.cache_json()
...
class PDF:
	...
	def __init__(self, path, summary_params):
		...
		self.__text_decoded = self.__text.decode(chardet.detect(self.__text)['encoding'])
		self.__langs = {
			"ar": "arabic",
			"da": "danish",
			"nl": "dutch",
