...
def main(target, junit):
    ...
    with open(target) as content:
        ...
        # TODO assert environments
        term.empty_line()
        term.header('Environments')
        for name in assertions:
            if 'environment_status' in assertions[name]:
                expected_environment_status = assertions[name]['environment_status']
                actual_environment_status = {}
