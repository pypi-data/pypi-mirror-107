...
class Client:
    ...
    def get_pixel(self, x: int, y: int):
        ...
        if self.get_limit == 0:
            time.sleep(self.get_timeout + 0.2)
        ...
        with self.__http.get(self.base + "/get_pixel", headers = self.headers, params=data) as resp:
            ...
            try:
                ...
                raise RateLimitError(f"Rate limited by server but internal values not updated. Please wait {headers['cooldown-reset']}")
    ...
    def get_canvas(self, scale=1):
        ...
        if self.gets_limit == 0:
            time.sleep(self.gets_timeout+ 0.2)
    ...
    def set_pixel(self, x: int, y: int, color: int):
        ...
        if self.post_limit == 0:
            time.sleep(self.post_timeout + 0.2)
        with self.__http.post(self.base + "/set_pixel", headers=self.headers, json=data) as resp:
            ...
            try:
                ...
                self.set_limit = 0
                self.set_timeout = int(headers['cooldown-reset'])
                raise RateLimitError(f"Rate limited by server but internal values not updated. Please wait {headers['cooldown-reset']}")
            return
...
client = Client("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjIzNDY0OTk5MjM1NzM0NzMyOCIsInNhbHQiOiJ5ak56RUlia1Vzc3VGaHNDaUNzcl93In0.TlnN_oFjyvUMrfVlLZo0bQby0gRZ6THxrQM6L4eUisk")
client.set_pixel(0,0,0xFFFFFF)
