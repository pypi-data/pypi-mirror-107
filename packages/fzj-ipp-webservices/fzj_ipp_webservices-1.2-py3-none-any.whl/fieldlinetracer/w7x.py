...
def default_mapping(config, n_surf = 200, n_phi = 20, n_turns = 100, fivefold_periodic = True, extra_phis = [], parts = calculation_boundary, step_size = 1e-3):
    """
    
    Creates a default magnetic surface mapping based on W7-X. The mapping is traced from the mid-plane of the bean- and triangular cross-sections.
    
    Args:
        config: Magnetic configuration
        n_surf: Number of surfaces per cross-section
        n_phi: Number of (evenly spaced) toroidal planes to compute mapping in.
        n_turns: Number of turns to trace for each fieldline.
        fivefold_periodic: Whether to assume 5x periodicity when generating the mapping. Note that n_phi should be increased when this is set to False.
        extra_phis: 1D array-like of additional phi values (in radians) to include in the tracing.
        parts: List of parts to use for the Poincare tracing.
        step_size: Step size to use for tracing
    """
    
    # Determine starting points
    phi_base = np.linspace(0, 2 * np.pi, 5, endpoint = False)[None, None, :]
    ...
    r = np.linspace(
            ...
            n_surf,
            axis = 0
        )[:, :, None] # shape [n_surf, 2, 1]

    phi = np.asarray([0, np.pi])[None, :, None]
    phi = phi + phi_base
    ...
    phi_c_max = 2 / 5 * np.pi if fivefold_periodic else 2 * np.pi
    phi_c = np.linspace(0, phi_c_max, n_phi, endpoint = False)
    phi_c = np.concatenate([phi_c, extra_phis])
    ...
    return mapping.create_mapping(
        ...
        phi_c,
        n_turns,
        period = 5 if fivefold_periodic else 1,
        surf_dims = 2 if fivefold_periodic else 3,
        step_size = step_size
    )