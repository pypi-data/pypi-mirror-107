...
def magnetic_field(points, magnetic_config, client = client):
    ...
    if isinstance(magnetic_config, inputs.PredefinedField):
        x, y, z = points
        r = np.sqrt(x**2 + y**2)
        phi = np.arctan2(y, x)
        
        rphiz = np.stack([r, phi, z], axis = 0)
        
        field = magnetic_config.field
        if magnetic_config.inverse:
            field = -field
        
        fr, fp, fz = magnetic_config.grid.interpolate(
            field, rphiz,
            axes_phi_r_z = [1, 2, 3], # The field has as its first axis the direction, and only after that follow the other ones
        )
        
        fx = np.cos(phi) * fr - np.sin(phi) * fp
        fy = np.sin(phi) * fr + np.cos(phi) * fp
        result = np.stack([fx, fy, fz], axis = 0)
    else:
