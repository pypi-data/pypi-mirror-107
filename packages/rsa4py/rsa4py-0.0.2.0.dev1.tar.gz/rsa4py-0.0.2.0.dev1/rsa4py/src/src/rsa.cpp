...
unsigned long long int constexpr IntPower(const unsigned long long x, unsigned long p, unsigned long mod)
{
	if (p == 0) return 1;
	if (p == 1) return x;
...
long long RSA::PublicKey::isSigned(long long cval){
	auto v = IntPower(cval%this->n, this->e, this->n);
	return (v);
}

long long RSA::PrivateKey::getSignature(long long val){
	return (IntPower(val, this->d, this->n)%this->n)%this->n;
}

long long RSA::PrivateKey::test(long long val){
	return this->isSigned(this->getSignature(val));
}