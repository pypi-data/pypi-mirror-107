...
This is the README of *pydwf*, a Python package to control the lineup of electronic test and measurement devices made by [Digilent](https://store.digilentinc.com/).

It wraps all functions of *libdwf*, the [low-level C library](https://pydwf.readthedocs.io/en/latest/background/C_Library.html) provided by Digilent, organizing the 415 functions provided by the C library in an easy-to-use, class-based Python API. Like the C library, the pydwf package supports Windows, Linux (Intel and ARM), and macOS.

The current release of pydwf is version 0.5.19. It is intended to be used with version 3.16.3 of *libdwf*.
...
*pydwf* depends on the *numpy* package to handle the considerable amount of data moving between the PC and Digilent devices when performing high-speed signal generation or capture operations.
...
The following command will list all Digilent devices connected to the system and, for each of them, list the supported configurations:
