...
class VoronoiTessellation(object):
    ...
    def __init__(self, system, fout):
        # Internal conversion to voro++ format
        # That suggests that the v++ trajectory shouldnt add step to path
        fvpp = fout + '.inp'
        fmt = '%i, %q, %A, %v, %f, %n'
        ...
        # Execute voro++
        cmd = 'voro++ -c "%s" -r %s -l 1 -o %s %s' % (fmt, periodic, box, fvpp)
        os.system(cmd)
...
class TrajectoryVoronoi(TrajectoryXYZ):
    ...
    def __init__(self, filename, mode='r', fields=None):
        ...
        if mode == 'w':
            if fields is None:
                self.fields = ['name', 'pos', 'signature', 'neighbors']
        self._offset = 1
    ...
    def read_sample(self, frame):
        ...
        system = super(TrajectoryVoronoi, self).read_sample(frame)
        ...
        system.voronoi = []
        for ipart, p in enumerate(s.particle):
            ...
            if hasattr(p, 'neighbors'):
                ...
                db = particle.composition([p] + [s.particle[n-1] for n in vi.neighbors])
    ...
    def write_sample(self, system, step):
        ...
        for i, p in enumerate(system.particle):
            ...
            for fmt_entry in self.fields:
                if fmt_entry == 'name' or fmt_entry == 'species' or fmt_entry == 'id':
                    data += " " + p.species.strip()
                if fmt_entry == 'pos' or fmt_entry == 'x':  # we ignore y and z
...
class FilterVoronoi(object):
    ...
    def read_sample(self, sample):
        s = super(FilterVoronoi, self).read_sample(sample)
