...
class ConfigMan:
  ...
  def read_space(self, **kwargs) -> Dict:
    if cmap := self.load_master_cmap(**kwargs):
      space_id = kwargs.get(space_kwarg) or DEFAULT_SPACE_ID
      json_encoded_space = cmap.data.get(space_id) or '{}'
      try:
        return json.loads(json_encoded_space)
      except JSONDecodeError:
        pwar(self, f"read failed on corrupt space: {json_encoded_space}")
        return {}
    else:
      return {}
  ...
  def patch_into_deep_dict(self, dict_key: str, _vars: Dict, **kwargs):
    ...
    self.write_typed_entry(dict_key, new_dict)
  ...
  # def install_token(self, **kwargs) -> Optional[str]:
