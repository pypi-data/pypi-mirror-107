...
class ModelsMan:
  ...
  def add_app_descriptors(self, descriptors: List[Dict]):
    self.add_descriptors(descriptors, 'app')

  def add_descriptors(self, descriptors: List[Dict], space_label=None):
    for descriptor in descriptors:
      if isinstance(descriptor, dict):
        if space_label:
          crt_labels: Dict = descriptor.get(LABELS_KEY) or {}
          if not crt_labels.get(SPACE_LABEL):
            descriptor[LABELS_KEY] = {**crt_labels, SPACE_LABEL: space_label}
        self._descriptors.insert(0, descriptor)
      else:
        pwar(self, f"skipping non-Dict {type({descriptor})} {descriptor}")
...
def helper_prop(prop_key):
  def inner(func):
    func.prop_key = prop_key
    return func
  return inner
...
class Model:
  ...
  def get_instance_methods(self):
    ...
    for method_name, _ in inspect.getmembers(self.__class__):
      result.append(getattr(self.__class__, method_name))
    return result

  def lookup(self, prop_key):
    def finder(candidate_method):
      is_prop = hasattr(candidate_method, 'prop_key')
      return is_prop and candidate_method.prop_key == prop_key
    method = next(filter(finder, self.get_instance_methods()), None)
    return method(self)

  @helper_prop('username')
  ...
  @cached_property
  def space_label(self):
    return self.labels.get(SPACE_LABEL)
  ...
  def config_space(self) -> str:
    ...
    if explicit := self.get_prop(CONFIG_SPACE_KEY):
      ...
      value = self.labels.get(SPACE_LABEL)
    return value or 'app'
  ...
  @classmethod
  def inflate_single(cls: T, **kwargs) -> Optional[T]:
  ...
  def inflate_all(cls, **kwargs) -> List[T]:
    selector = kwargs.pop(selector_kwarg, {})
    cls_pool = cls.lteq_classes(models_man.classes())
    config_pool = models_man.descriptors(selector)
  ...
  def inflate_with_id_reference(cls, id_ref: str, **kwargs) -> T:
    ...
    selector = kwargs.pop(selector_kwarg, None)
    candidate_configs = models_man.descriptors(selector)
  ...
  @classmethod
  def from_provider(cls, provider_kod: KoD = None) -> List[T]:
    from kama_sdk.model.supplier.base.supplier import Supplier
