...
from mediaio.exceptions import exceptions
from mediaio.config import __version__, __endpoint_map__

log = logging.getLogger(__name__)


class mediaiorestclient(object):
    ...
    user_agent = "MediaIOSDK/MediaIO-Python-SDK %s (%s)" % (
        __version__, library_details)

    def __init__(self, options=None, **kwargs):
        """Create Client object
        Usage::
            >>> import mediaio.mediaiorestclient as mediaiorestclient
            >>> rest_client = mediaiorestclient.mediaiorestclient(token='access_token', ssl_options={"cert": "/path/to/server.pem"})
        """
    ...
    def request(self, url, method, body=None, headers=None):
        """Make HTTP call, formats response and does error handling. Uses http_call method in mediaiorestclient class.
        Usage::
            >>> mediaiorestclient.request("https://api.sandbox.media.io/v2/jobs/JOB-ID", "GET", {})
            >>> mediaiorestclient.request("https://api.sandbox.media.io/v2/tasks/TASK-ID", "POST", "{}", {} )
        """
...
def download(url, filename):
    ...
    Usage::
        >>> mediaio.download(url="https://exported_url", filename="sample.pdf")
    """
    try:
...
def default_client():
    ...
    """
    from mediaio.environment_vars import MEDIAIO_API_KEY
    global __client__
    if __client__ is None:
        try:
            API_KEY = os.environ[MEDIAIO_API_KEY]
        except KeyError:
            raise exceptions.MissingConfig(
                "Required MEDIAIO_API_KEY \n Refer https://media.io/api/v2#overview")

        # Get default API mode
        sandbox = True if os.environ.get("MEDIAIO_SANDBOX", "false") == 'true' else False

        __client__ = mediaiorestclient({}, sandbox=sandbox, api_key=API_KEY)
...
def set_config(options=None, **config):
    ...
    """
    global __client__
    __client__ = mediaiorestclient(options or {}, **config)
    return __client__
