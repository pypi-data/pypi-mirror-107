...
from rocshelf.template.basic import (CommentNode, Node, NodesList,
...
class HtmlTagNodeArea(CloseNodeArea):
    ...
    def arealize(cls, litValues: list[_T.Union[LiteralValue, Node]], thisIndex: int) -> _T.Optional[range]:
        ...
        newNode = HtmlTagNode.create(
            callLitValue,
            juxtaposition_core(litValues[startIndex:stopIndex])
        )
        return (
...
class BaseHtmlTag(Node):
    ...
    def deconstruct(self, litValue: LiteralValue) -> None:
