...
class InitNode(object):
    ...
    proccParams: template.main.ProcessingParams
...
class ApiNode(InitNode):
    ...
    def _processing(self, proccParams: template.main.ProcessingParams) -> ProcessingOutputNode:
        """ Обработка ноды """

        return ProcessingOutputNode.from_node(self, proccParams)

    def _compile(self, proccParams: template.main.ProcessingParams) -> str:
...
class Node(ApiNode):
    ...
    def processing(self, proccParams: _T.Optional[template.main.ProcessingParams] = None) -> ProcessingOutputNode:
        """ Первая стадия компиляции нод - предварительная обработка нод:

        Удаление ненужных нод, преобразование статичных нод и т.д.

        Args:
            proccParams (_T.Optional[template.main.ProcessingParams], optional): Параметры компиляции. Defaults to None.
        ...
        if not isinstance(proccParams, template.main.ProcessingParams):
            proccParams = template.main.ProcessingParams(proccParams)
    ...
    def compile(self, proccParams: _T.Union[template.main.ProcessingParams, dict, None] = None) -> str:
...
class TextNode(Node):
    ...
    def _compile(self, proccParams: template.main.ProcessingParams):
...
class ProcessingOutputNode(Node):
    ...
    def start_sub_nodes_processing(self, subNodes: template.main.NodesList, proccParams: template.main.ProcessingParams):
        """ Запуск функции обработки у ноды и добавление результата в себя

        Args:
            subNodes (template.main.NodesList): Список нод
            proccParams (template.main.ProcessingParams): Параметры обработки
    ...
    @classmethod
    def from_node(cls, node: Node, proccParams: template.main.ProcessingParams, collectedData: _T.Optional[dict] = None):
        ...
        Args:
            ...
            proccParams (template.main.ProcessingParams): Парастеры компиляции.
    ...
    @classmethod
    def node(cls, node: Node, proccParams: template.main.ProcessingParams, collectedData: _T.Optional[dict] = None) -> ProcessingOutputNode:
