...
def _entrypoint(outCli: CLIOutput):
    find_true = [outCli.req[key] for key in outCli.req if key != 'help']
    if True in find_true:
        main.print_rocshelf_info(**outCli.req)
    else:
        outCli.next()


def _help(outCli: CLIOutput):
    find_true = [outCli.req[key] for key in outCli.req if key != 'help']
    if True in find_true:
        main.print_rocshelf_help_info(**outCli.req)
    else:
        outCli.next()


def _compile(outCli: CLIOutput):
    UICompile(outCli.req['framework']).compile()


def _shelf(outCli: CLIOutput):

    next_command = str(outCli.next_command)

    if next_command not in ['sync', 'create', 'delete', 'get'] or outCli.next_command is None:
        outCli.help()
        return

    if next_command != 'sync':
        sh = outCli.next_command.req['shelf']
        name = outCli.next_command.req['name']

        ui = UIShelves(sh, name)
        ui.output = 'print'

        if next_command == 'create':
            if 'layout' in outCli.next_command.req:
                ui.create(outCli.next_command.req['layout'])
        ...
        layout = False
        if 'layout' in outCli.next_command.req:
            layout = outCli.next_command.req['layout']
...
def _route_create(outCli: CLIOutput):
    name = outCli.req['name']
    page = False if 'page' not in outCli.req else outCli.req['page']
    layout = False if 'layout' not in outCli.req else outCli.req['layout']
...
def _route_delete(outCli: CLIOutput):
    name = outCli.req['name']
...
def _route_sync(outCli: CLIOutput):
    ui = UIRoute('name')
    ui.sync(
        False if 'layout' not in outCli.req else outCli.req['layout']
    )
    _print_counter(ui.counter)
