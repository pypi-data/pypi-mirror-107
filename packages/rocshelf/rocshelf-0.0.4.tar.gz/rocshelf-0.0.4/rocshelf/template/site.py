...
from rocshelf.template.basic import (DevNode, Node, ProcessingOutputNode,
                                     ProcessingParams, registration)
...
class DownloadStaticPlaceProcessing(object):
    ...
    def __url_to_static(self, staticType: str, loadTime: str, shelfSlugs: str, route: str) -> _T.Optional[str]:
        ...
        staticFileName = relation.static_filename(staticType, loadTime, shelfSlugs)

        staticFilePath = relation.static_path(staticFileName)
        if not staticFilePath.check():
            return None

        return relation.url_to_static(route, staticFileName)
    ...
    def _compile_style_final(self, route: str, loadTime: str, chunks: list[Chunk]) -> list[str]:
        ...
        downloadScript = cf.path('layout', 'html_links_final_css').read()
        conpileVars = {
...
class DownloadStaticPlaceNode(Node, DownloadStaticPlaceProcessing):
    ...
    def _compile(self, proccParams: ProcessingParams) -> str:
        ...
        route = proccParams.localVars['__meta__'].route
        chunks = proccParams.localVars['__meta__'].chunks
        return self._compile_links(loadTime, route, chunks)


class LinkBaseNode(Node):
...
class LinkPageNode(LinkBaseNode):
    ...
    def _compile(self, proccParams: ProcessingParams) -> str:
        activeRoute = proccParams.localVars['__meta__'].route
        targetRoute = self.callParameter

        return relation.url_to_page(activeRoute, targetRoute)


class LinkMediaNode(LinkBaseNode):
    ...
    def _compile(self, proccParams: ProcessingParams) -> str:
        localizationName = proccParams.localVars['__meta__'].localization
        activeRoute = proccParams.localVars['__meta__'].route

        targetMediaFile = relation.media_filename(self.callParameter)

        MediaFile(targetMediaFile).move_to_dist()

        return relation.url_to_media(activeRoute, targetMediaFile)


class LinkStaticNode(LinkBaseNode):
    """ Нода ссылки на медиа файл.
    ...
    def _compile(self, proccParams: ProcessingParams) -> str:
        ...
        targetMediaFile = relation.static_filename_safe(self.callParameter)

        StaticFile(targetMediaFile).move_to_dist()

        return relation.url_to_static(activeRoute, targetMediaFile)


class LocalizationNode(LinkBaseNode):
    """ Нода локализации """
    ...
    def deconstruct(self, localizationName: _T.Optional[str]) -> None:
    ...
    def _compile(self, proccParams: ProcessingParams) -> str:
        if self.localizationName is None:
            self.localizationName = proccParams.localVars['__meta__'].localization
