...
class ShelfItem(object):
    ...
    def __init__(self, userShelfConfig: dict):
        ...
        if self.type not in SHELFTYPES:
            raise ex.ex.DeveloperIsShitError(self.type + ' - несуществующий тип шелфа')
...
class InitComponent(InitComponentCore):
    ...
    def parse(self):
        ...
        processedImport = self.transformation_shelves_config(cf.get(['path', 'import']))
        self.shelvesPaths.update(processedImport)

        # Инициализация добавленных пользователем групп шелфов
        groupsPath = cf.path('import', 'groups')
        if groupsPath.check():
    ...
    def check(self):
        ...
        for shelfSlug in self.shelvesPaths:
            ...
            logger.debug(f'Проверка шелфа "{shelfSlug}" по пути "{shelfPath}"')
    ...
    def recover_id(self):
        ...
        pastShelvesIdsFile = rPath(saveShelvesIdsFileName, fromPath='project.cache')
        if pastShelvesIdsFile.check():
            self.shelvesIds = pastShelvesIdsFile.parse()

            for shelfId, shelfSlug in self.shelvesIds.items():
                shelfType, shelfName = ShelfItem.unSlug(shelfSlug)
    ...
    def generate_id(self, shelfType: str, shelfName: str, shelf: ShelfItem) -> str:
        ...
        Raises:
            ex.ex.DeveloperIsShitError: Если произошло дублирование идентификаторов
        ...
        if newId in self.shelvesIds and self.shelvesIds.get(newId) != str(shelf):
            raise ex.ex.DeveloperIsShitError('Ну и коринджовая ситуация получилась: {0} {1}'.format(
                newId, shelf
            ))
    ...
    def save_cache(self):
        ...
        rPath(saveShelvesPathsFileName, fromPath='project.cache').dump(self.shelvesPaths)
        rPath(saveShelvesIdsFileName, fromPath='project.cache').dump(self.shelvesIds)
    ...
    def all_stages(cls):
        ...
        logger.info('Результат:  Пути: "{0}"; Идентификаторы: "{1}"'.format(
            rPath(saveShelvesPathsFileName, fromPath="project.cache"),
            rPath(saveShelvesIdsFileName, fromPath="project.cache")
        ))
