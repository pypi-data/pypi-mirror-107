...
class GetRoute(object):
    ...
    @staticmethod
    def walk() -> _T.Generator[tuple[str, RouteItem]]:
...
class InitComponent(InitComponentCore):
    ...
    def parse(self):
        ...
        processedRoutes = _transformation_routes_config(cf.get(['route']))
    ...
    def check(self):
        ...
        for routeKey, route in routes.items():
            ...
            if route.page not in shelves.GetShelf.types('page'):
                raise ValueError('Шелф "{0}" не инициализирован'.format(
    ...
    def save_cache(self):
        ...
        rPath(saveRoutesFileName, fromPath='project.cache').dump(self.routesDict)
    ...
    def all_stages(cls):
        ...
        logger.info('Результат: "{0}"'.format(
            rPath(saveRoutesFileName, fromPath='project.cache')
        ))
