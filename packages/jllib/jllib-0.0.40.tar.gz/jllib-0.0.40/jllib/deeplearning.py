...
class AutoAuthenticator(AutoEvaluator):
    ...
    def get_mae(self, a: np.ndarray, b: np.ndarray):
    ...
    def fit(self) -> None:
        ...
        for i in range(self.nb_classes):
            ...
            for pred, x in zip(preds, x_train):
                mae_list.append(self.get_mae(pred, x))
    ...
    def determine_distances_to_cluster_border(self, sample: np.ndarray):
        possible_matches = []
        for i in range(self.nb_classes):
            ...
            pred_mae = self.get_mae(pred, sample)
    ...
    def check_sample_against_all_autoencoders(self, sample: np.ndarray, t: int = 0, impl: int = 1):
    ...
    def _do_eval(self, t, x_val, y_val):
        ...
        for x_val_sample, label_ohc in zip(x_val, y_val):
            ...
            accept, predicted_class_nb, info = self.check_sample_against_all_autoencoders(x_val_sample, t=t)
            if accept:
    ...
    def evaluate(self, x_val=None, y_val=None):
        ...
        possible_thresholds = Parallel(n_jobs=os.cpu_count(), backend='threading')(delayed(self.determine_distances_to_cluster_border)(x_val_sample) for x_val_sample in x_val)
        ...
        results_lst = Parallel(n_jobs=os.cpu_count(), backend='threading')(delayed(self._do_eval)(t, x_val, y_val) for t in possible_thresholds)

        return results_lst

    def accept_sample(self, sample, threshold) -> bool:
        assert self.MODELS_HAVE_TRAINED, 'Models must be trained before accessing accept_sample()'

        acceptance, best_ae_class, info = self.check_sample_against_all_autoencoders(sample, t=threshold)
        return acceptance
