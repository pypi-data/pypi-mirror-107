...
async def sync_all_commands(client : typing.Union[discord.ext.commands.Bot, discord.ext.commands.AutoShardedBot, discord.Client, discord.AutoShardedClient], case_sensitive=True, loading_message="Loading", send_hidden=False, hidden_commands=[], choices={}, error_function=None, button_functions=[]):
    ...
    for x in commands:
        w = await post(f"https://discord.com/api/v9/applications/{client.user.id}/commands", json_dict=x, headers=headers)
        try:
            if w["name"] == x["name"]:
    ...
    slash_commands = await get(f"https://discord.com/api/v9/applications/{client.user.id}/commands", headers=headers)
    name_list = []
    for x in commands:
        name_list.append(x["name"])
    for x in slash_commands:
        if x["name"] not in name_list or x["name"] in hidden_commands:
            t = await delete(f"https://discord.com/api/v9/applications/{client.user.id}/commands/{x['id']}", headers=headers)
            if t == "":
    ...
    async def on_socket_response(msg):
        if msg["t"] == "INTERACTION_CREATE":  
            ...
            ack = await post(f"https://discord.com/api/v9/interactions/{msg['d']['id']}/{msg['d']['token']}/callback", headers=headers, json_dict={"type" : 5, "data" : {"content" : loading_message, "flags" : flags}})
            class slash_context:
                ...
                async def send(self, message : str=None, *, embed : discord.Embed=None, buttons=[]):
                        ...
                        response = await patch(f"https://discord.com/api/v9/webhooks/{client.user.id}/{msg['d']['token']}/messages/@original", headers=headers, json_dict=dictionary)
                        try:
                            if response["message"] == "You are being rate limited.":
                                await asyncio.sleep(int(response["retry_after"]))
                                await patch(f"https://discord.com/api/v9/webhooks/{client.user.id}/{msg['d']['token']}/messages/@original", headers=headers, json_dict=dictionary)
                        except KeyError:
                            pass
            ...
            class slash_message:
                ...
                async def edit(self, content):
                    await patch(f"https://discord.com/api/v9/webhooks/{client.user.id}/{self.token}/messages/@original", headers=headers, json_dict={"content" : content})
            
                async def delete(self):
                    await delete(f"https://discord.com/api/v9/webhooks/{client.user.id}/{self.token}/messages/@original", headers=headers)
            ...
            for x in client.commands:
                try:
                    ...
                    for functions in button_functions:
                        if msg["d"]["data"]["custom_id"] == f"function_{functions.__name__}":
                            await functions(slash_context(msg))
                    return
                if x.name == msg["d"]["data"]["name"]:
                    ...
                    try:
                        ...
                        if error_function != None:
                            await error_function(slash_context(msg), error)
                        if error_function == None:
...
class Button:
    def __init__(self, when_button_is_clicked=None, type=2, style=1, label=None, emoji=None, url=None, disabled=False):
        self.when_button_is_clicked = when_button_is_clicked
        self.type = type
        self.style = style
        self.label = label
        self.emoji = emoji
        self.custom_id = self.when_button_is_clicked
        self.url = url
        self.disabled = disabled
        if self.url != None:
            if self.when_button_is_clicked != None:
                raise ValueError("URL buttons cannot have a click function")
    def dictionary(self):
        return {
            "type" : self.type,
            "style" : self.style,
