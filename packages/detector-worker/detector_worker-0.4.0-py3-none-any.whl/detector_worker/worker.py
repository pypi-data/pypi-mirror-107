    ...
    def _read_blob_by_chunks(self, input_blob):
        ...
        for chunk in blob_data.chunks():
            ...
            for l in lines[:-1]:
                ...
                if len(l) == 0:
                    continue
                source = self._preprocess(json.loads(l)[0])
                target = self._preprocess(json.loads(l)[1])
                if self.return_only_sources:
        ...
        if len(last_piece) > 0:
            ...
            try:
                ...
                target = self._preprocess(json.loads(last_piece)[1])
            except:
                pass
            if source:
                yield [source, target]
