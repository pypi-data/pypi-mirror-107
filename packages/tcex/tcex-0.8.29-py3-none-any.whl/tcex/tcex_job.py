...
class TcExJob(object):
    ...
    def _group_add(self, resource_type, resource_name, owner, data=None):
        ...
        self._tcex.log.debug('Adding {} "{}" in owner {}'.format(
        ...
        for param, value in list(data.items()):
            ...
            self._tcex.log.debug('Adding parameter {}:{}'.format(
                param, value))
        ...
        if results.get('status') == 'Success':
            ...
            for attribute in data.get('attribute', []):
                self._tcex.log.debug('Adding attribute type "{}" to Group ID {}'.format(
    ...
    def _group_delete_by_name(self, owner, resource_type, resource_name):
        ...
        for results in resource:
            if results.get('status') != 'Success':
                self._tcex.log.warning('Retrieve {} with name {} failed ({}).'.format(
    ...
    def _process_association(self, owner):
        ...
        for a in self._associations:
            self._tcex.log.info('creating association: resource {} association {}'.format(
    ...
    def _process_group_association(self, owner):
        ...
        for ga in self._group_associations:
            self._tcex.log.info('creating association: group "{}" indicator {}'.format(
    ...
    def _process_groups(self, owner, group_action):
        ...
        for group in self._groups:
            ...
            self._tcex.log.debug('Processing group ({})'.format(group_name))
            if group_action == 'duplicate':
                ...
                if group_id is None and self._batch_halt_on_error:
                    ...
                    break
                self._tcex.log.info('Created Group "{}" with id: {}'.format(group_name, group_id))
            elif group_action == 'replace':
                if group_id is not None:
                    # delete the group from TC and group cache
                    self._group_delete_by_name(owner, group_type, group_name)
                    self._tcex.log.info('Deleting Group "{}" with name: "{}"'.format(
                ...
                if group_id is None:
                    ...
                    if group_id is None and self._batch_halt_on_error:
                        ...
                        self.group_cache_add(group_name, owner, group_type, group_id)
                    self._tcex.log.info('Created Group "{}" with id: {}'.format(
                        group_name, group_id))
                else:
                    # update group results tracking
                    self._group_results['cached'].append(group_name)
                    self._group_results['not_saved'].remove(group_name)
                    self._tcex.log.info('Skipping existing Group "{}")'.format(group_name))
            else:
                self._tcex.log.info('Invalid group action ({})'.format(group_action))
        ...
        for chunk in self._chunk_indicators():
            ...
            if results['status'] == 'Success':
                ...
                if data_results['status'] == 'Success':
                    ...
                    poll_time = 0
                    while True:
                        self._tcex.log.debug('poll_time: {}'.format(poll_time))
    ...
    def batch_status(self, batch_id):
        ...
        if results.get('status') == 'Success':
            if results['data']['status'] == 'Completed':
                ...
                status_msg += 'succeeded: {}, '.format(results.get('data').get('successCount'))
                status_msg += 'failed: {}, '.format(results.get('data').get('errorCount'))
                status_msg += 'unprocessed: {}'.format(results.get('data').get('unprocessCount'))
                ...
                if results['data']['errorCount'] > 0:
                    ...
                    while True:
                        ...
                        if poll_time >= self._batch_poll_interval_max:
                            ...
                            break
                        self._tcex.log.info('Error retrieve sleep ({} seconds)'.format(
                    ...
                    for error in errors:
                        ...
                        for error_msg in self._batch_failures:
                            if re.findall(error_msg, error_reason):
                                # Critical Error
                                err = 'Batch Error: {}'.format(error)
                                self._tcex.message_tc(err)
                                self._tcex.log.error(err)
                                self._tcex.exit(1)
                        self._tcex.log.warn('Batch Error: {}'.format(error))
            else:
    ...
    def group_cache(self, owner, resource_type):
        ...
        **Cache Structure**
        ...
        self._tcex.log.info('Caching group type {} for owner {}'.format(resource_type, owner))
    ...
    def group_cache_remove(self, name, owner, resource_type):
        ...
        self._tcex.log.debug(
            'Removing {} ID for group "{}" in "{}".'.format(
                resource_type, name, owner))
    ...
    def group_cache_type(self, group_id, owner):
        ...
        **Cache Structure**
        ...
        if self._group_cache_id.get(owner) is None:
            ...
            self._tcex.log.info('Caching groups for owner {}'.format(owner))
    ...
    def process(self, owner, indicator_batch=True, group_action='skip'):
        ...
        if self._associations:
            self._tcex.log.info('Processing Associations')
