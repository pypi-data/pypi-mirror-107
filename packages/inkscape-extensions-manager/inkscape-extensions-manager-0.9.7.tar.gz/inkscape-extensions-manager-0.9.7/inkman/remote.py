...
class RemoteArchive(object):
    ...
    def _search(self, query, tags=[]):
        ...
        if query is None:
            response = self.session.get(self.url, params={'tags': tags, 'checked': 1, 'limit': 10, 'order': 'extra_status'})
        else:
            response = self.session.get(self.url, params={'q': query, 'tags': tags, 'checked': 1})
