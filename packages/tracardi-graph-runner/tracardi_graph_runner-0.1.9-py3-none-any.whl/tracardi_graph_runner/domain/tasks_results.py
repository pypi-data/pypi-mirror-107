...
class TasksResults:
    ...
    def add(self, edge_id: str, result: TaskResult):
    ...
    def get(self, edge_id, port) -> List[TaskResult]:
        if edge_id not in self._results:
            raise DagError(
        ...
        if port not in self._results[edge_id]:
            raise DagError("Edge {} did not return value in port {}.".format(edge_id, port), port=port)
