...
class RNN(Craft):
    def __init__(self, in_chs, hidden_dim, nlayers=1, scope='RNN'):
        super(RNN, self).__init__(scope)
        self.rnn = nn.RNN(in_chs, hidden_dim, nlayers, batch_first=True)
...
class BiRNN(Craft):
    def __init__(self, in_chs, hidden_dim, nlayers=1, scope='BIRNN'):
        super(BiRNN, self).__init__(scope)
        self.birnn = nn.RNN(in_chs, hidden_dim, nlayers, batch_first=True, bidirectional=True)
...
class LSTM(Craft):
    def __init__(self, in_chs, hidden_dim, nlayers=1, scope='LSTM'):
        super(LSTM, self).__init__(scope)
        self.lstm = nn.LSTM(in_chs, hidden_dim, nlayers, batch_first=True)
...
class BiLSTM(Craft):
    def __init__(self, in_chs, hidden_dim, nlayers=1, scope='BILSTM'):
        super(BiLSTM, self).__init__(scope)
        self.bilstm = nn.LSTM(in_chs, hidden_dim, nlayers, batch_first=True, bidirectional=True)
...
class MaxEigen(Craft):
    ...
    def run(self, m, in_pair=True):
        assert (m==m.T).all(), 'NEBULAE ERROR ⨷ only when m is real symmetric matrix can MaxEigen be applied.'
