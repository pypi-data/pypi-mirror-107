...
class TestSDK(unittest.TestCase):
    ...
    def test_qlattice_can_get_classifier(self):
        lt = QLattice()
        lt.reset()
        qgraph = lt.get_classifier(["age", "smoker", "insurable"], "insurable", max_depth=1)
    ...
    def test_qlattice_can_get_regressor(self):
        lt = QLattice()
        lt.reset()
        qgraph = lt.get_regressor(["age", "smoker", "insurable"], "lifexpectancy", max_depth=1)
    ...
    def test_fit_qgraph(self):
        lt = QLattice()
        lt.reset()
        qgraph = lt.get_classifier(["age", "smoker", "insurable"], "insurable", max_depth=2)
    ...
    def test_update_qgraph_with_older_graph(self):
        lt = QLattice()
        lt.reset()
        qg1 = lt.get_classifier(["age", "smoker", "insurable"], "insurable", max_depth=1)
        graph = qg1[0]

        lt.get_classifier(["age", "smoker", "insurable"], "insurable", max_depth=1)
        lt.update(graph)
    ...
    def test_can_get_qgraph_with_any_column_as_output(self):
        ...
        for target in columns:
            qgraph = lt.get_regressor(columns, target)
            self.assertGreater(len(qgraph), 0)
    ...
    def test_retries_failed_updates(self):
        ...
        with httpretty.enabled():
            ...
            lt._http_client.get_adapter("http://").max_retries.backoff_factor = 0.1  # Instant retries
