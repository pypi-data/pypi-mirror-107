...
class ApiFaturamento(ApiClientBase):
    ...
    def simular_proximo_ciclo(self, conta_id, plano_id, **kwargs):
        ...
        return self.to_dict(path, method="get", meta=True, months=kwargs.get('months', '1'), discount=kwargs.get('discount', '0'))

    def assinar(self, conta_id, plano_id, method="post", **kwargs):
        path = '/contas/{}/planos/{}/assinar/'.format(conta_id, plano_id)
        return self.to_dict(path, method=method, meta=True, months=kwargs.get('months', '1'), discount=kwargs.get('discount', '0'))
