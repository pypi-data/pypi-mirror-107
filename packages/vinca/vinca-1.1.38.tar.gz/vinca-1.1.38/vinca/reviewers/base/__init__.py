...
def review(card,mode):
	...
	def show_card():
		...
		# audio
		audio_front_path = card.path / 'front_audio.mp3'
		audio_front = Recording(audio_front_path) if audio_front_path.exists() else None
		if audio_front: audio_front.play()

		# images
		image_front_path = card.path / 'front_image.png'
		image_front = Recording(image_front_path) if image_front_path.exists() else None
		...
		# audio
		audio_back_path = card.path / 'back_audio.mp3'
		audio_back = Recording(audio_back_path) if audio_back_path.exists() else None
		if audio_back: audio_back.play()

		# images
		image_back_path = card.path / 'back_image.png'
		image_back = Recording(image_back_path) if image_back_path.exists() else None
