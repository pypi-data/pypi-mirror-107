    ...
    def get(
        ...
        if inline_range is None or xline_range is None:
            ...
            if inline_range is None:
                inline_range = (line_range.inline.min, line_range.inline.max)
            if xline_range is None:
                xline_range = (line_range.xline.min, line_range.xline.max)
        ...
        if z is not None:
            ...
            if len(horizon) == 0:
                raise ValueError("empty horizon given")
            horizon_inline_dim = len(horizon)
            horizon_xline_dim = len(horizon[0])
            if (inline_dim, xline_dim) != (horizon_inline_dim, horizon_xline_dim):
                raise ValueError(
                    "mismatch between shape of inline/xline ranges and given horizon. ranges shape: (%d,%d) horizon shape: (%d,%d)"
                    % (inline_dim, xline_dim, horizon_inline_dim, horizon_xline_dim)
                )
