...
class edgar_index:
    ...
    headers = {"User-Agent": "farr.ai daas demo"}
    ...
    def _get_full_index_url(self, date: datetime.datetime) -> str:
        ...
        return "{full_index}/{year}/QTR{quarter}/index.json".format(
            full_index=self.full_index, year=date.year, quarter=quarter
        )

    def _get_daily_index_url(self, date: datetime.datetime) -> str:
        quarter = self._get_quarter(date.month)
        return "{daily_index}/{year}/QTR{quarter}/index.json".format(
            daily_index=self.daily_index, year=date.year, quarter=quarter
        )
    ...
    def _get_full_index(self, date: datetime.datetime) -> Dict[str, Any]:
        ...
        if key in self._known_full_index:
            ...
            index = json.loads(httpx.get(self._get_full_index_url(date), headers=self.headers).text)
    ...
    def _get_daily_index(self, date: datetime.datetime) -> Dict[str, Any]:
        ...
        if key in self._known_daily_index:
            ...
            index = json.loads(httpx.get(self._get_daily_index_url(date), headers=self.headers).text)
    ...
    def _get_full_listing_url(self, date: datetime.datetime) -> Tuple[str, bool]:
        ...
        for item in index_dic["directory"]["item"]:
            if item["name"] == file_name:
                return (
                    "{edgar_url}/{parent}{file_name}".format(
                        edgar_url=self.edgar_url,
                        parent=index_dic["directory"]["name"],
                        file_name=file_name,
                    ),
                    True,
                )
    ...
    def get_full_listing(self, date: datetime.datetime) -> str:
        ...
        if status is False:
            return ""
        return httpx.get(listing_url, headers=self.headers).text
    ...
    def _format_listing_as_pd(self, listing: str, date: datetime.datetime) -> pd.DataFrame:
        ...
        df = pd.read_csv(
            ...
            index_col=False,
            skiprows=skip,
            names=["CIK", "Company Name", "Form Type", "Date Filed", "File Name"],
        )
    ...
    def get_filing(self, url: str):
        return httpx.get(
            "{archives_url}/{url}".format(archives_url=self.archives_url, url=url),
