...
def _parse_authorization_header(authorization_header: str) -> str:
    ...
    if len_auth == 1:
        raise Exception(f'Invalid token header "{authorization_header}". No credentials provided.')

    if len_auth > 2:
        raise Exception(f'Invalid token header "{authorization_header}". Token string should not contain spaces.')
