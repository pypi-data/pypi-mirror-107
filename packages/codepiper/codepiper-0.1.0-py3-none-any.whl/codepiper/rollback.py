...
def rerun_build(
    ...
    if "s3location" in input_artifact:
        build_kwargs["sourceTypeOverride"] = "S3"
        build_kwargs[
            "sourceLocationOverride"
        ] = f"{input_artifact['s3location']['bucket']}/{input_artifact['s3location']['key']}"
    ...
    **kwargs,
    ...
    for action in details:
        ...
        if action["input"]["actionTypeId"]["provider"] == "CodeBuild":
            primary_source_name = action["input"]["resolvedConfiguration"][
                "PrimarySource"
            ]
