    ...
    def get_sender_key(self, data, user):
        ...
        if isInvitation:  # An invited user is authorized
            ...
            if group in user.invited_groups.all():
                authorized = True
        elif stickId.startswith(user.parties.filter(self=False).id) or stickId.startswith(user.parties.filter(self=True).id):  # A user is authorized to fetch SenderKeys of their own profile (user.party.id)
            authorized = True
        else:
            groupId = stickId[:36]
