...
class StickProtocol():
    ...
    def get_sender_key(self, data, user):
        ...
        if isInvitation:  # An invited user is authorized
            ...
            if group in user.invited_groups.all():
                authorized = True
        elif stickId.startswith(user.parties.filter(individual=False).id) or stickId.startswith(user.parties.filter(individual=True).id):  # A user is authorized to fetch SenderKeys of their own profile (user.party.id)
            authorized = True
        else:
            groupId = stickId[:36]
    ...
    def get_stick_id(self, data, user):
        ...
        if not isProfile and 'partyId' not in data:
            ...
            partyId = Party.objects.get(user=user, self=isSelf).id
        dict = {}
        bundlesToFetch = []
        responseDict = {}
