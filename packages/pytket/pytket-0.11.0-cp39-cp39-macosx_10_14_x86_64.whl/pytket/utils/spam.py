...
class SpamCorrecter:
    ...
    def __init__(
        ...
        :param qubit_subsets: A list of lists of correlated Nodes of a `Device`. Qubits
            within the same list are assumed to only have SPAM errors correlated with
            each other. Thus to allow SPAM errors between all qubits you should provide
            a single list.
        ...
        :param backend: Backend on which the experiments are intended to be run
            ...
            experiment will be run on a `Device`with the nodes in `qubit_subsets`,
            and furthermore that the intended device natively supports X gates.
