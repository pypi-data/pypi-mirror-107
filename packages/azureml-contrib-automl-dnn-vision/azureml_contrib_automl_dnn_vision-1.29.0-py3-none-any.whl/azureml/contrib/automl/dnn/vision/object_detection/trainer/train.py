...
from azureml.contrib.automl.dnn.vision.common.constants import SettingsLiterals as CommonSettingsLiterals, \
...
def train_one_epoch(model, optimizer, scheduler, train_data_loader,
                    device, criterion, epoch, print_freq, system_meter, distributed):
...
def train(model, optimizer, scheduler, train_data_loader, val_data_loader,
    ...
    for epoch in range(number_of_epochs):
        ...
        if epoch % eval_freq == 0 or final_epoch:
            ...
            logger.info("Current best primary metric score: {0:.3f}".format(round(best_score, 3)))
