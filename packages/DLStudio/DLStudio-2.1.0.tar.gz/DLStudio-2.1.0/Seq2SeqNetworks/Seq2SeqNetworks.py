...
__date__      = '2021-May-28'   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.1.0.html'
...
SEQUENCE-TO-SEQUENCE LEARNING WITH ATTENTION: 
    ...
    at as you are reading this doc page.  This class, Seq2SeqNetworks,
    resides at the top-level of the distro.

    In Version 2.1.0, Seq2SeqNetworks contains the following two inner
    ...
    The following two scripts in the ExamplesSeq2SeqNetworks directory of the
...
#___________________________________  Seq2SeqNetworks Class Definition  ___________________________________


class Seq2SeqNetworks(object):

    def __init__(self, *args, **kwargs ):
        if args:
            raise ValueError(  
                   '''Seq2SeqNetworks constructor can only be called with keyword arguments for 
    ...
    class Seq2SeqWithLearnableEmbeddings(nn.Module):             
        ...
        class path:  Seq2SeqNetworks  ->  Seq2SeqWithLearnableEmbeddings
        """
        def __init__(self, dl_studio, dataroot, data_archive, max_length, embedding_size, num_trials):
            super(Seq2SeqNetworks.Seq2SeqWithLearnableEmbeddings, self).__init__()
        ...
        class EncoderRNN(nn.Module):
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithLearnableEmbeddings  ->  EncoderRNN
            """
            def __init__(self, dls, s2s, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithLearnableEmbeddings.EncoderRNN, self).__init__()
                self.dl_studio = dls
        ...
        class DecoderRNN(nn.Module):
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithLearnableEmbeddings  ->  DecoderRNN
            """
            def __init__(self, dls, s2s, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithLearnableEmbeddings.DecoderRNN, self).__init__()
                self.hidden_size = hidden_size
                self.target_vocab_size = s2s.vocab_es_size
                self.max_length = max_length
                self.embed = nn.Embedding(self.target_vocab_size, embedding_size)
                self.attn_calc = Seq2SeqNetworks.Seq2SeqWithLearnableEmbeddings.Attention_BCB(
                                                              dls,embedding_size,hidden_size,max_length)         ## (C)
#                self.attn_calc = Seq2SeqNetworks.Seq2SeqWithLearnableEmbeddings.Attention_SR(
#                                                               dls,embedding_size, hidden_size, max_length)     ## (D)
        ...
        class Attention_BCB(nn.Module):
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithLearnableEmbeddings  ->  Attention_BCB
            """
            def __init__(self, dl_studio, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithLearnableEmbeddings.Attention_BCB, self).__init__()
                self.dl_studio = dl_studio
                self.max_length = max_length
        ...
        class Attention_SR(nn.Module):
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithLearnableEmbeddings  ->  Attention_SR
            """
            def __init__(self, dl_studio, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithLearnableEmbeddings.Attention_SR, self).__init__()
    ...
    class Seq2SeqWithPretrainedEmbeddings(nn.Module):             
        """
        Please read the doc section of the previous Seq2SeqNetworks class, 
        ...
        this section of Seq2SeqNetworks we use the pre-trained word2vec embeddings from 
        ...
        ClassPath:  Seq2SeqNetworks -> Seq2SeqWithPretrainedEmbeddings
        """
        def __init__(self, dl_studio, dataroot, data_archive, path_to_saved_embeddings_en, 
            ...
            super(Seq2SeqNetworks.Seq2SeqWithPretrainedEmbeddings, self).__init__()
            self.dl_studio = dl_studio
            self.dataroot  = dataroot
            self.data_archive = data_archive
        ...
        def sentence_to_tensor(self, sentence, lang):
            ...
            part of Seq2SeqNetworks.  You are currently in the Seq2SeqWithPretrainedEmbeddings part.
        ...
        class EncoderRNN(nn.Module):
            ...
            part of Seq2SeqNetworks.  You are currently in the Seq2SeqWithPretrainedEmbeddings 
            part.
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithPretrainedEmbeddings ->  EncoderRNN
            """
            def __init__(self, dls, s2s, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithPretrainedEmbeddings.EncoderRNN, self).__init__()
                self.dl_studio = dls
                self.source_vocab_size = s2s.vocab_en_size
                self.embedding_size = embedding_size
                self.hidden_size = hidden_size
        ...
        class DecoderRNN(nn.Module):
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithPretrainedEmbeddings ->  DecoderRNN
            """
            def __init__(self, dls, s2s, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithPretrainedEmbeddings.DecoderRNN, self).__init__()
                self.hidden_size = hidden_size
                self.target_vocab_size = s2s.vocab_es_size
                self.max_length = max_length
                self.embed = nn.Embedding(self.target_vocab_size, embedding_size)
                self.attn_calc = Seq2SeqNetworks.Seq2SeqWithPretrainedEmbeddings.Attention_BCB(dls, 
                                                               embedding_size, hidden_size, max_length)  
#                self.attn_calc = Seq2SeqNetworks.Seq2SeqWithPretrainedEmbeddings.Attention_SR(dls,
#                                                               embedding_size, hidden_size, max_length)
        ...
        class Attention_BCB(nn.Module):
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithPretrainedEmbeddings ->  Attention_BCB
            """
            def __init__(self, dl_studio, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithPretrainedEmbeddings.Attention_BCB, self).__init__()
                self.dl_studio = dl_studio
                self.max_length = max_length
                self.WC1 = nn.Linear( 2 * hidden_size, hidden_size )
                self.WC2 = nn.Linear( 2*hidden_size + embedding_size, embedding_size )
        ...
        class Attention_SR(nn.Module):
            ...
            ClassPath:  Seq2SeqNetworks -> Seq2SeqWithPretrainedEmbeddings ->  Attention_SR
            """
            def __init__(self, dl_studio, embedding_size, hidden_size, max_length):
                super(Seq2SeqNetworks.Seq2SeqWithPretrainedEmbeddings.Attention_SR, self).__init__()
                self.W = nn.Linear(embedding_size + hidden_size, max_length)
                self.attn_combine = nn.Linear(3*hidden_size, hidden_size)      
...
#_________________________  End of Seq2SeqNetworks Class Definition ___________________________
