...
def _by_dict(options: Dict):
    if not options:
        return {}
    return {k: v for k, v in options.items()}


def _by_subclass(options: Enum):
    return EnumUtil.create_dict_by_enum(options)


def create_static(options: AllowedOptionsTypes) -> dict:
    if is_dict(options):
        return _by_dict(options)
    elif is_enum_subclass(options):
        return _by_subclass(options)
    else:
        raise ValueError(
...
class Allowed(IStaticOptions):
    ...
    def __init__(self, options: AllowedOptionsTypes):
        self._options = create_static(options)
