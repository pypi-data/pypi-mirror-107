...
class Reference(object):
	def __init__(self, raw_data):
		...
		content = data[data.find("{")+1:-1].split(',')
		self.ref = content[0]
		# lazy sanity fix
		content=content[1:]
		new_content=[]
		...
		for i in range(len(content)):
			...
			elif i+1<len(content) and content[i].count('{')==content[i].count('}')+content[i+1].count('}'):
				new_content+=[content[i]+','+content[i+1]]
				skip_ahead=True
			else:
				raise ValueError("Problems in formatting the following line of the reference: "+content[i])				
		content = new_content
		self.fields={}
		for el in content:
			tk = el.split('=')
			if len(tk)!=2:
				raise ValueError("Problems in formatting the following line of the reference: "+el)
