...
class GatherLastToken(Layer):
    """
    获取最后一个token对应的output，如果指定pad_token_id，则是最后一个非pad_token对应的output，否则为序列最后一个token对应的output
    """
    ...
    def call(self, inputs, **kwargs):
        ...
        # use iteration to gather value from batch
        result = []
        for p, i in zip(logits, sequence_lengths):
            r = K.gather(p, i)
            result.append(r)
