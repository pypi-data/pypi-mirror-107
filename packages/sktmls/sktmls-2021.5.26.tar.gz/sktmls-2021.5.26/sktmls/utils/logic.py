...
class LogicProcessor:
    ...
    def _if(self, data, pf_client, graph_client, *args):
        for i in range(0, len(args) - 1, 2):
            if self.apply(args[i], data, pf_client, graph_client):
                return self.apply(args[i + 1], data, pf_client, graph_client)
        if len(args) % 2:
            return self.apply(args[-1], data, pf_client, graph_client)
        else:
            return None

    def _with(self, data, pf_client, graph_client, *args):
        if len(args) % 2 == 0:
        ...
        for i in range(0, len(args), 2):
            if i == len(args) - 1:
                break
            data[args[i]] = self.apply(args[i + 1], data, pf_client, graph_client)

        results = self.apply(body, data, pf_client, graph_client)
    ...
    def _for(self, data, pf_client, graph_client, *args):
        results = []
        it = self.apply(args[0], data, pf_client, graph_client)
        for i in it:
            data["i"] = i
            results.append(self.apply(args[1], data, pf_client, graph_client))
        data.pop("i", None)
        return results
    ...
    def apply(self, tests, data=None, pf_client=None, graph_client=None):
        """
        ## Args
        ...
        if operator == "if":
            return self._if(data, pf_client, graph_client, *values)

        if operator == "with":
            return self._with(data, pf_client, graph_client, *values)

        if operator == "for":
            return self._for(data, pf_client, graph_client, *values)

        if not isinstance(values, list) and not isinstance(values, tuple):
            values = [values]

        values = [self.apply(val, data, pf_client, graph_client) for val in values]
