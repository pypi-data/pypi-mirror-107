...
class GenericLogicModel(MLSGenericModel, MLSTrainable):
    ...
    def predict(self, x: List[Any], **kwargs) -> Dict[str, Any]:
        ...
        preprocessed_x = self._preprocess(x, kwargs.get("keys", []), pf_client, graph_client)
        y = self._ml_predict(preprocessed_x)
        items = self._postprocess(x, kwargs.get("keys", []), y, pf_client, graph_client) or []
    ...
    def _preprocess(
        ...
        try:
            return logic_processor.apply(
                self.preprocess_logic, data=data, pf_client=pf_client, graph_client=graph_client
            )
    ...
    def _postprocess(
        ...
        try:
            return logic_processor.apply(
                self.postprocess_logic, data=data, pf_client=pf_client, graph_client=graph_client
            )
        except Exception as e:
