...
class ConditionalGenericLogicModel(MLSGenericModel):
    ...
    def predict(self, x: List[Any], **kwargs) -> Dict[str, Any]:
        ...
        result = self._select_model(x, kwargs.get("keys", []), pf_client, graph_client)
        ...
        preprocessed_x = self._preprocess(model_index, x, kwargs.get("keys", []), pf_client, graph_client)
        y = self._ml_predict(model_index, preprocessed_x)
        items = self._postprocess(model_index, x, kwargs.get("keys", []), y, pf_client, graph_client) or []

        return {"items": items}

    def _select_model(
        self, x: List[Any], additional_keys: List[Any], pf_client: MLSProfileAPIClient, graph_client: MLSGraphAPIClient
        ...
        try:
            return logic_processor.apply(
                self.model_selection_logic, data=data, pf_client=pf_client, graph_client=graph_client
            )
    ...
    def _preprocess(
        ...
        try:
            return logic_processor.apply(
                self.preprocess_logics[model_index], data=data, pf_client=pf_client, graph_client=graph_client
            )
        except Exception as e:
    ...
    def _postprocess(
        ...
        try:
            return logic_processor.apply(
                self.postprocess_logics[model_index], data=data, pf_client=pf_client, graph_client=graph_client
            )
        except Exception as e:
