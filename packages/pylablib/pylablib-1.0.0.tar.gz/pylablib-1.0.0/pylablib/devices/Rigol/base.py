...
class RigolError(DeviceError):
    """Generic Rigol devices error"""
class RigolBackendError(RigolError,DeviceBackendError):
    """Generic Rigol backend communication error"""
...
class IRigolScope(SCPI.SCPIDevice):
    """
    Generic Rigol oscilloscope.
    ...
    _trig_comm="TRIGGER"  # command used to set up trigger
    ...
    Error=RigolError
    ReraiseError=RigolBackendError
    ...
    def _read_sweep_fast(self, channel, wfmpre=None, timeout=None):
        ...
        if len(trace)!=wfmpre["pts"]:
            raise RigolError("received data length {0} is not equal to the number of points {1}".format(len(trace),wfmpre["pts"]))
...
class TDS2000(IRigolScope):
    """
    Rigol TDS2000 series oscilloscope.
...
class DPO2000(IRigolScope):
    """
    Rigol DPO2000 series oscilloscope.
