...
class Helper(DataClass, abc.ABC):
	...
	def cursor(self):
		...
		The object returned should have the following methods
		* execute
		* executemany
		* being an iterable
		"""
...
class IterableQuery:
	...
	def intersection(self, *others):
		...
		return self.__class__(self.helper, iq, self.index, self.apply)
	...
	def project(self, index=None, project_query=True):
		...
		return self.__class__(self.helper, self.query, index=index, apply=self.apply)
	def remap(self, f):
		if f is None:
			return self
		else:
			if self.apply:
				f = lambda e: f(self.apply(e))
			return type(self)(self.helper, self.query, self.index, f)
