...
class Graph(nxclasses.graph.Graph, abc.ABC):
	...
	node_cache_level: integer, default=0
	...
	edge_cache_level: integer, default=0
	...
	def __init__(
		...
		lazy=None, static=False, node_cache_level=0, edge_cache_level=0,
		...
		args, kwargs = (), dict(
			...
			lazy=lazy, static=static, node_cache_level=node_cache_level, edge_cache_level=edge_cache_level,
	...
	def __pre_init__(self, *args, db=None, sql_logger=notProvidedArg, autocommit=False, **kwargs):
		...
		return args, kwargs
	def __graph_init__(self, *args, schema=None, create=None, insert_schema=None, masterid=None, name=None, static=False, node_cache_level=0, edge_cache_level=0, **kwargs):
		kwargs.update(schema=schema, create=create, insert_schema=insert_schema, masterid=masterid, name=name, static=static, node_cache_level=node_cache_level, edge_cache_level=edge_cache_level)
		if schema is None:
		...
		return args, kwargs
	def __load_graph_init__(self, *args, create=False, lazy=None, masterid=None, name=None, schema=None, static=False, node_cache_level=0, edge_cache_level=0, **kwargs):
		"""
		...
		if not schema or masterid is None:
			schema, masterid = self.master.load_graph_schema(masterid=masterid, name=name, to_directed=self.is_directed())
		elif not self.master.is_initialized:
			raise NetworkDiskError("Cannot find graph schema in database without master table")
		self.setsignature(schema, masterid, lazy=lazy, create=create, static=static, node_cache_level=node_cache_level, edge_cache_level=edge_cache_level)
		return args, kwargs
	def __create_graph_init__(self, *args, schema=True, lazy=None, create=None, insert_schema=None, masterid=None, name=None, static=False, node_cache_level=0, edge_cache_level=0, **kwargs):
		"""
	...
	def setsignature(self, schema, masterid, lazy=None, create=None, static=False, node_cache_level=0, edge_cache_level=0):
		...
		self.node_dict_factory = lambda: TupleDict(master, schema.nodes, lazy=lazy, cache_level=node_cache_level)
		self.node_attr_dict_factory = dict
		self.adjlist_outer_dict_factory = lambda: TupleDict(master, schema.adj, lazy=lazy, cache_level=edge_cache_level)
		...
		self.graph_attr_dict_factory = lambda: TupleDict(master, schema.graph, lazy=lazy)
		self.edgestore = TupleDict(master, schema.edgestore, lazy=lazy, cache_level=edge_cache_level)
		if self.is_directed():
			self.pred_dict_factory = lambda: TupleDict(master, schema.pred, lazy=lazy, cache_level=edge_cache_level)
			self.write_edge_target = self.edgestore
			self.revedgestore = TupleDict(master, schema.revedgestore, lazy=lazy, cache_level=edge_cache_level)
		elif schema.asymedges is None:
			self.asymedges = None
			self.write_edge_target = self.edgestore
		else:
			self.asymedges = TupleDict(master, schema.asymedges, lazy=lazy, cache_leve=edge_cache_level)
	...
	def __getstate__(self):
		return dict(helper=self.helper, schema=self.schema, masterid=self.masterid, lazy=self.lazy, static=self.static)
	...
	def add_edge(self, u_of_edge, v_of_edge, refresh_nodes=True, **attr):
		...
		"""
		with self.helper.transaction():
			self.add_edge_without_data(u_of_edge, v_of_edge, refresh_nodes=refresh_nodes)
	...
	def add_edges_from(self, ebunch_to_add, refresh_nodes=True, **attr):
		...
		with self.helper.transaction():
			if iter(bunch) is bunch:
				...
				self.add_edges_without_data_from(edge_bunch, refresh_nodes=refresh_nodes)
