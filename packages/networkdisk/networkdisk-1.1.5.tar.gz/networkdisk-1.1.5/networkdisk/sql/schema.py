...
class SchemaTable(SchemaContainer):
	def __init__(self, dialect, name, defquery=None, constraints=(), temporary=False):
		...
		self.rowid = self.SchemaChildClass("rowid", sqltype="INTEGER", primarykey="REPLACE", unique=True, autoincrement=True, container_name=self.name, index_in_container=len(self.children),)
		if defquery:
	...
	def __getstate__(self):
		...
		state.update(children=self.children, constraints=self.constraints, defquery=self.defquery, temporary=self.temporary, rowid=self.rowid)
		return state
	def __hash__(self):
...
class SchemaView(SchemaContainer, dialect.queries.FromQueriesReadQuery.func, dialect.queries.FromQueryQuery.func):
	...
	def set_columns(self, column_names):
		...
		for idx, bcol in enumerate(self.internal_columns):
			...
			seen.add(alias)
			fcol = self.SchemaChildClass(name=alias, index_in_container=idx, container_name=self.name, encoder=encoder, sqltype=sqltype)
