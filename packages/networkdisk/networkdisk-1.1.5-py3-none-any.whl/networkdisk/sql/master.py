...
class MasterGraphs(collections.abc.Mapping):
	...
	#Copied from Graph
	def load_graph_schema(self, name=None, masterid=None, directed=None, to_directed=None):
		""" Load the Graph schema

		Parameters
		----------
		name:	non-mutable value or None, default=None
			The name of the graph schema to fetch

		masterid:	int or None, default=None
		...
		to_directed: bool or None, default=None
			If not None, the parameter has two effects:  (1) it will allow to
			resolve possible ambiguity in some cases;  (2) it will convert a
			loaded schema to the desired type.  Indeed, if True (resp. False)
			then, in case of multiple graph schema in the master table, the
			graph schema of the desired type is preferred, if it is unique
			(otherwise an error on ambiguity is raised).  When the directed
			parameter is also given (see above), only the second effect applies.
		...
		try:
			...
			if masterid:
				if name:
					raise NetworkDiskMetaError(f"No {graphtype} with masterid '{masterid}' and name '{name}' found")
				else:
					raise NetworkDiskMetaError(f"No {graphtype} with masterid '{masterid}' found")
			elif name:
		...
		try:
			...
			if to_directed is None or gcls == res[2]:
				...
				if name:
					raise NetworkDiskMetaError(f"Ambiguous selection of {graphtype} with name '{name}'")
				else:
