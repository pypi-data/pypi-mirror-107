...
class ReadOnlyTupleDictView(collections.abc.Mapping):
	...
	def __new__(cls, rowstore, address, cache_level=0, **kwargs):
		...
		self.__setstate__(dict(rowstore=rowstore, address=address, cache_level=cache_level, **kwargs))
		assert self.height>0
		return self
	def __getstate__(self):
		return dict(rowstore=self.rowstore, address=self.address, cache_level=self.cache_level)
	...
	def __getitem__(self, k):
		...
		if self.height == 1:
			...
			if not self.lazy and not self.rowstore.is_keyprefix(key_prefix):
				raise KeyError(key_prefix)
			res = self.build_from(self, address=key_prefix, cache_level=self.cache_level - 1)
			if self.cache_level > 0:
				self._cache[k] = res
			return res
	def __len__(self):
		if self.cache_level > 0:
			if self._full_key is not None:
				return len(self._full_key)
	...
	def __iter__(self):
		...
		if self.cache_level>0:
			self._full_key = set(res)
			res = iter(self._full_key)
		return res
	def __contains__(self, k):
		...
		if self.cache_level>0:
			if k in self._cache:
				return True
			if self._full_key is not None:
				return k in self._full_key
		return self.rowstore.is_keyprefix(self.address+(k,))
	def items(self):
		if self.cache_level > 0:
			if self._full_key is not None:
				if all(k  in self._cache for k in self._full_key):
		...
		if self.height == 1:
			res = self.rowstore.select(
				...
				start=preflen, stop=preflen+2,
				distinct=True, notnull=preflen,
				orderby=(preflen,)
			)
		else:
			res = super().items()
		if self.cache_level>0:
			...
			self._full_key = [k for k,_ in res]
		return res
...
class BaseAbstractRowStore(abc.ABC):
	...
	def view(self, key_prefix=()):
		return self._View(self, key_prefix, cache_level=self.cache_level - len(key_prefix))
