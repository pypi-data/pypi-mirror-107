...
class TestResults:
    ...
    def test_get_plan(self, results, response, get, mocker):
        url = results.tuxapi_url + "/v1/groups/{}/projects/{}/plans/{}".format(
            results.group, results.project, results.uid
        )
        response._content = b'{"a": "b"}'

        assert response.status_code == 200
        assert results.get_plan() == ({"a": "b"}, url.replace("plans", "tests"))
        get.assert_called_with(
            url,
            headers=results.headers,
            timeout=mocker.ANY,
