...
def test_plan_cli_options(mocker, plan_config, tuxsuite_config):
    ...
    plan_obj.builds = [mocker.Mock(uid="1"), mocker.Mock(uid="2")]
    plan_obj.tests = [
        mocker.Mock(uid="1", wait_for="1"),
        mocker.Mock(uid="2", wait_for=None),
    ]
    ...
    plan_obj.results = {
        "builds": {
            "1": tuxsuite.build.BuildState(
                build=None,
                state="finished",
                status="",
                message="build 1 pass",
                icon="🎉",
                cli_color="green",
            ),
            "2": tuxsuite.build.BuildState(
                build=None,
                state="finished",
                status="",
                message="build 2 pass",
                icon="🎉",
                cli_color="green",
            ),
        },
    ...
    assert (
        """
Summary: http://api/v1/groups/grp/projects/prj
* 🎉 build 1 pass: None
  -> 🎉 test 1 pass: None
* 🎉 build 2 pass: None
* 🎉 test 2 pass: None"""
...
def test_results_cli(
    ...
    mocker.resetall()
    results = mocker.patch(
        "tuxsuite.Results.get_plan", return_value=(sample_all_result, "http://api")
    )
    result = runner.invoke(
        tuxsuite.cli.results,
        [
    ...
    assert result.exit_code == 0
    assert results.call_count == 1
    assert format_result.call_count == 5
