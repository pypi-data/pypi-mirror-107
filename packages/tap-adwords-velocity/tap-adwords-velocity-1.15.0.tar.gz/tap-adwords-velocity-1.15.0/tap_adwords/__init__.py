...
def sync_report(stream_name, stream_metadata, sdk_client):
    ...
    while start_date < get_end_date():
        ...
        start_date = (start_date+relativedelta(days=1)) if (start_date+relativedelta(days=8)) > datetime.date.today() else (start_date+relativedelta(days=8)) 
...
def sync_report_for_day(stream_name, stream_schema, sdk_client, start, field_list): # pylint: disable=too-many-locals
    ...
    customer_id = sdk_client.client_customer_id
    end =  start if (start+relativedelta(days=7)).strftime('%Y%m%d') > datetime.date.today() else (start+relativedelta(days=7)).strftime('%Y%m%d') 
    report = {
