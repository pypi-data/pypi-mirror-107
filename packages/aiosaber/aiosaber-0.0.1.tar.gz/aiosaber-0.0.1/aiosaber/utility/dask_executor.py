...
from aiosaber.utility.utils import import_object
...
class DaskExecutor(object):
    ...
    def __init__(
        ...
        self.cluster_kwargs = {} if not cluster_kwargs else deepcopy(cluster_kwargs)
    ...
    async def shutdown(self):
        ...
        if not isinstance(self.cluster, str):
            await self.cluster.__aexit__(None, None, None)
        await self.client.__aexit__(None, None, None)

    def submit(self, fn: Callable, *args: Any, extra_context: dict = None, **kwargs: Any) -> Future:
        """
        ...
        if self._should_run_event is None:
            ...
            return self.client.submit(self._maybe_run, self._should_run_event.name, fn, *args, **kwargs)
    ...
    async def _watch_dask_events(self) -> None:
        ...
        try:
            ...
            self.logger.debug("Failure while watching dask worker events", exc_info=True)
        finally:
            if comm is not None:
                try:
                    await comm.close()
    ...
    def _pre_start_yield(self) -> None:
        ...
        if self.address is not None or is_inproc:
            ...
            self._should_run_event = Event(f"flowsaber-{uuid.uuid4().hex}", client=self.client)
