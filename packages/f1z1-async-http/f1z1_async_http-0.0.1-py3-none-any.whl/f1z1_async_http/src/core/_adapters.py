...
from . import IAdapters, MergeResult
from ..components import IMethod, IURL, IOptional, IOptions, TimoutTypes

ConvertedMethodTypes = Union[AnyStr, IMethod]
ConvertedURLTypes = Union[AnyStr, IURL]
ConvertedOptionTypes = Union[None, IOptions]
ConvertedTimeTypes = Union[TimoutTypes, Timeout]


class MethodAdapter(IAdapters):

    def __init__(self, method: ConvertedMethodTypes):
        self._method = self._to_anystr(method)

    def convert(self) -> AnyStr:
        return self._method

    def _to_anystr(self, method: ConvertedMethodTypes) -> AnyStr:
...
class URLAdapter(IAdapters):
    ...
    def __init__(self, url: ConvertedURLTypes):
        self._url = self._to_anystr(url)

    def convert(self) -> URL:
        return URL(self._url)

    def _to_anystr(self, url: ConvertedURLTypes) -> AnyStr:
        if self._is_url(url):
...
class OptionsAdapter(IAdapters):
    ...
    def __init__(self,
                 glob_timeout: ConvertedTimeTypes,
...
class Adapters(IAdapters):
    ...
    def __init__(self,
                 method: ConvertedMethodTypes,
                 url: ConvertedURLTypes,
                 glob_timeout: ConvertedTimeTypes,
                 options: IOptions = None):
