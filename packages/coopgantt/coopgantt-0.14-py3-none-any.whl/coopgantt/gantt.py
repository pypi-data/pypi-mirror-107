...
class ScheduledActivity(ABC):
    ...
    def schedule(self, start_date, max_resources_by_type_for_project: Dict[str, int]):
        ...
        for activity, completion_req in self.dependencies.items():
            ...
            dep_completion_thresh_date = self.project.chart.date_plus_working_days(activity.start_date, dep_completion_thresh_days - 1)
...
if __name__ == "__main__":
    date = date(month=5, year=2021, day=18)
    gantt = Gantt("Name")

    print(date)

    print(gantt.date_plus_working_days(start_date=date, days=5 - 1))
