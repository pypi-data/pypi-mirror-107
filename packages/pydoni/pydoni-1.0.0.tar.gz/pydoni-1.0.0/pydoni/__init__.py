...
def time_machine_start(tmutil_bin: typing.Union[str, pathlib.Path]):
    """
...
def time_machine_stop(tmutil_bin: typing.Union[str, pathlib.Path]):
    """
...
def macos_notify(title: str='',
                 subtitle: str=None,
                 message: str=None,
                 app_icon: typing.Union[str, pathlib.Path]=None,
                 content_image: typing.Union[str, pathlib.Path]=None,
                 command: str=None,
                 open_iterm: bool=False):
    """
    ...
    if os.name.lower() != 'posix':
        raise OSError('Operating system must be macOS to use macos_notify()!')
    ...
    if open_iterm:
        expected_iterm_path = '/Applications/iTerm.app'
        assert os.path.isdir(expected_iterm_path)
        cl_string.append(f"-execute 'open {expected_iterm_path}'")
    elif command is not None:
        cl_string.append('-execute {!r}'.format(command))

    # Build final command and execute
    cmd = f"{terminal_notifier_bin} {' '.join(cl_string)}"
    out = syscmd(cmd)
    logger.info(f'Fired macOS notification entitled "{title}"')
...
class AudioFile(object):
    ...
    def compress(self, output_fpath: typing.Union[None, str, pathlib.Path]=None):
        ...
        cmd = f'{self.ffmpeg_bin} -i "{self.fpath}" -map 0:a:0 -b:a 32k "{output_fpath}"'
        out = syscmd(cmd)

        logger.info(f'Compressed audio "{self.fpath}" to {output_fpath}')
        return out
    ...
    def join(self,
        ...
        cmd = f'{self.ffmpeg_bin} -f concat -safe 0 -i "{tmp_txtfile}" -c copy "{output_fpath}"'
        out = syscmd(cmd)
    ...
    def convert(self, output_fpath=None, output_format=['mp3', 'wav']):
        ...
        if output_fpath is None:
            output_fpath = os.path.splitext(self.fpath)[0] + '.' + output_format
...
def convert_audible(fpath: typing.Union[str, pathlib.Path], fmt: str, activation_bytes: str):
    ...
    cmd = '{ffmpeg_bin} -activation_bytes {activation_bytes} -i "{fpath}" -vn -c:a copy "{output_fpath}"'
    syscmd(cmd)
...
def mp4_to_mp3(fpath: typing.Union[str, pathlib.Path], bitrate: typing.Union[int, str]):
    ...
    cmd = 'f="{}";{} -i "$f" -acodec libmp3lame -ab {} "${{f%.mp4}}.mp3";'.format(fpath, ffmpeg_bin, bitrate)
...
def m4a_to_mp3(m4a_fpath: typing.Union[str, pathlib.Path],
               mp3_fpath: typing.Union[None, str, pathlib.Path]=None):
    """
    Use ffmpeg to convert a .m4a file to .mp3.
    """
    ffmpeg_bin = find_binary('ffmpeg', abort=True)
    m4a_fpath = os.path.abspath(m4a_fpath)

    if mp3_fpath is None:
        mp3_fpath = pydoni.append_filename_suffix(m4a_fpath, '-CONVERTED')
        mp3_fpath = os.path.splitext(mp3_fpath)[0] + '.mp3'

    cmd = f'{ffmpeg_bin} -i "{m4a_fpath}" -codec:v copy -codec:a libmp3lame -q:a 2 "{mp3_fpath}"'
    out = syscmd(cmd)

    logger.info(f'Converted "{m4a_fpath}" to format "{mp3_fpath}"')
    return out
