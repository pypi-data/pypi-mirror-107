...
from os.path import basename, dirname, isfile, isdir, getmtime, join, getctime
...
def pg_dump(backup_dir,
    ...
    backup_subdir = join(backup_dir, subdir)
    mkdir(backup_subdir)
    ...
    for f in dumped_files:
        result['dumped_files'][basename(f)] = dict(
            ...
            created=datetime.fromtimestamp(getctime(f)).strftime('%Y-%m-%d %H:%M:%S.%f'),
