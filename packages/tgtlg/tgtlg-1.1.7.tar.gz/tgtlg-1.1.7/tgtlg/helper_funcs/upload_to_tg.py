    ...
    if os.path.isdir(local_file_name):
        ...
        if not message.photo:
            new_m_esg = await message.reply_text(
                f"Found {len(directory_contents)} files <a href='tg://user?id={from_user}'>{message.from_user.first_name}</a>",
                quote=True
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    if os.path.isfile(file_upload):
        ...
        await messa_ge.reply_text(
            f"Uploaded successfully `{os.path.basename(file_upload)}` <a href='tg://user?id={g_id}'>{message.from_user.first_name}</a>n Size: {gjay}",
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(
            f"Uploaded successfully `{os.path.basename(file_upload)}` <a href='tg://user?id={g_id}'>{message.from_user.first_name}</a>\n Size: {gjay}",
            reply_markup=button_markup,
        )
