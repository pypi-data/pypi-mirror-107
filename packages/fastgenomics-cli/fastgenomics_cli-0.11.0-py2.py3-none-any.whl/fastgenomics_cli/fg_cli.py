...
"""
    ...
    login_parser.add_argument(
        "-m", "--login_method", help="the login method 'pat' (personal access token), 'password' or 'bearer'. Default: pat",
        choices=["pat", "password", "bearer"], default="pat")
...
"""
    ...
    dataset_create_parser = _add_parser(
        dataset_subparsers, "create", action="dataset-create", helptext="create a FASTGenomics dataset")

    dataset_create_parser.add_argument("-T",
                                       "--title", help="the title of the FASTGenomics dataset",
                                       type=str, required=False)

    dataset_create_parser.add_argument("--dataset_type", help="the datasettype",
                                       type=str, nargs='?', default="cs-singlecell")
    ...
    dataset_set_metadata_parser = _add_parser(
        dataset_subparsers, "set-metadata", action="dataset-set-metadata", helptext="set the metadata for a FASTGenomics dataset")

    dataset_set_metadata_parser.add_argument(
        "-id", help="the id of the dataset", type=str, required=True)

    dataset_set_metadata_parser.add_argument(
        "-d", "--data", help="the metadate to set. Use @<filename> to provide the data by file or '{ json }'", type=str, required=True)
    ...
    try:
        ...
        if args.action == "lfs-create":
            ...
            if args.data.startswith("@"):
                filename = args.data[1:len(args.data)]
                logger.info(
                ...
                logger.info(
                    f"setting metadata for dataset '{args.id}'")
