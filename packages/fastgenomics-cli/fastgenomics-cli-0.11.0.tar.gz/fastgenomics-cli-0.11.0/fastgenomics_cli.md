...
```bash
usage: fg-cli login [-h] [-v] -u USER -p PASSPHRASE [-m {pat,password,bearer}]
                   [--url URL]
...
```bash
fg-cli dataset set-metadata
```

output

```bash
usage: fg-cli dataset set-metadata [-h] [-v] -id ID -d DATA

set the metadata for a FASTGenomics dataset

optional arguments:
  -h, --help            show this help message and exit
  -v, --verbose         Activate verbose output
  -id ID                the id of the dataset
  -d DATA, --data DATA  the metadate to set. Use @<filename> to provide the data by file or '{ json }'
```
