...
def attach_kubernetes(name, compute_resource_id, workspace_name=None, resource_group_name=None, amlk8s_config=None):
    import os
    import json
    workspace = get_workspace_or_default(workspace_name=workspace_name, resource_group=resource_group_name)
    ...
    amlk8s_config_json = {}
    if amlk8s_config and os.path.isfile(amlk8s_config):
        with open(amlk8s_config, 'r') as fp:
            amlk8s_config_json = json.load(fp)

    k8s_attach_configuration = KubernetesCompute.attach_configuration(resource_id=compute_resource_id,
                                                                      aml_k8s_config=amlk8s_config_json)
