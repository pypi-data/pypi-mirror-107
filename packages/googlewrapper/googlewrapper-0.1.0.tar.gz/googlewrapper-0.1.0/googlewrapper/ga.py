...
class GoogleAnalytics():
  ...
  def set_dimensions(self,d):
    ...
    d: which ga dimensions we want to use, ensure they are named correct
      type: list of strings
      
    if you would like to pull multiple days worth of data, and would like to 
    maintain the time aggregation, remember to include one of the following:
  ...
  def set_dimension_filters(self,f):
    ...
    operator: how to match 
      possible values: ["REGEXP","BEGINS_WITH","ENDS_WITH",
                          ...
                          "NUMERIC_GREATER_THAN","NUMBER_LESS_THAN","IN_LIST"]
      type: str
      default: REGEXP
  ...
  def set_metric_filters(self,m):
    ...
    operator: how you want to compare 
      possible values: ["EQUAL", "LESS_THAN","GREATER_THAN","IS_MISSION"]
      type: string 

    for more details:
      https://developers.google.com/analytics/devguides/reporting/core/v4/rest/v4/reports/batchGet#operator_1
    '''
  ...
  def build_request(self,
      ...
      Previously have assigned the following variables using their respective modules
      ...
      page_token: if you reached your max, pass this in to use this as starting row
        type: str
  ...
  def pull(self,request_body):
    '''
    Parameters:
    request_body: structured request body for API call, created by using self.build_request
      type: dict

    this function is often called right after `self.build_request()` being triggered by the 
      pull=True parameter from the previous module

    returns the data in two options depending on how the class was initialized
    self.make_df is the variable that will change this return
  ...
  def create_df(self):
    '''
    Loops through the Google Analytics batchGet() response json to create a pd.DataFrame
    ...
    for x in self.raw_data['reports']:
      ...
      metric_df = pd.DataFrame([m['metrics'][0]['values'] for m in row_data], columns = met)
