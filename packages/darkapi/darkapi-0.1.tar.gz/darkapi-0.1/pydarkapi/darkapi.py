# API BY BOTFATHER
# https://dark2web.net/members/284715/
...
class Api:
    def __init__(self, cookie, userAgent):
        ...
        res = requests.get("https://dark2web.net", headers=self.headers).text
        xf = BeautifulSoup(res, 'lxml')
        self._xfToken = xf.find('input', {'name': '_xfToken'})['value']
    
    def getThreadInfo(self, thread_id: int):
        response = requests.get(f'https://dark2web.net/threads/{thread_id}/', headers=self.headers)
    ...
    def createPost(self, thread_id: int, message_html):
        requests.post(f'https://dark2web.net/threads/{thread_id}/add-reply', headers=self.headers, data={
            '_xfToken': self._xfToken,
            'message_html': message_html
        })

    def editPost(self, post_id: int, message_html):
        requests.post(f'https://dark2web.net/posts/{post_id}/edit', headers=self.headers, data={
            '_xfToken': self._xfToken,
            'message_html': message_html
        })
    ...
    def getPosts(self, thread_id: int, page: int):
        if page == 1:
            ...
            seen = f'page-{page}'
        response = requests.get(f'https://dark2web.net/threads/{thread_id}/{seen}', headers=self.headers)
        soup = BeautifulSoup(response.text, 'lxml')
    ...
    def editThread(self, thread_id: int, message_html):
        response = requests.get(f'https://dark2web.net/threads/{thread_id}/', headers=self.headers)
        soup = BeautifulSoup(response.text, 'lxml')
    ...
    def getThreads(self, forum_id: int, page: int):
        if page == 1:
            ...
            seen = f"page-{page}"
        response = requests.get(f'https://dark2web.net/forums/{forum_id}/{seen}', headers=self.headers)
        soup = BeautifulSoup(response.text, 'lxml')
