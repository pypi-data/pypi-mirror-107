...
class NuisanceParameterHarmonizer(object):
    ...
    def filter_nuis(source, patterns=None, excludes=None, exact=False):
        ...
        if excludes is not None:
            anti_matches = WSNuisParamHarmonizer.filter_nuis(source, excludes)
    ...
    def display_mappings(mappings, reference, level=0):
        for k, v in mappings.items():
            ...
            if 'source' in reference[k]:
                WSNuisParamHarmonizer.display_mappings(v, reference[k]['source'], level=1)
            else:
                for old_name, new_name in v.items():
                    WSNuisParamHarmonizer.display_difference(old_name, new_name)
    ...
    def reshape_mappings(mappings, roots=[], result=None):
        ...
        for key, value in mappings.items():
            if isinstance(value, dict):
                WSNuisParamHarmonizer.reshape_mappings(value, roots + [key], result)
    ...
    def flatten_mappings(mappings):
        flattened_mappings = {}
        # check duplicates
        reshaped_mappings = WSNuisParamHarmonizer.reshape_mappings(mappings)
    ...
    def get_mappings(nuis_names, reference):
        ...
        for group in reference:
            ...
            selected += WSNuisParamHarmonizer.filter_nuis(nuis_names, reference[group].get('pattern', None), 
                                                          reference[group].get('exclude', None))
            # NPs that match exactly
            selected += WSNuisParamHarmonizer.filter_nuis(nuis_names, reference[group].get('match', None))
            if not selected:
                continue
            if ('source' not in reference[group]) and ('names' not in reference[group]):
                ...
                if 'names' in reference[group]:
                    ...
                    for key, value in reference[group]['source'].items():
                        names += value['names']
                suggested_names = WSNuisParamHarmonizer.get_suggested_names(selected, names, cutoff)
                if suggested_names:
                    if 'names' in reference[group]:
                        mappings[group] = suggested_names
                    elif 'source' in reference[group]:
                        source = reference[group]['source']
                        mappings[group] = WSNuisParamHarmonizer.match_source(suggested_names, source)
        return mappings
    ...
    def get_harmonize_map(self, nuis_names):
        ...
        if len(unknown_nuis) > 0:
            ...
            for unknown in unknown_nuis:
                WSNuisParamHarmonizer.display_difference(unknown, '?')
    ...
    def harmonize(self, ws_paths, outfile=None):
        if isinstance(ws_paths, str):
            ws_paths = [ws_paths]
        nuis_names = WSNuisParamHarmonizer.get_nuiance_parameter_names_collection(ws_paths)
        harmonize_map = self.get_harmonize_map(nuis_names)
        if outfile is not None:
            json.dump(harmonize_map, open(outfile, 'w'), indent=2)
        return harmonize_map
    
    def _harmonize_multi_input(self, input_expr):
        harmonize_map = {}
        if isinstance(input_expr, (str, list)):
            harmonize_map = self.harmonize(input_expr)
        elif isinstance(input_expr, dict):
            for k,v in input_expr.items():
                harmonize_map[k] = self.harmonize(v)
        return harmonize_map
            
    def harmonize_multi_input(self, input_config_path, outfile=None):
        config = json.load(open(input_path))
        harmonize_map = self._harmonize_multi_input(config)
        if outfile is not None:
            json.dump(harmonize_map, open(outfile, 'w'), indent=2)
        return harmonize_map
