...
class OpenPYXLReader(FileHandler):
    ...
    def parse(self):
        if self.streams:
            for handler, buffer, kwargs in self.streams:
                self.__parse(handler=handler, buffer=buffer, **kwargs)
        else:
            return self.__parse()
            ...
            with load_workbook(self.file_path) as xls_file:
                ...
                for tab in xls_file.sheetnames:
                    if not self.sheet_names or tab.lower() in self.sheet_names:
                        ...
                        if handler:
                            self.__openpyxl_sheet(sheet=xls_ws, handler=handler, buffer=buffer, **kwargs)
                        else:
    ...
    def __openpyxl_sheet(self, sheet, handler=None, buffer=None, **kwargs):
        import openpyxl
        ...
        for row in sheet.rows:
            if 0 <= self.skiprows <= row_number and 0 <= self.header <= row_number and (row_number <= self.nrows
                ...
                if handler and buffer < len(data):
                    ...
                    handler(self.file_path, df, row_number - len(df) + 1, row_number, **kwargs)
                    data.clear()
                    data.append(header)
                    last_row_with_data = -1
...
class ODFReader(FileHandler):
        ...
        try:
            for xls_ws in self.__odf_file.getElementsByType(Table):
                ...
                if not self.__sheet_names or tab.lower() in self.__sheet_names:
                    if handler:
                        self.__odf_sheet(sheet=xls_ws, handler=handler, buffer=buffer, **kwargs)
                    else:
    ...
    def __odf_sheet(self, sheet, handler=None, buffer=None, **kwargs):
...
class XLRDReader(FileHandler):
        ...
        try:
            for tab in self.__xls_file.sheet_names():
                if not self.sheet_names or tab.lower() in self.sheet_names:
                    ...
                    if handler:
                        self.__xlrd_sheet(sheet=xls_ws, handler=handler, buffer=buffer, **kwargs)
                    else:
                        sheets[tab] = self.__xlrd_sheet(sheet=xls_ws, handler=handler, buffer=buffer)
        finally:
            if self.__xls_file and hasattr(self.__xls_file, 'close'):
    ...
    def __xlrd_sheet(self, sheet, handler=None, buffer=None, **kwargs):
        data: List[List[Scalar]] = []
        row_number = 0
        header = None
        ...
        for i in range(sheet.nrows):
            if 0 < self.skiprows <= row_number <= self.nrows and 0 < self.header <= row_number:
                ...
                if handler and len(data) <= buffer:
                    df = self.convert_data(data)
                    handler(df, row_number - len(df) + 1, row_number, **kwargs)
                    data.clear()
                    data.append(header)
...
class PYXLSBReader(FileHandler):
        ...
        try:
            for tab in self.__xls_file.sheets:
                if not self.sheet_names or tab.lower() in self.sheet_names:
                    ...
                    if handler:
                        self.__pyxlsb_sheet(sheet=xls_ws, handler=handler, buffer=buffer, **kwargs)
                    else:
    ...
    def __pyxlsb_sheet(self, sheet, handler, buffer, **kwargs):
        data: List[List[Scalar]] = []
        row_number = 0
        header = None
