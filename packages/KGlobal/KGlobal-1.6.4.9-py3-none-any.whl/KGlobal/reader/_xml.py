    ...
    def __parse(self, handler=None, buffer=None, **kwargs):
        ...
        if is_an_xml(self.file_path):
            ...
            if isinstance(self.dict_var, dict):
                ...
                self.__parse_xml_root(xml_objs, handler, buffer, **kwargs)
            else:
                return self.__parse_xml_root(xml_objs, handler, buffer, **kwargs)

    @staticmethod
    def __parse_xml_root(xml_objs, handler=None, buffer=None, **kwargs):
        xml_dicts = list()
        row_num = 0
        header = None
        ...
        for xml_obj in xml_objs:
            ...
            if handler and buffer <= len(xml_dicts):
                handler(self.file_path, self.__parse_df(xml_dicts), row_num - len(xml_dicts) + 1, **kwargs)
                xml_dicts.clear()
