...
class LitBiLSTMAttention(pl.LightningModule):
    """
    Bilstm Attention包含
    是否是启用嵌入 ，默认不启用
    ...
    def __init__(self,vocab_size=100, dim=10,n_hidden=10,out_num_classes=10,embedding_enabled=False):
        """[summary]
        实现简单的双向长短记忆

        Args:
            vocab_size (int, optional): [description]. Defaults to 100.
            dim (int, optional): [description]. Defaults to 10.
            n_hidden (int, optional): [description]. Defaults to 10.
            out_num_classes (int, optional): [description]. Defaults to 10.
            embedding_enabled (bool, optional): [description]. Defaults to False.
        """
        super().__init__()
