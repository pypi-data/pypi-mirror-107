...
from DuDepParser.parser.utils import Embedding
...
class Environment(object):
    ...
    def __init__(self, args):
        ...
        if not os.path.exists(self.args.fields_path) or self.args.preprocess:
            ...
            self.WORD = field.Field('word',
                                    pad=utils.pad,
                                    unk=utils.unk,
                                    bos=utils.bos,
                                    lower=True)
            if self.args.feat == 'char':
                self.FEAT = field.SubwordField('chars',
                                               pad=utils.pad,
                                               unk=utils.unk,
                                               bos=utils.bos,
                ...
                self.FEAT = field.Field('postag', bos=utils.bos)
            self.ARC = field.Field('head',
                                   bos=utils.bos,
                                   use_vocab=False,
                                   fn=utils.numericalize)
            self.REL = field.Field('deprel', bos=utils.bos)
            if self.args.feat == 'char':
                self.fields = corpus.CoNLL(FORM=(self.WORD, self.FEAT),
                                           HEAD=self.ARC,
                                           DEPREL=self.REL)
            else:
                self.fields = corpus.CoNLL(FORM=self.WORD,
                                           CPOS=self.FEAT,
                                           HEAD=self.ARC,
                                           DEPREL=self.REL)

            train = corpus.Corpus.load(self.args.train_data_path, self.fields)
