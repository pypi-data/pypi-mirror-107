...
def _determine_grid_size(arr, starting_x_size=100, starting_y_size=100, increment=100, min_size=800, max_size=1100, x_name='lon', y_name='lat'):
    """
...
# def _process_grid_blocks(block_list, band_parameter_dict, feature, method, product_code, owner, aggregation_statistic, units, data_license, attribution, encoding, description, frequency_interval='T', utc_offset='0H', x_name='lon', y_name='lat', run_date=None):
#     """
...
class Grid(object):
    ...
    def process_datasets(self, dataset_list, remote, processing_code, public_url=None, run_date=None):
        ...
        # setattr(self, 'file_remote', file_remote)
    ...
    def determine_grid_block_size(self, starting_x_size=100, starting_y_size=100, increment=100, min_size=800, max_size=1100):
        ...
        """
        size_dict = _determine_grid_size(self.data[self.parameter], starting_x_size, starting_y_size, increment, min_size, max_size)
    ...
    def save_results(self, x_size=None, y_size=None, threads=30):
        ...
        s3 = s3_connection(self.s3_remote['connection_config'], threads*2)

        for ds in self.datasets:
            ds_new = put_remote_dataset(s3, self.s3_remote['bucket'], ds)
            ds_stations = put_remote_agg_stations(s3, self.s3_remote['bucket'], ds['dataset_id'], threads*2)

        # Aggregate all datasets for the bucket
        ds_all = put_remote_agg_datasets(s3, self.s3_remote['bucket'], threads*2)
