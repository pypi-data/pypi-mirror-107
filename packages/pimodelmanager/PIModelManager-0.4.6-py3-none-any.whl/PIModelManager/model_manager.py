...
class ModelManager:
    ...
    def upload_model(
        ...
        Args:
            ...
            delete_after (bool): Whether to delete the uploaded file
        ...
        if path.exists():
            ...
            with open(final_path, "rb") as fp:
                files = {"file": (final_path.name, fp, "multipart/form-data")}
            ...
            if response.ok:
                ...
                if(delete_after):
                    model_manager_logger.info(f"Deleting file from local storage: {final_path.name}")
                    final_path.unlink()
            else:
