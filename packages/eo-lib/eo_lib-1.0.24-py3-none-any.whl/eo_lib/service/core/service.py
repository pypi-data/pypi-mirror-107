...
class ConfigurationService(BaseService):
    ...
    def retrive_by_organization_and_application(self, organization, application):
        return self.session.query(Configuration).filter(Configuration.organization_id == organization.id,
                                                        Configuration.application_id == application.id).all()
...
class ApplicationReferenceService(BaseService):
    ...
    def retrive_by_external_id (self, external_id):
        ...
        if isinstance(external_id, int): external_id = str(external_id)
