from datetime import datetime, timedelta
from django.conf import settings
from django.core.management.base import BaseCommand
from django.contrib.auth import get_user_model
from django.template.loader import render_to_string
from django.utils.html import strip_tags
import pytz
import re


def to_text(html):
  return re.sub('[ \t]+', ' ', strip_tags(html)).replace('\n ', '\n').strip()


class Command(BaseCommand):
  help = 'I will send a digest.'

  def add_arguments(self, parser):
    parser.add_argument(
        "--dry-run",
        default=False,
        nargs="?",
        const=True,
        help="Do not send any mail",
    )

  def handle(self, *args, **options):
    # Set datetime, 7 days in past, UTC-based
    delta = pytz.UTC.localize(datetime.now() - timedelta(7))
    total = 0

    # Ensure the user accept to receive emails
    for user in get_user_model().objects.all():
      if hasattr(user, 'settings'):
        if user.settings.receiveMail:
          message_html = ""

          # The circles container from an user only contains circlememberships
          for circlemember in user.circles.all():
            circle = circlemember.circle

            if hasattr(circle, 'polls'):
              polls = circle.polls.get_queryset().filter(creationDate__gte=delta).order_by('-creationDate')
              polls_html = ""
              for poll in polls:
                if poll.author != user:
                  polls_html += render_to_string('digest/poll.html', {"poll": poll})
              if polls_html != "":
                message_html += render_to_string('digest/polls.html', {
                  "circle": circle,
                  "count": polls.count(),
                  "content": polls_html
                })

            if hasattr(circle, 'resources'):
              resources = circle.resources.get_queryset().filter(creationDate__gte=delta)
              resources_html = ""
              for resource in resources:
                if resource.user != user:
                  resources_html += render_to_string('digest/resource.html', {"resource": resource})
              if resources_html != "":
                message_html += render_to_string('digest/resources.html', {
                  "circle": circle,
                  "count": resources.count(),
                  "content": resources_html
                })

            if hasattr(circle, 'events'):
              events = circle.events.get_queryset().filter(creationDate__gte=delta)
              events_html = ""
              for event in events:
                if event.author != user:
                  events_html += render_to_string('digest/event.html', {"event": event})
              if events_html != "":
                message_html += render_to_string('digest/events.html', {
                  "circle": circle,
                  "count": events.count(),
                  "content": events_html
                })

          for poll in user.createdVotes.all():
            votes = poll.votes.get_queryset().filter(creationDate__gte=delta).order_by('-creationDate')
            votes_html = ""
            for vote in votes:
              if vote.user != user:
                votes_html += render_to_string('digest/vote.html', {"vote": vote})
            if votes_html != "":
              message_html += render_to_string('digest/votes.html', {
                "poll": poll,
                "count": votes.count(),
                "content": votes_html
              })


          if message_html != "":
            message_html = render_to_string('digest/template.html', {"settings": settings, "user": user, "content": message_html})
            total = total + 1
            if(options["dry_run"]):
              self.stdout.write(self.style.SUCCESS(user.urlid))
              self.stdout.write(message_html)
              self.stdout.write(to_text(message_html))
            else:
              try:
                send_mail(
                    'Votre semaine de La République ESS',
                    to_text(message_html),
                    (getattr(settings, 'DEFAULT_FROM_EMAIL', False)\
                      or getattr(settings, 'EMAIL_HOST_USER', False)\
                      or "noreply@" + settings.JABBER_DEFAULT_HOST),
                    [user.email],
                    fail_silently = False,
                    html_message = message_html
                )
                self.stdout.write(self.style.SUCCESS('Sent digest to ' + user.email))
              except:
                self.stdout.write(self.style.WARNING('Unable to send digest to ' + user.email))

    if(options["dry_run"]):
      self.stdout.write(self.style.SUCCESS('Digest done. Generated: ' + str(total)))
    else:
      self.stdout.write(self.style.SUCCESS('Digest done. Sent: ' + str(total)))
