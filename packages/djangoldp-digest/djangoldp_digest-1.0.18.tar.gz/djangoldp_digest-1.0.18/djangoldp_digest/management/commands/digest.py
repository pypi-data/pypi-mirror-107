          ...
          for circlemember in user.circles.all():
            ...
            if hasattr(circle, 'polls'):
              ...
              for poll in polls:
                ...
                polls_html += render_to_string('digest/poll.html', {"poll": poll, "alternate": color(alternate)})
              if polls_html != "":
            ...
            if hasattr(circle, 'resources'):
              ...
              for resource in resources:
                alternate = not alternate
                resources_html += render_to_string('digest/resource.html', {"resource": resource, "alternate": color(alternate)})
              if resources_html != "":
            ...
            if hasattr(circle, 'events'):
              ...
              for event in events:
                alternate = not alternate
                events_html += render_to_string('digest/event.html', {"event": event, "alternate": color(alternate)})
              if events_html != "":
