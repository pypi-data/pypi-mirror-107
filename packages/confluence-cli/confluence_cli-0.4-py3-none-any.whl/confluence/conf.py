...
from confluence import yamlparser
from confluence.cli.confluence_wrapper import ConfluenceWrapper
from confluence.cli.types import NavigableDict

def __check_vars():    
    if deconf("MATOMO_TOKEN_AUTH") and \
        deconf("CONFLUENCE_PGDB_PASS") and \
        deconf("CONFLUENCE_ADMIN_PASS"):
            print(".env vars set.")
    else:
...
def get_confluence_api() -> ConfluenceWrapper:
    ...
    try:
        ...
        raise ValueError("No Confluence server configured via conf.set_confluence_server()")
