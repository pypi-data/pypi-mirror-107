...
def get_video_from_wechat(bucket, access_token, media_id, path):
    ...
    try:
        video_url = requests.get(url).json().get("video_url")
        if video_url:
            download_from_internet_and_sync(bucket=bucket, url=video_url, path=path, timeout=160)
...
def get_audio_from_wechat(bucket, access_token, media_id, path):
    url = "https://api.weixin.qq.com/cgi-bin/media/get?access_token=%s&media_id=%s" % (access_token, media_id)
    download_from_internet_and_sync(bucket=bucket, url=url, path=path, timeout=60)
...
def wechat_text_image_handler(wechat_user_id, bucket, xml_data):
    ...
    if msg_type == "image":  # 纯图片
        ...
        path_to_store = get_now_from_bucket(bucket, time_format="/_private/video/%Y/%m/%Y-%m-%d %H-%M-%S.mp4")
        text_to_append = "![%s](%s)" % (msg_id, path_to_store)
    ...
    if msg_type == "voice":
        # 语音也保存一份
        path_to_store = get_now_from_bucket(bucket, time_format="/_private/audio/%Y/%m/%Y-%m-%d %H-%M-%S.amr")
        access_token = get_access_token()
        if access_token:
