...
def render_as_static_file_for_farbox_bucket(path):
    ...
    if not bucket:
        return
    if path.lstrip("/").startswith("_private/") and not is_bucket_login(bucket):
        # 默认会隐藏 _private/ 相关的文件 URL
        site_configs = get_bucket_site_configs(bucket)
        hide_private_file = site_configs.get("hide_private_file", True)
        if hide_private_file:
            return
