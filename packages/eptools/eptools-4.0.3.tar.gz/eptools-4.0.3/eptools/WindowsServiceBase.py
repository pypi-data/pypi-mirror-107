...
class SMWinservice(win32serviceutil.ServiceFramework):
    ...
    def connect_to_db(self,retry=0):
        try:
            ...
            if retry < 4:
                ...
                if self.logger:
                    self.logger.debug("SQL connection error, ocr", e)
                return False
            ...
            try:
                ...
                if status == 'start':
                    ...
                    cursor.execute(qry_start,(
                            ...
                            self.daily, # DECLARE @daily int = ?; 
                            self.looptime, # DECLARE @looptime int = ?; 
                        )
                    )
                if status == 'run':
                ...
                if retry < 4:
                    ...
                    self.log_general(str(ex))

        return False

    def log_general(self, msg: str):
        """ Logs to both file and to slack. """
        if self.slacker:
            self.slacker.log(msg)
        if self.logger:
    ...
    def SvcDoRun(self):
        ...
        self.logger = EasyPostLogger()
        self.slacker = self.logger.slack_logger
