...
class SMWinservice(win32serviceutil.ServiceFramework):
    ...
    finish_query = """
        ...
        END     
        ;
        IF NOT EXISTS(SELECT 1 FROM (SELECT TOP(1) * FROM EasyPostServices.dbo.{table} ORDER BY DateTime DESC) one WHERE  Type = '{status_type}' AND Description = ?)
        BEGIN
    ...
    def connect_to_db(self,retry=0):
        try:
            connection_object = self.config_reader.config_data.easypost_services_connection
            connection_string = connection_object.to_sql_connection_string()
    ...
    def write_to_db(self,status = 'start', options=['FINISH','SUCCESS'],retry=0):
        ...
        if db_conn:
            try:
                ...
                if status == 'run':
                    qry_run = self.run_query.format(description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily,table=self._svc_name_,status=options[0],status_type='RUNNING')
                    print("run_query\n", qry_run)
                    cursor.execute(qry_run)
                if status == 'finish':
                    qry_finish = self.finish_query.format(description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily,table=self._svc_name_,status=options[0],status_type='FINISH')
                    print("finish_query\n", qry_finish)
                    cursor.execute(qry_finish,(options[1],options[1]))
                if status == 'error':
                    qry_error = self.finish_query.format(description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily,table=self._svc_name_,status=options[0],status_type='ERROR')
                    print("error_query\n", qry_error)
                    cursor.execute(qry_error,(options[1],options[1]))
    ...
    def __init__(self, args):
        ...
        # Setup config reader
        self.config_reader = ConfigCredentialsReader()
