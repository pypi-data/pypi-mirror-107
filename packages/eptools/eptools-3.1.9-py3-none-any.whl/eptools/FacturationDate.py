...
# input can be a datetime or a string of formats: "%Y-%m-%d %H:%M:%S.%f" , "%Y-%m-%d %H:%M:%S" , "%Y-%m-%d %H:%M", "%Y-%m-%d" or nothing (then the factdate for current timestamp is calculated)
def generate_dates(inputdate = None, customer_id=None, retry = 0):
    # database connection
    conn = get_conn()
    if conn:
    # string to datetime
