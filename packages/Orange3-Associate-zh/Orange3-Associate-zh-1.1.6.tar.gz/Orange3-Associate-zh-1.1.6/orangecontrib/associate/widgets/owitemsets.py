...
class OWItemsets(widget.OWWidget):
    name = '频繁项集(Frequent Itemsets)'
    description = '探索频繁一起出现的项目.'
    ...
    def __init__(self):
        ...
        self.tree.setHeaderLabels(["项目集合", "支持度", "%"])
        ...
        gui.label(box, self, "项目集合数量: %(nItemsets)s")
        gui.label(box, self, "选中的项目集合: %(nSelectedItemsets)s")
        gui.label(box, self, "选中的样本: %(nSelectedExamples)s")
        hbox = gui.widgetBox(box, orientation='horizontal')
        gui.button(hbox, self, "全部展开", callback=self.tree.expandAll)
        gui.button(hbox, self, "全部收起", callback=self.tree.collapseAll)

        box = gui.widgetBox(self.controlArea, '寻找项目集合')
        gui.valueSlider(
            box, self, 'minSupport',
        ...
        gui.hSlider(box, self, 'maxItemsets', minValue=10000, maxValue=100000, step=10000,
                    label='项目集合最大数量:', labelFormat="%d",
                    callback=lambda: self.find_itemsets())
        self.button = gui.auto_commit(
            box, self, 'autoFind', '寻找项目集合', commit=self.find_itemsets,
            callback=lambda: self.autoFind and self.find_itemsets())

        box = gui.widgetBox(self.controlArea, '筛选项目集合')
        gui.lineEdit(box, self, 'filterKeywords', '包含:',
                     callback=self.filter_change, orientation='horizontal',
                     tooltip='逗号或者空格分割的正则表达式列表.')
        hbox = gui.widgetBox(box, orientation='horizontal')
        gui.spin(hbox, self, 'filterMinItems', 1, 998, label='最小项目数量:',
                 callback=self.filter_change)
        gui.spin(hbox, self, 'filterMaxItems', 2, 999, label='最大项目数量:',
                 callback=self.filter_change)
    ...
    def find_itemsets(self):
        ...
        self._is_running = False
        self.button.button.setText('寻找项目集合')
