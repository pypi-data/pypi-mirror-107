...
class Publish:
    ...
    def run(self, stdin, log):
        ...
        print(sh.git("commit", "-a", "-m", "Bump version in pyproject.toml"))
        print(sh.git("push"))
        version = sh.poetry("version", "-s")
        asset = sh.tail(sh.grep(sh.ls("-tr", "dist"), ".whl"), "-n1")
        print(sh.gh("release", "create", version, f"dists/{asset}"))
        print(sh.git("pull"))
        print("Published v{version}!")
        return 0
