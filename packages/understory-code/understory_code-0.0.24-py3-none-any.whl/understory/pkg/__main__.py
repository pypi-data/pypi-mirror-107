...
class Publish:
    ...
    def run(self, stdin, log):
        try:
            ...
            pass
        sh.poetry("version", self.rule)
        sh.poetry("build")
        sh.poetry("publish")
        sh.git("commit", "-a", "-m", "Bump version in pyproject.toml")
        sh.git("push")
        sh.gh("release", "create", sh.poetry("version", "-s"),
              sh.tail("-n1", sh.grep(".whl", sh.ls("-tr", "dist"))))
        sh.git("pull")
        return 0
