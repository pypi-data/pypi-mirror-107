...
class circle():
    ...
    #    print "inner"
        ...
        yt = Bezier(y, t)
        # print(total_size)
        plot(xt, yt, '-', color=cl, lw=0.3, alpha = 0.1)#alpha 
            ...
            if (row[0] == '\n'):
                ...
                if self.class1:
                    ...
                    if id1.split('^')[0] == id2.split('^')[0] and abs(pos1 - pos2) < (100 * gene_average):
                        data1.append([order1, pos1, self.radius_a, order2, pos2, self.radius_a, 'lime', total_size])
                        # self.plot_bez_inner((order1, pos1, self.radius_a), (order2, pos2, self.radius_a), 'silver', total_size)
                        # pass
                    elif id1.split('^')[0] == id2.split('^')[0] and abs(pos1 - pos2) < (300 * gene_average):
                        data2.append([order1, pos1, self.radius_a, order2, pos2, self.radius_a, 'cyan', total_size])
                        # self.plot_bez_inner((order1, pos1, self.radius_a), (order2, pos2, self.radius_a), 'silver', total_size)
                        # pass
                    else:
                        data3.append([order1, pos1, self.radius_a, order2, pos2, self.radius_a, 'silver', total_size])
                    ...
                    pass
        for i in data3:
            self.plot_bez_inner((i[0],i[1],i[2]),(i[3],i[4],i[5]),i[6],i[7])
        for i in data1:
            self.plot_bez_inner((i[0],i[1],i[2]),(i[3],i[4],i[5]),i[6],i[7])
        for i in data2:
            self.plot_bez_inner((i[0],i[1],i[2]),(i[3],i[4],i[5]),i[6],i[7])
