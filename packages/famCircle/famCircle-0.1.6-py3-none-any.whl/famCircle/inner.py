    ...
    def ksrun(self):
        ...
        for row in genefamily:
            if (row != '\n'):
                ...
                self.genepair0.append(self.readname(familyname))
    ...
    def run(self):
        ...
        #    print genepair
            ...
            if (gene1 not in gene2location.keys()):
                # print(gene1)
                continue
            if gene1 not in self.genepair0:
                continue
            if gene2 not in self.genepair0:
                continue
            ...
            color, class1 = self.run_plot(Ks, start1, start2)
            if (color != None):
                self.plot_bez_Ks2(start1, self.radius_a - (shift1+1) * self.shiftratio * self.blockthick, start2, self.radius_a - (shift2+1) * self.shiftratio * self.blockthick, color, 0.5)
                lt = [gene1, gene2, start1, shift1, start2, shift2, class1]
                with open(self.savecsv, "a", newline='', encoding='utf-8') as file:
                    writer = csv.writer(file ,delimiter=',')
                    writer.writerow(lt)
