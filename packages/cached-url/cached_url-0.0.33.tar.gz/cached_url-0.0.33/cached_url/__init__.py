...
def cachedContent(url, headers={}, mode='', sleep=0, ttl=float('inf')):
...
def get(url, headers={}, force_cache=False, mode='', sleep=0, ttl=0):
    if force_cache or 'test' in str(sys.argv):
        ttl = float('inf')
    if ttl != 0:
        return cachedContent(url, headers, mode, sleep, ttl)
    else:
