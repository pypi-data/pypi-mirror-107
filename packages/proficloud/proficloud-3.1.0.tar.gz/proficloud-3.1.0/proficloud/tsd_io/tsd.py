...
class ProficloudIOMetrics():
    ...
    def queryMetrics(self, uuid, metrics, start_time=None, end_time=None, dropNa=True, chunkSize=20000, groupIntervalMs=200):
        """
        ...
        for metric in metrics:
            ...
            while True:
                ...
                query = "SELECT mean(value) FROM \"{}\" WHERE (\"uuid\" = '{}') AND time >= '{}' AND time < '{}' GROUP BY time({}ms) fill(none) LIMIT {}".format(metric, uuid, start, end, groupIntervalMs, chunkSize)
                if groupIntervalMs == 0:
                    query = "SELECT value FROM \"{}\" WHERE (\"uuid\" = '{}') AND time >= '{}' AND time < '{}' fill(none) LIMIT {}".format(metric, uuid, start, end, chunkSize)
                ...
                data_metric = Enumerable(result.json().get("results")).select(lambda s: s.get("series")).select_many().select(lambda s: s.get("values")).select_many()
