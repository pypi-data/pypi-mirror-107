...
from spins2cpu.mae import p3m1_update
...
def run(file, init, X, Y, J, A, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 2:
        ...
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = p3m1_update.iteration2(latt, X_s, Y_s, Ja, J0, JA, arrays_values[i], nequilibrium, nworks)
                ...
                if arrays_values[i] == 0:
                    if (i+1) < lav:
                        t, Nw, Ew = p3m1_update.iteration2(latt, X_s, Y_s, Ja, J0, JA, arrays_values[i+1], nequilibrium, 0)
                    if (i+2) < lav:
                        t, Nw, Ew = p3m1_update.iteration2(latt, X_s, Y_s, Ja, J0, JA, arrays_values[i+2], nequilibrium, 0)
        elif init == "afm1":
        ...
        if init == "fm":
            ...
            logging.info("{:>16} {:>16} {:>18} {:>16} {:>16}".format(
                "Temperature", "magnetism(z)", "susceptibility(z)", "specific heat", "time(s)"))
            for i in range(lav):
                t, Nw, Ew = p3m1_update.iteration3(latt, X_s, Y_s, Ja, J0, J1, JA, arrays_values[i], nequilibrium, nworks)
                m_ave = functions.Average(Nw)
                s_ave = functions.Average2(Nw)
                susceptibility = arrays_values[i] * num * (s_ave - m_ave ** 2)
                Cv = arrays_values[i] ** 2 * (functions.Average2(Ew) - functions.Average(Ew) ** 2) / num
                ...
                if arrays_values[i] == 0:
                    if (i+1) < lav:
                        t, Nw, Ew = p3m1_update.iteration3(latt, X_s, Y_s, Ja, J0, J1, JA, arrays_values[i+1], nequilibrium, 0)
                    if (i+2) < lav:
                        t, Nw, Ew = p3m1_update.iteration3(latt, X_s, Y_s, Ja, J0, J1, JA, arrays_values[i+2], nequilibrium, 0)
        elif init == "afm1":
            logging.info("{:>16} {:>16} {:>16}".format("Round", "magnetism1", "magnetism2"))
            latt = functions.OnesZNN(2, 4, Y_s, X_s)
            latt[1,:,2] = -1
