...
def run(file, init, X, Y, J, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 1:
        ...
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = honeycomb_update.iteration(latt, X_s, Y_s, Ja, arrays_values[i], nequilibrium, nworks)
            ...
            latt = functions.Onesint4(2, 4, Y_s, X_s)
            latt[1] = -1
            m_ave1 = functions.Average(latt[0])
            m_ave2 = functions.Average(latt[1])
            ...
            for i in range(1,21):
                ...
                t, m_ave1, m_ave2 = honeycomb_update.looping(latt, X_s, Y_s, Ja, val, nequilibrium)
        ...
        if init == "fm":
            ...
            logging.info("{:>16} {:>16} {:>16} {:>16} {:>16}".format("Temperature", "magnetism", "susceptibility", "specific heat", "time(s)"))
            for i in range(lav):
                t, Nw, Ew = honeycomb_update.iteration2(latt, X_s, Y_s, Ja, Jb, arrays_values[i], nequilibrium, nworks)
                m_ave = functions.Average(Nw)
                s_ave = functions.Average2(Nw)
                susceptibility = arrays_values[i] * num * (s_ave - m_ave ** 2)
                Cv = arrays_values[i] ** 2 * (functions.Average2(Ew) - functions.Average(Ew) ** 2) / num
            ...
            for i in range(1,21):
                val = i + np.random.rand() + 1
                t, m_ave1, m_ave2 = honeycomb_update.looping2(latt, X_s, Y_s, Ja, Jb, val, nequilibrium)
                logging.info("{:>16} {:>16.6f} {:>16.6f}{:>16.6f}".format(i, m_ave1, m_ave2, t))
                if abs(m_ave1) > 0.99 and abs(m_ave2) > 0.99:
                    break
            if m_ave1 * m_ave2 > 0.5:
        ...
        if init == "fm":
            ...
            for i in range(1,21):
                val = i + np.random.rand() + 1
                t, m_ave1, m_ave2 = honeycomb_update.looping3(latt, X_s, Y_s, Ja, Jb, Jc, val, nequilibrium)
                logging.info("{:>16} {:>16.6f} {:>16.6f}{:>16.6f}".format(i, m_ave1, m_ave2, t))
                if abs(m_ave1) > 0.99 and abs(m_ave2) > 0.99:
                    break
            if m_ave1 * m_ave2 > 0.5:
