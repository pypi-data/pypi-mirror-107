    ...
    if p == 3:
        ...
        if init == "fm":
            ...
            for i in range(lav):
                ...
                a_arr = np.hstack((arr_a, arr_c))
                b_arr = np.hstack((arr_b, arr_d))
            ...
            latt = functions.Onesint4(4, 4, Y_s, X_s)
            latt[:,[1,2]] = -1
            m_ave1 = functions.Average(latt[:,[0,3]])
            ...
            for i in range(lav):
                t, Nw, Ew = ammm_update.iteration3(latt, X_s, Y_s, Ja, Jb, Jc, arrays_values[i], nequilibrium, nworks)
                a_arr = Nw[:,[0,3,4,7,8,11,12,15]]
                b_arr = Nw[:,[1,2,5,6,9,10,13,14]]
                m_ave1 = functions.Average(a_arr)
                m_ave2 = functions.Average(b_arr)
                s_ave1 = functions.Average2(a_arr)
                s_ave2 = functions.Average2(b_arr)
            ...
            latt[::2,1::2] = -1
            latt[1::2,::2] = -1
            m_ave1 = functions.Average(latt[::2,::2] + latt[1::2,1::2]) / 2
            m_ave2 = functions.Average(latt[::2,1::2] + latt[1::2,::2]) / 2
            logging.info("{:>16} {:>16.6} {:>16.6}".format(0, m_ave1, m_ave2))
            logging.info("{:>16} {:>16} {:>16} {:>16} {:>16} {:>16} {:>16}".format(
                "Temperature", "magnetism1", "magnetism2", "susceptibility1", "susceptibility2", "specific heat", "time(s)"))
            for i in range(lav):
                t, Nw, Ew = ammm_update.iteration3(latt, X_s, Y_s, Ja, Jb, Jc, arrays_values[i], nequilibrium, nworks)
                a_arr = Nw[:,[0,2,5,7,8,10,13,15]]
                b_arr = Nw[:,[1,3,4,6,9,11,12,14]]
                m_ave1 = functions.Average(a_arr)
                m_ave2 = functions.Average(b_arr)
                s_ave1 = functions.Average2(a_arr)
                s_ave2 = functions.Average2(b_arr)
