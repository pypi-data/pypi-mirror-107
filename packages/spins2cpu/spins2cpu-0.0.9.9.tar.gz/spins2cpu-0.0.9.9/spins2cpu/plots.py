...
def plot(arr):
    ...
    for i in range(1, l+1):
        plt.plot(arr[:,0],arr[:,i],linewidth=1,linestyle=':',label="spin%s"%i,marker='o',markersize=2)
    ...
    for i in range(l+1, m-1):
        plt.plot(arr[:,0],arr[:,i],linewidth=1,linestyle=':',label='susceptibility%s'%(i-l),marker='o',markersize=2)
