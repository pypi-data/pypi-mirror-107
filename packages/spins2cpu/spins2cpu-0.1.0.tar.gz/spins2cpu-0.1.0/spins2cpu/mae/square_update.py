    ...
    for k in range(4):
        for j in range(Y_s):
            for i in range(X_s):
                ...
                energy0 = -Ja * ( latt[k,0,j,i] * ( latt[kx,0,j,x_ipp] + latt[kx,0,j,x_inn] + latt[ky,0,y_jpp,i] + latt[ky,0,y_jnn,i] ) +
                                  latt[k,1,j,i] * ( latt[kx,1,j,x_ipp] + latt[kx,1,j,x_inn] + latt[ky,1,y_jpp,i] + latt[ky,1,y_jnn,i] ) +
                                  latt[k,2,j,i] * ( latt[kx,2,j,x_ipp] + latt[kx,2,j,x_inn] + latt[ky,2,y_jpp,i] + latt[ky,2,y_jnn,i] ) )
                ez = -JA * latt[k,2,j,i] ** 2
                Erandn = ( -Ja * ( randvaln[k,0,j,i] * ( latt[kx,0,j,x_ipp] + latt[kx,0,j,x_inn] + latt[ky,0,y_jpp,i] + latt[ky,0,y_jnn,i] ) +
                                   randvaln[k,1,j,i] * ( latt[kx,1,j,x_ipp] + latt[kx,1,j,x_inn] + latt[ky,1,y_jpp,i] + latt[ky,1,y_jnn,i] ) +
                                   randvaln[k,2,j,i] * ( latt[kx,2,j,x_ipp] + latt[kx,2,j,x_inn] + latt[ky,2,y_jpp,i] + latt[ky,2,y_jnn,i] ) ) -
