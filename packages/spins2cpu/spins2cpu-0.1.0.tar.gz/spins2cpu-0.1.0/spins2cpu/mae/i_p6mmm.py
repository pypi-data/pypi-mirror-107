...
from spins2cpu.mae import i_p6mmm_update
...
def run(file, init, X, Y, J, A, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 2:
        ...
        X_s = X // 2
        Y_s = Y // 2
        logging.info("{} {:<8} {} {} + {:<8} {} {} {} {:<8} {} {} {}".format("init:", init, "iterations:", nequilibrium, nworks, "parameters:", Ja, J0, "(meV)",
            "anisotropy:", JA, "(meV)"))
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = i_p6mmm_update.iteration2(latt, X_s, Y_s, Ja, J0, JA, arrays_values[i], nequilibrium, nworks)
                ...
                if arrays_values[i] == 0:
                    if (i+1) < lav:
                        t, Nw, Ew = i_p6mmm_update.iteration2(latt, X_s, Y_s, Ja, J0, JA, arrays_values[i+1], nequilibrium, 0)
                    if (i+2) < lav:
                        t, Nw, Ew = i_p6mmm_update.iteration2(latt, X_s, Y_s, Ja, J0, JA, arrays_values[i+2], nequilibrium, 0)
        elif init == "afm1":
            logging.info("{:>16} {:>16} {:>16}".format("Round", "magnetism1", "magnetism2"))
            latt = functions.OnesZNN(3, 4, Y_s, X_s)
            latt[1,:,2] = -1
            m_ave1 = functions.Average(latt[[0,2],:,2])
            m_ave2 = functions.Average(latt[1,:,2])
            ...
            m_ave2 = functions.Average(latt[:,2:,2])
            logging.info("{:>16} {:>16.6} {:>16.6}".format(0, m_ave1, m_ave2))
            logging.info("{:>16} {:>16} {:>16} {:>18} {:>18} {:>16} {:>16}".format(
                "Temperature", "magnetism1(z)", "magnetism2(z)", "susceptibility1(z)", "susceptibility2(z)", "specific heat", "time(s)"))
            for i in range(lav):
            ...
            latt[1,[0,1],2] = -1
            latt[2,[2,3],2] = -1
            m_ave1 = functions.Average(latt[0,[0,1],2] + latt[1,[2,3],2] + latt[2,[0,1],2]) / 3.0
            m_ave2 = functions.Average(latt[0,[2,3],2] + latt[1,[0,1],2] + latt[2,[2,3],2]) / 3.0
            logging.info("{:>16} {:>16.6} {:>16.6}".format(0, m_ave1, m_ave2))
            logging.info("{:>16} {:>16} {:>16} {:>18} {:>18} {:>16} {:>16}".format(
                "Temperature", "magnetism1(z)", "magnetism2(z)", "susceptibility1(z)", "susceptibility2(z)", "specific heat", "time(s)"))
            for i in range(lav):
        ...
        X_s = X // 2
        Y_s = Y // 2
        logging.info("{} {:<8} {} {} + {:<8} {} {} {} {} {:<8} {} {} {}".format("init:", init, "iterations:", nequilibrium, nworks, "parameters:", Ja, J0, J1,
            "(meV)", "anisotropy:", JA, "(meV)"))
        if init == "fm":
            logging.info("{:>16} {:>16}".format("Round", "magnetism"))
            latt = functions.OnesZNN(3, 4, Y_s, X_s)
            m_ave = functions.Average(latt[:,:,2])
            ...
            logging.info("{:>16} {:>16} {:>18} {:>16} {:>16}".format(
                "Temperature", "magnetism(z)", "susceptibility(z)", "specific heat", "time(s)"))
            for i in range(lav):
                t, Nw, Ew = i_p6mmm_update.iteration3(latt, X_s, Y_s, Ja, J0, J1, JA, arrays_values[i], nequilibrium, nworks)
                m_ave = functions.Average(Nw)
                s_ave = functions.Average2(Nw)
                susceptibility = arrays_values[i] * num * (s_ave - m_ave ** 2)
                Cv = arrays_values[i] ** 2 * (functions.Average2(Ew) - functions.Average(Ew) ** 2) / num
                ...
                if arrays_values[i] == 0:
                    if (i+1) < lav:
                        t, Nw, Ew = i_p6mmm_update.iteration3(latt, X_s, Y_s, Ja, J0, J1, JA, arrays_values[i+1], nequilibrium, 0)
                    if (i+2) < lav:
                        t, Nw, Ew = i_p6mmm_update.iteration3(latt, X_s, Y_s, Ja, J0, J1, JA, arrays_values[i+2], nequilibrium, 0)
        elif init == "afm1":
            logging.info("{:>16} {:>16} {:>16}".format("Round", "magnetism1", "magnetism2"))
            latt = functions.OnesZNN(3, 4, Y_s, X_s)
            latt[1,:,2] = -1
