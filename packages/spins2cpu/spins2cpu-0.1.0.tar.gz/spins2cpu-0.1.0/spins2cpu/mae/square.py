...
def run(file, init, X, Y, J, A, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 1:
        ...
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = square_update.iteration(latt, X_s, Y_s, Ja, JA, arrays_values[i], nequilibrium, nworks)
                ...
                if arrays_values[i] == 0:
                    if (i+1) < lav:
                        t, Nw, Ew = square_update.iteration(latt, X_s, Y_s, Ja, JA, arrays_values[i+1], nequilibrium, 0)
                    if (i+2) < lav:
                        t, Nw, Ew = square_update.iteration(latt, X_s, Y_s, Ja, JA, arrays_values[i+2], nequilibrium, 0)
        elif init == "afm1":
            logging.info("{:>16} {:>16}".format("Round", "magnetism(z)"))
            latt = functions.OnesZN(4, Y_s, X_s)
            latt[[1,3],2] = -1
