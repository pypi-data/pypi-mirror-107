...
class RecursiveFeatureSelector:
    def __init__(self):
        self.best_com = {} # keys:n_trial, val: best_subset
        self.best_score = {} # keys:n_trial, val: best_score
    ...
    def trial(self, estimator, X, y, cv, scoring, max_round=None, fail_tolerance=None, least_gain=None, max_feats=None, start_from=None, pool=None, n_jobs=-1, n_digit=4):
        ...
        while True:
            ...
            if last_key != 'Trial 0':
                if least_gain == None:
                    if self.best_score[curr_key] < self.best_score[last_key]:  # if fail to improve score, then take away one chance
