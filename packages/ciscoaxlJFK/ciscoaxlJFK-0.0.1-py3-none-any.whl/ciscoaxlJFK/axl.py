# Custom version of ciscoaxl -> https://pypi.org/project/ciscoaxl/
...
class axl(object):
    ...
    def getNumberDetails(self, phoneNumber):
        query = "select pkid,dnorpattern,description,fkroutepartition from numplan np where np.dnorpattern='" + phoneNumber + "'"
        return self.sql_query(query = query)

    def getNumberFromUser(self, userID):
        query = """select np.pkid,userid,np.dnorpattern from numplan np, devicenumplanmap dnmp, device d,enduserdevicemap edm, enduser e
                ...
                UPPER(userid)=UPPER('""" + userID + """') GROUP BY 1,2,3"""
        return self.sql_query(query = query)

    def deletePrimaryNumberassignment(self, userID):
        query="""delete from endusernumplanmap where fkenduser=(Select pkid from enduser where UPPER(userid)=UPPER('"""+userID+"""')) and tkdnusage=1"""
        return self.sql_update(query=query)

    def createAllDevices(self, templateName, deviceName, userID):
        #creates 5 Devices from a Template
        query="""insert into device (name, description, tkmodel, tkdeviceprotocol, tkprotocolside, fkdevicepool, fkphonetemplate, fkcallingsearchspace, tkclass, fkprocessnode, 
            ...
            select UPPER(REPLACE(name, '"""+templateName+"""')), SUBSTRING('"""+deviceName+"""' FROM 1 FOR 50), tkmodel, tkdeviceprotocol, tkprotocolside, fkdevicepool, fkphonetemplate, fkcallingsearchspace, 
            CASE WHEN tkclass='252' then '254' ELSE '1' END, fkprocessnode, defaultdtmfcapability, fklocation, tkproduct, (Select pkid from enduser where userid='"""+userID+"""'), 
            ...
            fkwifihotspotprofile from device d where name like '%"""+templateName+"""%' and tkclass in ('253', '252')"""
        return self.sql_update(query=query)
