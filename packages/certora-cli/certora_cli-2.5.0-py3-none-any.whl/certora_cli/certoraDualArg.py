...
def __check_bool_arg_and_setting_consistency(args: argparse.Namespace, arg_name: str, setting_name: str) -> None:
    """
    We accept two syntaxes for settings: --rule or --settings -rule.
    This function checks boolean setting, that can either appear, or not.
...
def check_arg_and_setting_consistency(args: argparse.Namespace) -> None:
    ...
    for (argument, setting) in bool_arg_to_setting.items():
        __check_bool_arg_and_setting_consistency(args, argument, setting)
