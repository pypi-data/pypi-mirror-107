...
class RPCServer(object):
    ...
    async def __call_worker(self, websocket, rpc_id, method, **params) -> None:
        ...
        remote_address = "%s:%s" % (str(websocket.remote_address[0]), str(websocket.remote_address[1]))
    ...
    async def __on_new_onnection(self, websocket: WebSocketServerProtocol,
        ...
        loggers.get(LOGGER_NAME).info("Client(%s:%s) connected." % (str(ip), str(port)))
        ...
        if method:
            ...
            if isinstance(rpc_playload, Exception):
                ...
                if "code" in error_str:
                    ...
                    rpc_playload = {"code": -32000, "message": "%s" % error_str}
