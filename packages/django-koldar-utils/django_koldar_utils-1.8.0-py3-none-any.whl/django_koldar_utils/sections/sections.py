...
from networkx.drawing.tests.test_agraph import pygraphviz
...
def setup_dependency_graph(sections: List[AbstractDjangoSection], filename: str = None) -> nx.DiGraph:
    ...
    if filename is not None:
        dotgraph: pygraphviz.AGraph = nx.nx_agraph.to_agraph(SECTION_DEPENDENCY_GRAPH)
        dotgraph.layout(prog="dot")
        dotgraph.draw(filename)
...
def get_ordered_sections() -> Iterable["AbstractDjangoSection"]:
    ...
    """
    return list(
        map(lambda section_name: SECTION_DEPENDENCY_GRAPH[section_name]["section"],
        reversed(list(nx.topological_sort(SECTION_DEPENDENCY_GRAPH))))
    )
