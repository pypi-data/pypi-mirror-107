...
class DriftEvaluation(AIOSBaseJob):
    ...
    def evaluate(
        ...
        if not drift_database or not drift_table:
            ...
            drifted_records_count = payload_spark_df.count()

            logger.info("Total number of drifted records in this run: {}".format(drifted_records_count))
            if drifted_records_count > 0:
