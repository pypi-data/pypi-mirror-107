...
class PortfolioDataApi(metaclass=Singleton):
    ...
    def get_asset_price(self, begin_date, end_date, time_para, asset_list: Tuple[str] = ()):
        try:
            ...
            if len(_asset_list_3) > 0:
                ...
                _df = derived.get_fof_nav_public_adj(fund_ids=real_ids,start_date=begin_date,end_date=end_date).pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
    ...
    def get_asset_price_dt(self, asset_list, begin_date, end_date):
        try:
            ...
            if len(_asset_list_3) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_3]
                _df = derived.get_fof_nav_public_adj(fund_ids=real_ids,start_date=_begin_date,end_date=end_date).pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
            if len(_asset_list_4) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_4]
    ...
    def get_product_price(self, begin_date, end_date, time_para, product_list: Tuple[str] = (), price_type:str='价格线', benchmark_id:str='hs300'):#'价格比线'
        try:
            ...
            _df = derived.get_fof_nav_public_adj(fund_ids=real_ids,start_date=begin_date,end_date=end_date)
            if not _df.empty > 0:
    ...
    def product_recent_rate(self, product_list, year:int=None, weekly:bool=False):
        try:
            ...
            result = []
            real_ids = product_list
            _df = derived.get_fof_nav_public_adj(fund_ids=real_ids,start_date='20091231')
            if not _df.empty > 0:
                _df = _df.pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
    ...
    def asset_recent_rate(self, asset_list, year:int=None, weekly:bool=False):
        ...
        try:
            ...
            if len(_asset_list_3) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_3]
                _df = derived.get_fof_nav_public_adj(fund_ids=real_ids,start_date='20091231').pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
            if len(_asset_list_4) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_4]
    ...
    def get_all_asset_price(self, start_date:str='', end_date:str='', id_list:list=[]):
        try:
            ...
            hf_fund_nav = derived.get_fof_nav_public_adj(fund_ids=id_list, start_date=begin_date, end_date=end_date)
