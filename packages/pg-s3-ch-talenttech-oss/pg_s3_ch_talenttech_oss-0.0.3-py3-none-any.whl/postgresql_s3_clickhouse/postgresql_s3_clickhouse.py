...
class PGS3CH:
    ...
    def create_temporary_table(self):
        ...
        sql = """CREATE TABLE {staging_database}.{temp_table_prefix}{table} AS {database}.{table} 
                    ENGINE=MergeTree() 
                    PARTITION BY tuple() 
    ...
    def drop_temporary_table(self):
        sql = """DROP TABLE IF EXISTS {staging_database}.{temp_table_prefix}{table}""".format(staging_database=self.entity['ch']['staging_database'],
                                                                                              temp_table_prefix=self.temp_table_prefix,
                                                                                              table=self.entity['ch']['table'])
        self.ch_client.execute(sql)
        logging.info(sql)
    ...
    def optimize(self):
        ...
        for row in result:
            ...
            sql_optimize = """OPTIMIZE TABLE {database}.{table} PARTITION {partition} """.format(database=self.entity['ch']['database'], table=self.entity['ch']['table'], partition=row[0])
    ...
    def exchange_temp_to_prod(self):
        ...
        sql = """EXCHANGE TABLES {staging_database}.{temp_table_prefix}{table} AND {database}.{table}""".format(staging_database=self.entity['ch']['staging_database'],
                                                                                                                temp_table_prefix=self.temp_table_prefix,
                                                                                                                database=self.entity['ch']['database'],
                                                                                                                table=self.entity['ch']['table'])
