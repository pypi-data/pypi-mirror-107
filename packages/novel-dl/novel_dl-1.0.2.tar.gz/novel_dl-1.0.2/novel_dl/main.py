...
def get_data(url):
	...
	if int(data.status_code/100)==5:
		return raise_error("network error (5xx)")
	elif int(data.status_code/100)==4:
		return raise_error("novel not found")
	return data.content
...
def main(args):
	...
	if args["short"] and not args["episode"]:
		return raise_error("invalid option -- 's'\nThe -s,--short argument requires -e,--episode")
	if not args["theme"] in THEMES:
		return raise_error('invalid theme name `'+args["theme"]+'`')

	#==CHECK-url==
	ret=urllib.parse.urlparse(args["url"])
	if not ret.hostname:
		return raise_error("invalid argument 'url'")
	...
	if re.match(r'.*syosetu.com',ret.hostname):
		...
		return raise_error("novel-dl: url is not supported")
	if args["axel"]:
		delay=0
	else:
		delay=1
	...
	if conf.get('parent'):
		...
		for file in htmls:
			if os.path.isfile(THEME_DIR+htmls[file]):
				...
				return raise_error("cannot load theme file: "+htmls[file])
	...
	if args["episode"]:
		if not re.match(r'^\d*$',args["episode"]):
			return raise_error("incorrect episode number `"+args["episode"]+"`")
