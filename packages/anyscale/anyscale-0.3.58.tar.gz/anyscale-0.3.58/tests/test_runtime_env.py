...
import anyscale
from anyscale.utils.conda import get_conda_activate_commands
...
def test_client_setver_env_prep_preinstalled() -> None:
    job_config = ray.job_config.JobConfig(runtime_env={"conda": "test_env"})
    new_job_config = ray_client_server_env_prep(job_config)
    assert job_config.runtime_env == new_job_config.runtime_env
...
opentelemetry-sdk==1.0.0rc1""",
        ...
        def prep(runtime_env: Dict[str, Any]) -> ray.job_config.JobConfig:
            job_config = ray.job_config.JobConfig(
                runtime_env=copy.deepcopy(input_runtime_env)
            )
            new_job_config = ray_client_server_env_prep(job_config)
            assert "uris" in new_job_config.runtime_env
            resource_dir_pkg, base_pkg = _get_conda_packages(
                new_job_config.runtime_env["uris"]
            )

            conda_env = new_job_config.runtime_env.get("conda")
            pip_env = new_job_config.runtime_env.get("pip")
        ...
        new_job_config = prep(input_runtime_env)
        runtime_env = new_job_config.runtime_env
        assert "uris" in runtime_env
        resource_dir_pkg, base_pkg = _get_conda_packages(runtime_env["uris"])
        assert resource_dir_pkg is not None
        assert base_pkg is not None
        # conda pkg created
        assert resource_dir_pkg.local_path().is_dir()
        with pytest.raises(ModuleNotFoundError):
            # Ensure pip-install-test is not installed on the test machine
            import pip_install_test  # noqa
        cmds = get_conda_activate_commands(resource_dir_pkg.local_path())
        ...
        ray.job_config.JobConfig(runtime_env=copy.deepcopy(input_runtime_env))
        prep(input_runtime_env)
        assert old_get_cnt == meta.get_cnt
        assert old_put_cnt == meta.put_cnt

        # Delete it and set it up from caching
        shutil.rmtree(resource_dir_pkg.local_path())
        old_get_cnt, old_put_cnt = meta.get_cnt, meta.put_cnt
        prep(input_runtime_env)
        assert old_put_cnt == meta.put_cnt
        assert old_get_cnt + 1 == meta.get_cnt
