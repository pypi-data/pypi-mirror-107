...
from ..modules.modules import TransformerEncoder, TransformerDecoder, TanhHead
from ..utils import transposed_cat


class BartModel(Module):
    def __init__(
            self, vocab_size: int, padding_idx: int, eos_token_id: int, hidden_size: int, max_position_embeddings: int,
            encoder_layers: int, encoder_heads: int, encoder_ffn_dim: int,
            decoder_layers: int, decoder_heads: int, decoder_ffn_dim: int
    ):
        super().__init__()

        self.embeddings = Embedding(vocab_size, hidden_size, padding_idx)

        self.encoder_positions = Embedding(
            num_embeddings=max_position_embeddings, embedding_dim=hidden_size, padding_idx=padding_idx)

        self.encoder_norm = LayerNorm(hidden_size)

        self.encoder = TransformerEncoder(
            n_heads=encoder_heads, n_layers=encoder_layers, embedding_size=hidden_size, ffn_size=encoder_ffn_dim)

        self.pre_decoder_layer = Linear(hidden_size, 2 * hidden_size * decoder_layers)
        ...
        self.decoder = TransformerDecoder(
            n_heads=decoder_heads, n_layers=decoder_layers, embedding_size=hidden_size, ffn_size=decoder_ffn_dim)
...
class BartForSequenceClassification(Module):
    def __init__(
            self, vocab_size: int, padding_idx: int, hidden_size: int, max_position_embeddings: int,
            encoder_layers: int, encoder_heads: int, encoder_ffn_dim: int,
            decoder_layers: int, decoder_heads: int, decoder_ffn_dim: int, eos_token_id: int, num_labels: int
    ):
        super().__init__()
        self.transformer = BartModel(
            vocab_size=vocab_size, padding_idx=padding_idx, eos_token_id=eos_token_id, hidden_size=hidden_size,
        ...
        self.classification_head = TanhHead(hidden_size, hidden_size, num_labels)
    ...
    def from_pretrained(cls, model_path: str) -> 'BartForSequenceClassification':
        model = cls(
            vocab_size=50265, padding_idx=1, hidden_size=1024, max_position_embeddings=1024,
