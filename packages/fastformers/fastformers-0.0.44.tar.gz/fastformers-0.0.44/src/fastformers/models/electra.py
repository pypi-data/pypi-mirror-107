...
class ElectraModel(Module):
    def __init__(
            self, vocab_size: int, token_types: int, hidden_size: int, pad_token_id: int,
            max_position_embeddings: int, ffn_size: int, n_heads: int, n_layers: int
    ):
        super().__init__()
        self.embeddings = EmbeddingsWithTokenTypes(
            vocab_size=vocab_size, token_types=token_types, embedding_size=hidden_size, pad_token_id=pad_token_id,
...
class ElectraForSequenceClassification(Module):
    def __init__(
        ...
        self.transformer = ElectraModel(
            vocab_size=vocab_size, token_types=token_types, hidden_size=hidden_size, pad_token_id=pad_token_id,
            max_position_embeddings=max_position_embeddings, ffn_size=ffn_size, n_heads=n_heads, n_layers=n_layers)
        self.classifier = ElectraClassificationHead(hidden_size, num_labels)
    ...
    def from_pretrained(cls, model_path: str, num_labels: int) -> 'ElectraForSequenceClassification':
        model = cls(
            vocab_size=30522, token_types=2, hidden_size=768, pad_token_id=0, max_position_embeddings=512,
