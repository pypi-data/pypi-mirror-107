...
class ClientAPI:
    ...
    def forward(self, **inputs):
        logger.info('Inside client forward')
        ...
        logger.info('Sockets acquired')

        for client_socket in select.select(client_sockets, [], [], 0)[0]:
            client_socket.empty()

        logger.info('Sockets cleared')
        ...
        logger.info(f'Num batches: {len(batched_inputs)}')
        ...
        while received_counter < len(outputs):
            ...
            for client_socket in output_sockets:
                ...
                received_counter += 1
                if batched_inputs:
                    logger.info('Requesting server again')
                    client_socket.request_server()

        logger.info('Stacking outputs')
