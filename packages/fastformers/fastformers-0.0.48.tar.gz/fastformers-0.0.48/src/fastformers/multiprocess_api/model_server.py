...
class SessionState:
    ...
    def send_error(self, e: Exception):
        try:
            self.connection.send_error(self.message_id, Config.error_prefix + str(e).encode('utf-8'))
        except ConnectionError:
            pass
...
class ModelServer:
    ...
    def split_dict_output(
        ...
        outputs = {key: value.to('cpu') if isinstance(value, torch.Tensor) else value for key, value in outputs.items()}
        last_ind = 0
        for size in sizes:
            new_ind = last_ind + size
