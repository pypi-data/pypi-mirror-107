...
class Caste(CASTE_HELPER):
    ...
    def get_caste(self, n=1):
        return self._list_to_title(random.sample(self.GET_CASTE_LIST(), n))

    def get_caste_position(self, str_l):
        caste_in = self.detect_caste(str_l)
        caste_in_list = []
        for position, name in caste_in:
    ...
    def split_name(self, str_name):
        ...
        caste_index = self.get_caste_position(str_name)
        ...
        if total_caste_found <= 1:
            ...
            for _, index in enumerate(caste_index):
                if index + 1 < len(raw_name) and self.is_caste(raw_name[index]) and self.is_caste(raw_name[index + 1]):
                    pass
                else:
