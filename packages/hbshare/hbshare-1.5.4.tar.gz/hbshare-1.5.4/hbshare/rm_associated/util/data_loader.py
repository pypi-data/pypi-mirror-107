...
class SectorIndexCalculatorLoader:
    ...
    def load(self):
        ...
        sql_script = "SELECT SECODE, INDEXNAME FROM finchina.TQ_IX_BASICINFO " \
                     "WHERE INDEXNAME LIKE '%申万%' and CREATINDEXORGCODE = '80000856' and " \
                     "ESTCLASS LIKE '%一级行业%' and MCSTATUS in ('1', '2')"
        post_body = self.post_body
        post_body['sql'] = sql_script
        res = requests.post(url=self.url, data=json.dumps(post_body), headers=self.headers).json()
        data = pd.DataFrame(res['data'])
        # TQ_QT_INDEX version
        industry_code_list = sorted(data['SECODE'].unique())
        industry_index = []
        post_body = self.post_body
        for secode in industry_code_list:
            sql_script = "SELECT TRADEDATE, SECODE, INDEXNAME, TCLOSE, NEGOTIABLEMV as NEG_MKV " \
                         "FROM finchina.TQ_QT_INDEX WHERE " \
                         "SECODE = '{}' and TRADEDATE >= {} and TRADEDATE <= {} " \
                         "ORDER BY TRADEDATE".format(secode, pre_date, self.end_date)
            post_body['sql'] = sql_script
            res = requests.post(url=self.url, data=json.dumps(post_body), headers=self.headers).json()
            industry_index.append(pd.DataFrame(res['data']))
        industry_index = pd.concat(industry_index)
        # funddb.ZSJY version, 后续需要切换到这个
