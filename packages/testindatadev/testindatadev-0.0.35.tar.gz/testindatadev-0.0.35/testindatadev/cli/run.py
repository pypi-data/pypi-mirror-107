...
def _noLoginMessage():
    click.echo("  you have not set your account, use:")
    click.echo("\n")
    click.echo("  tda config <access_key> [host] to set your account")
    click.echo("\n")
...
def main(ctx, access_key, host, debug):
    """ testin dataset management tool  """
    info = {
...
@click.option("-save", "--saveDir", 'save_dir', default="", help="save path for downloading files")
@click.pass_context
def download(ctx, ds_id, save_dir):
    ...
    while True:
        ...
        for file in fileData["files"]:
            ...
            with open(filePath + "_label.json", "w", encoding="utf-8") as jf:
                json.dump(labelData.anotations.labels, jf)
                if ctx.obj.debug:
                    print(f"[SAVE_LABEL]file total: {fileTotal}, save label data: [{filePath + '_label.json'}]")
