...
def empyrial(returns, benchmark, rf=0.0, confidence_value=0.95, sigma_value=1):
  ...
  win = sum(condition(x) for x in returns.iloc[:,0])
  total = len(returns.iloc[:,0])
  win_ratio = win/total
  win_ratio = win_ratio*100
  win_ratio = round(win_ratio,2)

  IR = information_ratio(returns.iloc[:,0], benchmark.iloc[:,0])
  IR = round(IR,2)


  ret = returns[returns.columns[0]]
  benchm = benchmark[benchmark.columns[0]]
  ...
  data = {'':['Annual return', 'Cumulative return', 'Annual volatility','Winning day ratio', 'Sharpe ratio','Calmar ratio', 'Information ratio', 'Stability', 'Max Drawdown','Sortino ratio','Skew', 'Kurtosis', 'Tail Ratio', 'Common sense ratio', 'Daily value at risk',
        ...
        'Backtest':[f'{CAGR}%', CUM, VOL,f'{win_ratio}%', SR, CR, IR, STABILITY, MD, SOR, SK, KU, TA, CSR, VAR, AL, BTA]}
  ...
  display(df)
  return qs.plots.returns(returns,benchmark, cumulative=True), qs.plots.monthly_heatmap(ret), qs.plots.drawdown(ret), qs.plots.drawdowns_periods(ret), qs.plots.rolling_volatility(ret), qs.plots.rolling_sharpe(ret), qs.plots.rolling_beta(ret, benchm)
