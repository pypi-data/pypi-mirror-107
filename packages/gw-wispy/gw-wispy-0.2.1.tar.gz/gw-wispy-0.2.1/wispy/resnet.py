...
def identity_block(input_tensor, units, batch_norm):
...
def dens_block(input_tensor, units, batch_norm):
...
def ResNet(input_shape, output_shape, width=64, num_blocks=3, batch_norm=True):
    ...
    x = dens_block(Res_input, width, batch_norm=batch_norm)
    x = identity_block(x, width, batch_norm=batch_norm)
    x = identity_block(x, width, batch_norm=batch_norm)

    if num_blocks > 1:
        for i in range(1, num_blocks):
            x = dens_block(x, width, batch_norm=batch_norm)
            x = identity_block(x, width, batch_norm=batch_norm)
            x = identity_block(x, width, batch_norm=batch_norm)
