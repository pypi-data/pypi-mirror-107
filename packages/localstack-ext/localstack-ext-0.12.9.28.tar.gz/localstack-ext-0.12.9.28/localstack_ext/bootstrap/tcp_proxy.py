...
class AttrDict(OdvQC):
 def __init__(self,*args,**kwargs):
  OdvQY(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and OdvQo(socket,'IPV6_V6ONLY'):
...
class Half(OdvQI):
 def __init__(self,opt,sock,addr,OdvQH):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.OdvQH=OdvQH
  self.name='peer' if self.OdvQH else 'client'
  self.queue=[]
  self.dest=OdvQE
  self.err=OdvQE
  self.ready=OdvQN
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':OdvQN,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':OdvQN,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':OdvQN,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':OdvQN,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
