...
class TestMaterialize(unittest.TestCase):
    ...
    ###
    def test_online_join_overwrite_fails(self):
        """
        Changing online join fails.
        """
        ...
        result = self._invoke_cli(JOIN_FOLDER_NAME)
        assert result.exit_code == 0
        # Assert joins are not overwritten.
        assert f"cannot overwrite {JOIN_CLASS_NAME} {BASE_JOIN_NAME} with existing online conf" in result.output

    ###
    # Test overwriting protected entities.
    ###
    def test_online_group_by_overwrite_fails(self):
        """
        Changing online group_by fails.
        """
        # First materialize online join.
        self.setup_group_by_and_join(
            group_by_online=True,
        ...
        result = self._invoke_cli(GROUP_BY_FOLDER_NAME)
        assert result.exit_code == 0
        # Assert group bys are not overwritten.
        assert f"cannot overwrite {GROUP_BY_CLASS_NAME} {BASE_GROUP_BY_NAME} with existing online conf" in result.output

    def test_change_online_group_by_to_prod_works(self):
        """
        Changing online group by to production works.
        """
        ...
        # Change the group by back to to non-production.
        self.setup_group_by_and_join(
            group_by_online=True,
            group_by_production=False,
            join_online=None,
    ...
    def test_change_online_join_to_prod_works(self):
        """
        Changing online join to production works.
        """
        # First materialize online join.
        self.setup_group_by_and_join(
            group_by_online=None,
        ...
        # Change the join back to to non-production.
        self.setup_group_by_and_join(
            group_by_online=None,
            group_by_production=None,
            join_online=True,
