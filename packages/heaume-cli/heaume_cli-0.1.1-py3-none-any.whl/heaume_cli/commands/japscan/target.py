...
def target(email, type, add, delete):
    ...
    if not add and not delete:
        response = requests.get(f"{endpoint}/{route}?email={email}")
    else:
        if add:
            response = requests.post(f"{endpoint}/{route}?email={email}", json={"targets": list(add)})
        if delete:
            response = requests.delete(f"{endpoint}/{route}?email={email}", json={"targets": list(delete)})        
