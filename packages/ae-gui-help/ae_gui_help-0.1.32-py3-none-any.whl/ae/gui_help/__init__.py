...
class TourBase:
    ...
    """
    auto_switch_pages: Union[bool, int] = False
    """ switch automatically tour pages within the seconds specified by :attr:`.page_data`['next_page_delay'].

    set to `True`, `1` or `-1` for to automatically switch tour pages; `True` and `1` will switch to the next page until
    the last page is reached, while `-1` will switch back to the previous pages until the first page is reached; `-1`
    and `1` automatically toggles at the first/last page the to other value (endless ping-pong until back/next button
    gets pressed by the user).
    """
    ...
    * `next_page_delay`: time in seconds to read the current tour page before next request_auto_page_switch() (def=9.6).
    * `tip_text` or (empty string): tour page tooltip text fstring message text template.
    ...
    def __init__(self, main_app: 'HelpAppBase'):
        ...
        self.last_page_idx: Optional[int] = None
    ...
    def stop(self):
        ...
        self.cancel_auto_page_switch_request()
        self.teardown_page()
...
class OnboardingTour(TourBase):
    ...
    page_ids = ['', 'page_switching', 'responsible_layout', 'tip_help_intro', 'tip_help_tooltip', 'layout_font_size',
