...
For to activate the help mode the widget to display the help texts have to be assigned to the main app attribute
...
class TourBase:
    ...
    def __init__(self, main_app: 'HelpAppBase'):
        ...
        set to `True`, `1` or `-1` for to automatically switch tour pages; `True` and `1` will switch to the next page
        until the last page is reached, while `-1` will switch back to the previous pages until the first page is
        reached;
        `-1` and `1` automatically toggles at the first/last page the to other value (endless ping-pong until back/next
        button gets pressed by the user).
        """
    ...
    def setup_app_flow(self):
        """ setup app flow and load page data for to prepare a tour page. """
