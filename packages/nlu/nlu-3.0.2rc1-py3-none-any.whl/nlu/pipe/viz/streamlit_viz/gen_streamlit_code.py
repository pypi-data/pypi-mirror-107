...
def get_code_for_viz(viz_type:str,models:str,data:Union[str,List[str]],max_len:int=100)->str:
    """Generate code sample for displaying how to generate visualization"""
    models = f"'{models}'"
    if viz_type =='SIMILARITY':
        if len(data) > 2 and isinstance(data[0],List) and isinstance(data[0],List):
            data = f"['{data[0][:max_len]}', '{data[1][:max_len]}']"
