...
class ModelVersion:
    ...
    def _handle_local_notebook_trainer(self):
        ...
        notebook_path = (
            f"/workspace/models/{self.model_name}/{self.model_version_id}/ipynb.py"
        )

        return_code = _subprocess.call(
            f"/workspace/env/{self.model_name}/bin/python -m jupyter nbconvert {notebook_path} --to=python",  # noqa
            shell=True,
        )
