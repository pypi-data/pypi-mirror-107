...
def _offsetLinks(network, offset):
    ...
    for link_id, link in network.link_dict.items():
        if link.from_bidirectional_way:
            ...
            geometry_xy = link.geometry_xy.parallel_offset(distance=2, side=side, join_style=2)
            if offset == 1:
...
def _parseWays(network, link_type, offset, network_type, POIs, POI_percentage):
    if link_type != 'all':
        if isinstance(network_type, str):
    ...
    for osm_way_id, way in network.osm_way_dict.items():
        if way.building or way.amenity:
            ...
            try:
                ...
                if link_type_ != 'all' and way.link_type_name not in link_type_:
                    continue
            except KeyError:
    ...
    if offset: _offsetLinks(network, offset)

    if POIs:
        POI_list = generatePOIs(POI_way_list, network, POI_percentage)
        network.POI_list = POI_list


def _parseOSM(network, link_type, offset, network_type, POIs, POI_percentage):
    _parseWays(network, link_type, offset, network_type, POIs, POI_percentage)


def _buildNet(network,network_type_valid, link_type, POIs, POI_percentage, offset, min_nodes, combine, int_buffer, default_lanes, default_speed):

    _updateDefaultLaneSpeed(default_lanes, default_speed, network)

    _parseOSM(network, link_type, offset, network_type_valid, POIs, POI_percentage)
...
def getNetFromOSMFile(osm_filename='map.osm', network_type=('auto',), link_type='all', POIs=False, POI_percentage=1.0,
                      strict_mode=True, offset=0, min_nodes=1, combine=False, int_buffer=default_int_buffer, bbox=None,
    ...
    network = getNetFromFile(osm_filename, network_type, link_type, POIs, POI_percentage, strict_mode, offset, min_nodes,
                             combine, int_buffer, bbox, default_lanes, default_speed)
    return network


def getNetFromPBFFile(pbf_filename='map.osm.pbf', network_type=('auto',), link_type='all', POIs=False, POI_percentage=1.0,
                      strict_mode=True, offset=0, min_nodes=1, combine=False, int_buffer=default_int_buffer, bbox=None,
                      default_lanes=False, default_speed=False):

    print('Warning: getNetFromOSMFile() and getNetFromPBFFile() are deprecated and will be removed in a future release.\n'
          '         Please use getNetFromFile().')
    network = getNetFromFile(pbf_filename, network_type, link_type, POIs, POI_percentage, strict_mode, offset, min_nodes,
                             combine, int_buffer, bbox, default_lanes, default_speed)
    return network


def getNetFromFile(filename='map.osm', network_type=('auto',), link_type='all', POIs=False, POI_percentage=1.0,
                   strict_mode=True, offset=0, min_nodes=1, combine=False, int_buffer=default_int_buffer, bbox=None,
                   default_lanes=False, default_speed=False):
    # check arguments

    network_type_valid = _getValidNetworkType(network_type)
    network = readOSMFile(filename, POIs, POI_percentage, strict_mode, bbox)
    _buildNet(network, network_type_valid, link_type, POIs, POI_percentage, offset, min_nodes, combine, int_buffer, default_lanes, default_speed)
    return network
