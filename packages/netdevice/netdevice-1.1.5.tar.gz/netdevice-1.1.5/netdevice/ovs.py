...
class OvsHost(linux.LinuxDevice):
    ...
    def add_device (self, bridge_name, *args, **kwargs):
        ...
        for d in args:
            ...
            if dtype == "container":
                self.add_container(bridge_name, **d)
                macaddr = self.cmd(
                    "docker exec -it %s ip link show %s | grep link/ether"
                    %(d["name"], d["interface"]), log_level = 4)
                d["mac"] = macaddr.split()[1]
                d["ofport"] = self.ovs_get_container_ofport(d["name"])
            elif dtype == "qemu-system-x86_64":
                self.add_vm(**d)
            else:
                self.log("device type %s is not supported now..." %(dtype))

    def add_container (self, bridge_name,
            name = "con1",
            type = "docker",
            host_dir = "/var/shared",
        ...
        # Add the offlow for the same subnet.
        if ip != "" and self["fail-mode"] == "secure":
            # Confiure the default offlow for subnet.
            ipaddr = ipaddress.ip_interface(ip)
            ofport = self.ovs_get_container_ofport(name)
            self.ofctl_add_flow(bridge_name,
                "priority=65535,arp,arp_tpa=%s actions=output:%s"
                %(ipaddr.ip, ofport),
                "priority=10000,ip,nw_src=%s,nw_dst=%s actions=output:%s" 
                %(ipaddr.network, ipaddr.ip, ofport))
    ...
    def ovs_get_interface (self, interface):
        '''
        Get a self-port which connect to the kwargs["name"]
        '''
        i = {}
        result = self.cmd("ovs-vsctl list interface %s" %(interface), log_level=4)
        for p in StringIO(result).readlines():
            k,v = p.split(":", 1)
            i[k.strip()] = v.strip()
        return i

    def ovs_port_to_ofport (self, name):
        '''
        Get a ofport at given interface
        '''
        #i = self.ovs_get_interface(name)
        #return i.get("ofport", None)
        return self.cmd("ovs-vsctl get interface %s ofport" %(name)).strip()
    ...
    def ovs_set_interface (self, name, **kwargs):
        '''
        Get a self-port which connect to the kwargs["name"]
        '''
        cmd = "ovs-vsctl set interface %s" %(name)
        for k,v in kwargs.items():
            cmd += ' %s=%s' %(k,v)
        self.cmd(cmd)
