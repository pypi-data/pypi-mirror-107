...
class DB:
  ...
  def collections(self):
    return self._dbName.list_collection_names()
  ...
  Data is returned as a [list] list with the _id field.
  """
  def findAll(self):
    mycol = DB.__createCol(self)
    lst = []
  ...
  If there is only one, that field will be omitted if there is a 0:
    {"job": 0}
  Data is returned as a [list] list, with ID if otherwise specified.
  ...
  If [ascending] is not specified, a descending [list] list will be returned.
  ...
  Returns a [list] list of the updated collection.
  """
  ...
  def updateMany(self, oldquery: dict, newquery: dict):
    mycol = DB.__createCol(self)
    lst = []
    x = mycol.delete_many(oldquery, newquery)
