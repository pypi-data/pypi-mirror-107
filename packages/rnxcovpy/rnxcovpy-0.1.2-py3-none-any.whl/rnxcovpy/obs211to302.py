# -*- coding: utf-8 -*-
# @Time : 2021/4/24 16:48

# -*- coding: utf-8 -*-
# @Time : 2021/4/17 16:46
...
def doconvert(filepath):
    ...
    list_obs_type302_G = list()  # 3.02版本的GPS系统的观测类型
    list_obs_type302_R = list()  # 3.02版本的GLONASS系统的观测类型
    list_obs_type302_E = list()  # 3.02版本的Galileo系统的观测类型
    list_obs_type302_S = list()  # 3.02版本的SBAS的观测类型
    # -----------------------------
    obs_type302_G_conv = list()  # GPS系统的观测值2.11对应3.02的关系
    obs_type302_R_conv = list()  # GLONASS系统的观测值2.11对应3.02的关系
    obs_type302_E_conv = list()  # Galileo系统的观测值2.11对应3.02的关系
    obs_type302_S_conv = list()  # SBAS系统的观测值2.11对应3.02的关系
    ...
    for singleline in infilelist[0:loc_flag]:
            ...
            if len(list_obs_type211) == num_obs_type211:  # 观测数据类型添加完整之后对观测值类型进行转换
                ...
                for i in range(0, len(list_obs_type211)):
                    if list_obs_type211[i][1] == '1':
                        list_obs_type302_G.append(list_obs_type211[i])
                        list_obs_type302_R.append(list_obs_type211[i])
                        list_obs_type302_E.append(list_obs_type211[i])
                        list_obs_type302_S.append(list_obs_type211[i])
                        obs_type302_G_conv.append(1)
                        obs_type302_R_conv.append(1)
                        obs_type302_E_conv.append(1)
                        obs_type302_S_conv.append(1)
                        ...
                        obs_type302_E_conv.append(1)
                        obs_type302_S_conv.append(1)
                    elif list_obs_type211[i][1] == '6' or list_obs_type211[i][1] == '7' or list_obs_type211[i][
                        1] == '8':
                        list_obs_type302_E.append(list_obs_type211[i])
                        obs_type302_G_conv.append(-1)
                        obs_type302_R_conv.append(-1)
                ...
                if 'P1' in list_obs_type211:
                    list_obs_type302_E.remove('P1')
                    list_obs_type302_S.remove('P1')
                    obs_type302_E_conv[list_obs_type211.index('P1')] = -1
                    obs_type302_S_conv[list_obs_type211.index('P1')] = -1

                # 对GPS和GLONASS系统的观测类型进行转换
                list_obs_type302_G = [el.obs211to302_obs_map_G[i] if i in el.obs211to302_obs_map_G else
                                      i + ' ' for i in list_obs_type302_G]
                list_obs_type302_R = [el.obs211to302_obs_map_R[i] if i in el.obs211to302_obs_map_R else
                                      i + ' ' for i in list_obs_type302_R]
                list_obs_type302_E = [i + ' ' for i in list_obs_type302_E]
                list_obs_type302_S = [i + ' ' for i in list_obs_type302_S]

                # 转换为符合格式标准的字符串添加到列表outfilelist中
                type_obs_str_G = el.obs211to30x_obs_typr_str(list_obs_type302_G, 'G')
                type_obs_str_R = el.obs211to30x_obs_typr_str(list_obs_type302_R, 'R')
                type_obs_str_E = el.obs211to30x_obs_typr_str(list_obs_type302_E, 'E')
                type_obs_str_S = el.obs211to30x_obs_typr_str(list_obs_type302_S, 'S')
                if 'G' in sate_sys:
        ...
        elif headerlable not in el.obs211to302_funcdic.keys():
            print(f'Warning:"{headerlable}" is unidentifiable!')
        else:
            outfilelist.append(el.obs211to302_funcdic[headerlable](singleline))  # 根据文件头标签调用相应的函数
        ...
        for i in range(0, lines_of_epoch_star, line_of_one_star):
            ...
            if current_sate_sys == 'G':
                for num in range(0, num_obs_type211):
                    if obs_type302_G_conv[num] == -1:
                        obs_str = obs_str[:16 * i] + ' ' * 16 + obs_str[16 * (i + 1):]
            elif current_sate_sys == 'R':
                for num in range(0, num_obs_type211):
                    if obs_type302_R_conv[num] == -1:
                        obs_str = obs_str[:16 * i] + ' ' * 16 + obs_str[16 * (i + 1):]
            elif current_sate_sys == 'E':
                for num in range(0, num_obs_type211):
                    if obs_type302_E_conv[num] == -1:
                        obs_str = obs_str[:16 * i] + ' ' * 16 + obs_str[16 * (i + 1):]
            elif current_sate_sys == 'S':
                for num in range(0, num_obs_type211):
                    if obs_type302_S_conv[num] == -1:
                        obs_str = obs_str[:16 * i] + ' ' * 16 + obs_str[16 * (i + 1):]
