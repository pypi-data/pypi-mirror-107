# -*- coding: utf-8 -*-
# @Time : 2021/5/14 23:30

# -*- coding: utf-8 -*-
# @Time : 2021/5/14 14:30


"""可能有问题 @time-2021年5月15日09:32:22"""
...
def doconvert(filepath):
    ...
    for i in range(0, loc_flag):
        if infilelist[i].find("RINEX VERSION / TYPE") != -1:
            infilelist[i] = infilelist[i].replace("3.02", "3.05")
            ...
            if system == "G":
                for j in range(0, len(obs_type)):
                    if len(obs_type[j]) == 2:
                        obs_type[j] = el.obs211to305_obs_map_G[obs_type[j]]
                ...
                for j in range(0, len(obs_type)):
                    if len(obs_type[j]) == 2:
                        obs_type[j] = el.obs211to305_obs_map_R[obs_type[j]]
                obs_str = infilelist[i][0:7] + " ".join(obs_type)
                obs_str = obs_str + ' ' * (60 - len(obs_str)) + infilelist[i][60:]
                infilelist[i] = obs_str
            elif system == "E":
                for j in range(0, len(obs_type)):
                    if len(obs_type[j]) == 2:
                        obs_type[j] = el.obs211to305_obs_map_E[obs_type[j]]
                obs_str = infilelist[i][0:7] + " ".join(obs_type)
                obs_str = obs_str + ' ' * (60 - len(obs_str)) + infilelist[i][60:]
                infilelist[i] = obs_str
            elif system == "S":
                for j in range(0, len(obs_type)):
                    if len(obs_type[j]) == 2:
                        obs_type[j] = el.obs211to305_obs_map_S[obs_type[j]]
                obs_str = infilelist[i][0:7] + " ".join(obs_type)
                obs_str = obs_str + ' ' * (60 - len(obs_str)) + infilelist[i][60:]
                infilelist[i] = obs_str
    return infilelist
