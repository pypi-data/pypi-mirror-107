# -*- coding: utf-8 -*-
# @Time : 2021/4/24 22:06

# -*- coding: utf-8 -*-
# @Time : 2021/4/23 10:35
...
def doconvert(filepath):
    ...
    for singleline in infilelist[0:loc_flag]:
        if singleline.find("SYS / # / OBS TYPES") != -1:
            ...
            obs_type302_linestr = singleline[7:60].strip()

            obs_type302_linelist = obs_type302_linestr.split()  # 每行的观测类型列表 如['C1C', 'C2W', 'C2X', 'D1C', 'D2W']
            # 判断卫星系统并对每一行的观测类型进行处理
            if sate_sys == 'G':
                el.obs30xto211_convobstype(obs_type211, obs_type302_linelist,
                                           data_map_G, rank_G, el.obs302to211_obs_map_G_rank)
            elif sate_sys == 'R':
                el.obs30xto211_convobstype(obs_type211, obs_type302_linelist,
                                           data_map_R, rank_R, el.obs302to211_obs_map_R_rank)
            elif sate_sys == 'E':
                el.obs30xto211_convobstype(obs_type211, obs_type302_linelist, data_map_E,
                                           rank_E, el.obs302to211_obs_map_E_rank)
            elif sate_sys == 'S':
                el.obs30xto211_convobstype(obs_type211, obs_type302_linelist,
                                           data_map_S, rank_S, el.obs302to211_obs_map_S_rank)
            else:
                continue
        else:
            continue

    # obs_type211 = list(obs_type211)
    obs_type211_num = len(obs_type211)  # 转换后的观测类型的数量

    # 文件头----------------------------------------------------------------------------
    for singleline in infilelist[0:loc_flag]:
        headerlable = singleline[60:-1].strip()  # *文件头的各行的标签*
        if headerlable not in el.obs302to211_funcdic.keys():
            ...
            outfilelist.append(el.obs302to211_funcdic[headerlable](singleline))
            outfilelist.append("     1     1     0                                          WAVELENGTH FACT L1/2\n")
        else:
            outfilelist.append(el.obs302to211_funcdic[headerlable](singleline))  # 根据文件头标签调用相应的函数
    # 文件头转换完成----------------------------------------------------------------------------------
    ...
    for singleline in infilelist[loc_flag:]:
        if singleline[0] == '>':
            ...
            if epoch_star_num % 12 == 0:
                ...
                epoch_star_prn = epoch_star_prn + singleline[0:3]
            temp_obs_str302 = singleline[3:-1]  # 不带行尾换行符
            a = len(temp_obs_str302)
            b = len(data_map_G) * 16
            if a < b:
                temp_obs_str302 = temp_obs_str302 + ' ' * (b - a)
            ...
            for i in range(0, len(data_map_G)):
                if data_map_G[i] != -1:
                    temp_obs_str211[data_map_G[i] * 16 + int(data_map_G[i] / 5):data_map_G[i] * 16 + int(
                        data_map_G[i] / 5) + 16] = temp_obs_str302[i * 16:i * 16 + 16]
            ...
            for i in range(0, len(data_map_R)):
                if data_map_R[i] != -1:
                    # temp_obs_str211[data_map_R[i] + int(data_map_R[i] / 5)] = temp_obs_str302[i * 16:i * 16 + 16]
                    temp_obs_str211[data_map_R[i] * 16 + int(data_map_R[i] / 5):data_map_R[i] * 16 + int(
                        data_map_R[i] / 5) + 16] = temp_obs_str302[i * 16:i * 16 + 16]
                else:
                    continue
            epoch_converted_data.append(''.join(temp_obs_str211))
        elif singleline[0] == 'E':
            ...
            for i in range(0, len(data_map_E)):
                if data_map_E[i] != -1:
                    # temp_obs_str211[data_map_E[i] + int(data_map_E[i] / 5)] = temp_obs_str302[i * 16:i * 16 + 16]
                    temp_obs_str211[data_map_E[i] * 16 + int(data_map_E[i] / 5):data_map_E[i] * 16 + int(
                        data_map_E[i] / 5) + 16] = temp_obs_str302[i * 16:i * 16 + 16]
                else:
                    continue
            epoch_converted_data.append(''.join(temp_obs_str211))
        elif singleline[0] == 'S':
            ...
            for i in range(0, len(data_map_S)):
                if data_map_S[i] != -1:
                    # temp_obs_str211[data_map_S[i] + int(data_map_S[i] / 5)] = temp_obs_str302[i * 16:i * 16 + 16]
                    temp_obs_str211[data_map_S[i] * 16 + int(data_map_S[i] / 5):data_map_S[i] * 16 + int(
                        data_map_S[i] / 5) + 16] = temp_obs_str302[i * 16:i * 16 + 16]
                else:
                    continue
            epoch_converted_data.append(''.join(temp_obs_str211))
        else:
