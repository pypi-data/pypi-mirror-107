...
telegraph = Telegraph()
telegraph.create_account(short_name="Lion Cmds List")
...
UPSTREAM_REPO_URL = "https://github.com/TeamLion-X/Lion-x"
...
async def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>Lion {Lion_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"Lion {Lion_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def updater():
    ...
    try:
        ...
        await Lion_bot.asst.send_message(
...
async def get_user_id(ids):
    if str(ids).isdigit():
        ...
        try:
            userid = (await Lion_bot.get_entity(ids)).id
...
async def create_token_file(token_file, event):
    ...
    async with Lion_bot.asst.conversation(Lion_bot.uid) as conv:
        await event.edit(
            f"Go to the following link in your browser: [Authorization Link]({authorize_url}) and reply the code",
            link_preview=False,
        )
        response = conv.wait_event(events.NewMessage(from_users=Lion_bot.uid))
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "Uploaded using Lion Userbot",
        "mimeType": mime_type,
    }
    if parent_id is not None:
    ...
    try:
        try:
            for i in LOADED[shortname]:
                Lion_bot.remove_event_handler(i)
            try:
            ...
            for i in reversed(range(len(Lion_bot._event_builders))):
                ev, cb = Lion_bot._event_builders[i]
                if cb.__module__ == name:
                    del Lion_bot._event_builders[i]
                    try:
                        del LOADED[shortname]
                        del LIST[shortname]
                        ADDONS.remove(shortname)
...
async def restart(ult):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        execl(executable, executable, "-m", "pyLion")
...
async def get_chatinfo(event):
    ...
    if not chat:
        if event.reply_to_msg_id:
            ...
            chat = event.chat_id
    try:
        chat_info = await Lion_bot(GetFullChatRequest(chat))
    except BaseException:
        try:
            chat_info = await Lion_bot(GetFullChannelRequest(chat))
...
async def fetch_info(chat, event):
    chat_obj_info = await Lion_bot.get_entity(chat.full_chat.id)
    broadcast = (
    ...
    if admins is None:
        try:
            participants_admins = await Lion_bot(
            ...
            if "(" not in downloaded_file_name:
                ...
                try:
                    ...
                    if plug in HELP:
                        ...
                        await ok.edit(f"✓ `Lion-X - Installed`: `{plug}` ✓\n\n{kk}")
                        await asyncio.sleep(9)
                        await ok.delete()
                    else:
                        try:
...
async def allcmds(event):
    ...
    t = telegraph.create_page(title="Lion-X All Cmds", content=[f"{xx}"])
    w = t["url"]
    await eod(event, f"All Lion-X Cmds : [Click Here]({w})", link_preview=False)
...
async def randomchannel(tochat, channel, range1, range2, caption=None):
    ...
    async for x in Lion_bot.iter_messages(channel, add_offset=do, limit=1):
        try:
            if x.media and caption:
                await Lion_bot.send_file(tochat, file=x, caption=caption)
                return
            await Lion_bot.send_message(tochat, x)
        except BaseException:
            try:
                await Lion_bot.send_file(tochat, x)
            except BaseException:
                pass
