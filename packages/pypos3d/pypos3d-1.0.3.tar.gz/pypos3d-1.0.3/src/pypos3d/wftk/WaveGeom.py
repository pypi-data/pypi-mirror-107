...
#
  ...
  def optimizeGroups(self, cleaning=False, radius=0.0):
    ...
    for grp in self.groups:
      grp.vertIdx[:] = [ mapVert[vi] for vi in grp.vertIdx ]
    ...
    if hasTexture:
      ...
      for grp in self.groups:
        grp.tvertIdx[:] = [ mapTVert[vti] for vti in grp.tvertIdx ]
    ...
    if hasNormal:
      ...
      for grp in self.groups:
        grp.normIdx[:] = [ mapNVert[vti] for vti in grp.normIdx ]
...
#           nidx += 1
  ...
  def calcGroupVertIndex(self, grp):
    ...
    #       svi |= set(grp.vertIdx[grp.stripCount[faceno]:grp.stripCount[faceno + 1]])
    svi = set( grp.vertIdx )
