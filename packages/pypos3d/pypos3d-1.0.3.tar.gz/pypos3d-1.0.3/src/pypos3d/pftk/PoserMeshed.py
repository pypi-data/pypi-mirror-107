    ...
    try:
      ...
      if len(tor)>1:
        ...
        if tor[0]=='(' and tor[-1]==')': # The orientation is given by a Tuple of float
          ...
          n = Point3d.triangle_normal(vxtab[0], vxtab[1], vxtab[2])
          if AxisOrder=='yzx':
            rotz = math.asin(-n.x)
            cz   = math.cos(rotz)
            rotx = math.acos(n.y / cz) if math.fabs(cz)>FEPSILON else 0.0 #
          elif AxisOrder=='xyz':
            roty = math.asin(n.z)
            cy   = math.cos(roty)
            rotz = math.acos(n.x/cy) if math.fabs(cy)>FEPSILON else 0.0
          else: # 'zxy'
            rotx = math.asin(-n.y)
            cx   = math.cos(rotx)
            roty = math.acos(n.z/cx) if math.fabs(cx)>FEPSILON else 0.0
      ...
      ret = fp.status(C_FAIL, 'Definition error for [{:s}]', self.getName())
