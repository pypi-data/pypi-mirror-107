      ...
      if fp.partType[:3]=='act': # Actor Creation
        ...
        if isinstance(fp.geom, dict):
          for wfmat in fp.geom.values():
            self.addMaterial(PoserMaterial(wfmat))
          
        elif isinstance(fp.geomGroup, Figure):
          lstMat = fp.geomGroup.getLstMaterial() if fp.name=='*' else [ fp.geomGroup.getMaterial(fp.name), ]
          
          for mat in lstMat: # Add each material in self
            self.addMaterial(mat)
            
        else: # partGeom is supposed to be an Actor/Prop
          if fp.geomGroup.getCustomMaterial():
            for mat in [ a for a in fp.geomGroup.getLstAttr() \
                         if (a.getPoserType()==PoserToken.E_material) and (fp.name=='*' or fp.name==a.getName()) ]:
              self.addMaterial(mat)
          else:
