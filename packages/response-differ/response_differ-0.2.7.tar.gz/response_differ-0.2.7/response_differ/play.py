...
def import_pages(path):
    def _import():
        ...
        return SourceFileLoader(name, os.path.join(root, file))

    pwd = os.path.abspath(os.curdir)
    for root, _dirs, files in os.walk(pwd):
        for file in files:
            if file in path:
                _import()
...
def get_vcr():
    if Serialize.custom_serializer_path:
        path = Serialize.custom_serializer_path[0]
        pwd = os.path.abspath(os.curdir)
        for root, _dirs, files in os.walk(pwd):
            for file in files:
                if file in path:
                    name = os.path.splitext(file)[0]
                    serializer = SourceFileLoader(name, os.path.join(root, file)).load_module()
    else:
