...
class SpeechAudio:
    ...
    def sparse_transform(self, W_nominal, W_mask, nwin=240, novlp=None):
        ...
        Sxx_target = sparse_transform(Sxx, W_nominal, W_mask)
        k = 0
        isC = True
        while isC:
            xwin, fft_xwin, isC, idx = spec_frame(x2, k, nwin, nhop, nfft)
            h_orig = np.abs(Sxx[:, k])
            h_new = np.abs(Sxx_target[:, k])
            x_out = change_frame(fft_xwin, h_new, h_orig,
                                         nfft, idx, x_out, xwin=xwin, isNorm=False)
    ...
    def frequency_warping(self, dil_fact, trans_fact, dist=None, ord_filt=None,
        ...
        while isC:
            xwin, fft_xwin, isC, idx = spec_frame(x2, k, nwin, nhop, nfft)
            h_orig = np.abs(Sxx[:, k])
            if (form_in.frequency[:,k] != 0).all():
                # new_freq = ceps2peaks(h_orig, form_in.frequency[:,k], fxx)
                new_freq = form_in.frequency[:,k]
                h_new = dfw(h_orig, fxx, np.sort(form_output.frequency[:,k]), new_freq)
            else:
                h_new = 1
    
            x_out = change_frame(fft_xwin, h_new, h_orig,
                                         nfft, idx, x_out, xwin=xwin,
                                         isChange=(form_in.frequency[:,k] != 0).all())
