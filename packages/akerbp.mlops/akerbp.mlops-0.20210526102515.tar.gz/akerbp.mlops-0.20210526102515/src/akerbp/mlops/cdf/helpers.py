...
def call_function_thread(function, data):
    """
    Make an asynchronous call to a function deployed in CDF

    Input:
        - function: (CDF Function) a retrieved function 
        - data: (dictionary) data for the call
    """
    t_start = time.process_time()
    call = function.call(data, wait=False)
    logging.debug(f"Called function ({call.id=})")
    while call.status == "Running":
        ...
        try:
            ...
            logging.warning(f"Cognite API error ({call.id=})")
    if call.status == "Completed":
        response = call.get_response()
        t_proc = time.process_time() - t_start
        logging.debug(f"Call complete ({call.id=}, {t_proc=}): {response=}")
    else:
        logging.debug(f"Error ({call.id=})")
        response = call.error
    
    return response


def call_function_concurrent_thread(function_name, data):
    ...
    with ThreadPoolExecutor(max_workers) as p:
        futures = [p.submit(call_function_thread, function, d) for d in data]    
    return [f.result() for f in futures]


async def call_function_async(function, data):
    """
    Make an asynchronous call to a function deployed in CDF
...
async def call_function_concurrent_async(function_name, data):
    """
    Concurrent calls to a CDF function
    ...
    responses = await asyncio.gather(
        *(call_function_async(function, d) for d in data)
    )
    return responses


def call_function_concurrent(function_name, data):
    """
    Concurrent calls to a CDF function
    This is a synchronous wrapper for `call_function_concurrent_async`

    Input:
        - function_name: (string) function's external id in CDF
        - data: (list of dictionaries) list of data for the call
    """
    return asyncio.run(call_function_concurrent_async(function_name, data))


def call_function_process_wrapper_2(function_name, data):
    """
    Set up the cdf client and call a function. This wrapper makes it possible to call a function from an independent processess invoked by the multiprocessing library
    ...
    """
    set_up_cdf_client(context='deploy')
    return call_function_concurrent(function_name, data)


def call_function_parallel_2(function_name, data, n_calls=None):
    """
    Make parallel calls to a function deployed in CDF
    ...
    """
    if not n_calls:
        n_calls = os.cpu_count()
    if n_calls > len(data):
        n_calls = len(data)
    logging.debug(f"Parallel calls with {n_calls=}")
    
    # Split data in n_calls lists of data
    data = [data[n::n_calls] for n in range(n_calls)]
    f = partial(call_function_process_wrapper_2, function_name)
    with Pool(n_calls) as p:
        return p.map(f, data)
