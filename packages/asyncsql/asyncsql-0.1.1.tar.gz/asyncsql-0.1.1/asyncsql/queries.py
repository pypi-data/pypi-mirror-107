...
class Queries:
    ...
    def get_cleaned_data(self, model: Model, create: bool = False) -> dict:
        ...
        return {k: v for k, v in model if k not in exclude_fields if v}
...
""".strip()
    ...
    def get_order_by(self, direction: str = "asc", order_fields: list = ()) -> (str):
        return ", ".join(
            f"{f} {direction.upper()}"
            for f in (order_fields or self.order_fields or [self.id_field])
            if f in self.model_cls.__fields__
        )

    def get_select_by_id_sql(self):
        return f"SELECT * FROM {self.table} WHERE id = $1"

    def get_select_json_sql(self, order_by: str = "", where: str = None):
...
""".strip()
    ...
    def get_select_sql(self, order_by: str = "", where: str = None):
        order_by = order_by or self.get_order_by()
        return f"""
WITH rows AS (
  SELECT
...
""".strip()
    ...
    async def insert(self, conn: Connection, model: Model):
        ...
        return self.model_cls(**{**model.dict(), **dict(row.items())})
    ...
    async def select(
        ...
        sql = self.get_select_sql(order_by=order_by, where=where)
    ...
    async def select_json(
        ...
        sql = self.get_select_json_sql(order_by=order_by, where=where)
