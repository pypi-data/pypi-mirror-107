...
class Remotable:
    ...
    def _pid(self):
        cls = self.__class__
        return cls.__module__, cls.__name__, self._identity
...
class Context:
    ...
    @classmethod
    def _application(cls, connect, launch, process_name=None):
    ...
    @classmethod
    def _connect(cls, host, port, timeout=5):
        ...
        if hasattr(socket, 'SIO_LOOPBACK_FAST_PATH'):
            try:
                ...
                if exc.winerror != _WSAEOPNOTSUPP:
                    raise
