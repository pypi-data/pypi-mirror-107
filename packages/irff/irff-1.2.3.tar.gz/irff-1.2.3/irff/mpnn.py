...
class MPNN(ReaxFF):
      ...
      for bd in self.bonds: 
          ...
          if self.nbd[bd]>0:
             ...
             if self.spv_be:
                ...
                fe  = tf.where(tf.less_equal(self.rbd[bd],self.re[bd]),1.0,0.0) ##### r< r_e that bo > bore_
                self.diffe[bd]  = tf.reduce_sum(input_tensor=tf.nn.relu((0.2*self.p['Desi_'+bd]+self.EBD[bd])*fe))
                l_atol  = tf.add(self.diffe[bd],l_atol)

             if self.EnergyFunction == 1:
                fe  = tf.where(tf.less_equal(self.bo0[bd],self.botol),1.0,0.0) ##### bo <= 0.0 that e = 0.0
             ...
             if self.regularize:                                               # regularize
                pen_bo_w  = tf.reduce_sum(tf.square(self.m['fsiwi_'+bd]))
                pen_bo_w += tf.reduce_sum(tf.square(self.m['fpiwi_'+bd]))
                pen_bo_w += tf.reduce_sum(tf.square(self.m['fppwi_'+bd]))
                for l in range(self.bo_layer[1]):                                               
                    pen_bo_w += tf.reduce_sum(tf.square(self.m['fsiw_'+bd][l]))
                    pen_bo_w += tf.reduce_sum(tf.square(self.m['fpiw_'+bd][l]))
                    pen_bo_w += tf.reduce_sum(tf.square(self.m['fppw_'+bd][l]))
                pen_bo_w += tf.reduce_sum(tf.square(self.m['fsiwo_'+bd])) 
                pen_bo_w += tf.reduce_sum(tf.square(self.m['fpiwo_'+bd])) 
                pen_bo_w += tf.reduce_sum(tf.square(self.m['fppwo_'+bd])) 

                for t in range(1,self.messages+1):
                    pen_bo_w  += tf.reduce_sum(tf.square(self.m['f'+str(t)+'wi_'+bd]))
                    for l in range(self.mf_layer[1]):                                               
                        pen_bo_w += tf.reduce_sum(tf.square(self.m['f'+str(t)+'w_'+bd][l]))
                    pen_bo_w += tf.reduce_sum(tf.square(self.m['f'+str(t)+'wo_'+bd])) 

                pen_bo_w  = tf.reduce_sum(tf.square(self.m['fewi_'+bd]))
                for l in range(self.be_layer[1]):                                               
                    pen_bo_w += tf.reduce_sum(tf.square(self.m['few_'+bd][l]))
                pen_bo_w += tf.reduce_sum(tf.square(self.m['fewo_'+bd])) 
                l_atol= tf.add(self.lambda_pen*pen_bo_w,l_atol)

             l_atol= tf.add(tf.nn.relu(self.rc_bo[bd]-self.rcut[bd]),l_atol)
      return l_atol*self.penalty
  ...
  def get_bondorder(self,t,bd,atomi,atomj):
      ...
      if self.MessageFunction==1:
         ...
         Fj    = self.fmessage(flabel,bdr,self.nbd[bd],[Dbj,Dbi,h],layer=self.mf_layer[1])
         F     = Fi*Fj
