...
def np_calc_learning_rate_non_sucrose(df, path, phase, group_dict, no_lick_ex, no_lick_rem, no_lick_only, nameMod="", mean_no_med=1):
    # create out path
    ...
    for i in range(len(sheets)):
        ...
        chart.set_title({'name': 'Learning Rate per Individual',})
        ...
        chart2.set_title({'name': 'Learning Rate per Group',})
...
def np_calc_learning_rate_sucrose(df, path, phase, group_dict, no_lick_ex, no_lick_rem, no_lick_only, water_y_suc_n,
                                  water_y_suc_y, sucrose_label, mean_no_med=1):
    ...
    if water_y_suc_n:
        ...
        np_calc_learning_rate_non_sucrose(df_yn, path, phase, group_dict, no_lick_ex, no_lick_rem,
                                          no_lick_only, nameMod="_" +sucrose_label+ "TreatedAsIncorrect", mean_no_med= mean_no_med)
    if water_y_suc_y:
        df_yy = df.copy()
        ...
        np_calc_learning_rate_non_sucrose(df_yy, path, phase, group_dict, no_lick_ex, no_lick_rem,
                                          no_lick_only, nameMod="_" +sucrose_label+ "TreatedAsCorrect", mean_no_med= mean_no_med)
    ...
    for i in range(len(sheets)):
        ...
        chart.set_y_axis({'name': 'Lick Duration: ' + sucrose_label + '/Water Ratio', })
...
def vis_calc_learning_rate(df, path, phase, group_dict, mean_no_med=1):
    # define intervalls
    df["hourIntervall"] = df["StartTimecode"] // 3600
    ...
    for i in range(len(sheets)):
        ...
        if "side" in sheets[i] or "Side" in sheets[i]:
            chart.set_title({'name': 'Side Error Learning Rate per Individual', })
        else:
            chart.set_title({'name': 'Place Error Learning Rate per Individual', })
        ...
        if "side" in sheets[i] or "Side" in sheets[i]:
            chart2.set_title({'name': 'Side Error Learning Rate per Group', })
        else:
            chart2.set_title({'name': 'Place Error Learning Rate per Group', })
...
def vis_calc_pivot(df, path, phase, group_dict, hour_intervals=[], mean_no_med=1):
    ...
    for i in range(len(sheets)):
        ...
        for j in range(len(df_pivot_list[i].index) - len(group_dict)):
            ...
            #todo: access error bars via "j"
            # also unintend set-stuff
...
tk.Radiobutton(master, text="Pivot Group Analysis Option: Mean + Standard Error",
...
for j in range(len(phases)):
    ...
    if curr_visit_df.empty:
        ...
        vis_calc_learning_rate(curr_visit_df.copy(), path=analysis_dir + "/" + phases[j], phase=phases[j],
                               group_dict=group_dict, mean_no_med= mean_no_med)
        vis_calc_pivot(curr_visit_df.copy(), path=analysis_dir + "/" + phases[j], phase=phases[j],
                       group_dict=group_dict,
                       hour_intervals=hour_intervals, mean_no_med= mean_no_med)
        ...
        if sucrose_label:
            if sucrose_label in curr_nosepoke_df["SideCondition"].unique():
                np_calc_learning_rate_sucrose(curr_nosepoke_df.copy(), path=analysis_dir + "/" + phases[j],
                                              ...
                                              water_y_suc_y=water_y_suc_y, sucrose_label=sucrose_label, mean_no_med= mean_no_med)
            else:
                np_calc_learning_rate_non_sucrose(curr_nosepoke_df.copy(), path=analysis_dir + "/" + phases[j],
                                              phase=phases[j], group_dict=group_dict, no_lick_ex=no_lick_ex.get(),
                                              no_lick_rem=no_lick_rem.get(),
                                              no_lick_only=no_lick_only.get(), mean_no_med= mean_no_med)
        else:
            np_calc_learning_rate_non_sucrose(curr_nosepoke_df.copy(), path=analysis_dir + "/" + phases[j],
                                              phase=phases[j], group_dict=group_dict, no_lick_ex=no_lick_ex.get(),
                                              no_lick_rem=no_lick_rem.get(),
