...
from solace.messaging.config.solace_properties.message_properties import APPLICATION_MESSAGE_TYPE, ELIDING_ELIGIBLE, \
    ...
    PERSISTENT_DMQ_ELIGIBLE, PERSISTENT_ACK_IMMEDIATELY, SEQUENCE_NUMBER, APPLICATION_MESSAGE_ID
...
def validate_message_props(props: dict):
    ...
    for key in props:
        if key in properties_type_mapping:
            if not type(props[key]) == properties_type_mapping[key]:  # pylint: disable=unidiomatic-typecheck
                if isinstance(props[key], bool):
                    return
                raise InvalidDataTypeError(INVALID_EXPECTED_DATATYPE.substitute(expected_type=type(props[key]),
                                                                                actual_type=properties_type_mapping[
                                                                                    key]))
