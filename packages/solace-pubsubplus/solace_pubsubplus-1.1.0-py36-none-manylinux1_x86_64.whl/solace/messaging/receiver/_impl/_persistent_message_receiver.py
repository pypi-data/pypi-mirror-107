...
from solace.messaging.utils._solace_utilities import get_last_error_info, is_type_matches, is_not_negative, \
    convert_ms_to_seconds, COMPLETED_FUTURE, _ThreadingUtil, _Released
...
class _PersistentMessageReceiver(_MessageReceiver, PersistentMessageReceiver) \
    ...
    def __init__(self, persistent_message_receiver: '_PersistentMessageReceiverBuilderImpl') \
        ...
        self._missing_resource_strategy = self._persistent_message_receiver.missing_resources_creation_strategy
        self._replay_strategy = self._persistent_message_receiver.replay_strategy
    ...
    def _event_callback_routine(self, _opaque_flow_p, event_info_p, _user_p) \
        ...
        if event in [_SolClientFlowEvent.SOLCLIENT_FLOW_EVENT_DOWN_ERROR.value,
                     _SolClientFlowEvent.SOLCLIENT_FLOW_EVENT_SESSION_DOWN.value]:
    ...
    def _notify_metrics_and_cleanup(self, error_info):
        ...
        self.__do_metrics()
        self._message_receiver_queue = Queue()  # reset queue
