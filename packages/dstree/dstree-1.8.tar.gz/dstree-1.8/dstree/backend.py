...
mapping_type2size = {int: 0, np.integer : 0, NoneType: 0}
...
def compress(cls: Options, compress_level):
    if compress_level == 1:
        ...
        cls.print_max_children = {"depth_multiplier": 1, "upperbound_constant": 5, "exclude_parent_classes": [dict, Undefined_class]}
...
def generate_tree(root_obj):
    ...
    while stack:
        ...
        if len(children) == 0:
            continue
        # Key-Value pair type
        if parent_node.dtype_mapped in (dict, Undefined_class):
...
if __name__ == "__main__":
    ...
    default_options.print_max_children = {"depth_multiplier" : 3, "upperbound_constant" : 5, "exclude_parent_classes" : [dict, Undefined_class]}
