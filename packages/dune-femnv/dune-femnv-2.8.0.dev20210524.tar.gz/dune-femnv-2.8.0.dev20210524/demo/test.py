...
spc1 = create.space("Lagrange", grid, dimrange=1, order=2, storage='istl')
...
for i in range(5):
    ...
    l2_error = l2_norm(grid, solution[1] - exact[0])
    h1_error = h1_norm(grid, solution[1] - exact[0])
    ...
    print(info,math.sqrt(l2_error),', l2 eoc:',l2_eoc,', h1 eoc:',h1_eoc,', time:',total_t)
    grid.writeVTK("test"+str(i),pointdata=[solution])
